...
module.exports = extend(exports, {
    ...
    qmks: qmks,
    running: new Promise(function (resolution, rejection) {
        process.once('mysql', function running() {
            debug('running');
            resolution(new Date);
        });
    }),
...
process.running.then(function () {
    ...
    process.once('terminate', function _mysql() {
        ...
        setTimeout(function _mysqlTimeout() {
            ...
            delay < 0 && (delay = 1);
            debug('terminate: delay', delay.toFixed(3) + 's');
...
function qmks(array) {
    ...
    return '?'.repeat(array.length).split('').join();
}
