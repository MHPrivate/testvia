...
function flush(done) {
    ...
    chain(function cleanup(err) {
        ...
        //Process for AWS if needed, otherwise keep local DNS processing
        if (main.modules.aws.getIsAwsEnvironment()) {
            // AWS Route53 Update DNS
            // Pass the DB results as input
            main.modules.aws.route53UpdateDNS(fqdns, this);
        } else {
            // Use OVH DNS management
        ...
        // AWS gives HTTP respose code, so translate to 0 = OK
        if (main.modules.aws.getIsAwsEnvironment()) {
            let rc = (code["$metadata"] || {}).httpStatusCode;
            // Translate response to 0 for OK if HTTP response from AWS Route53 update is 200-299
            code = (rc && rc >= 200 && rc < 300) ? 0 : rc;
        }
