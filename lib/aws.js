...
async function getPublicIps() {
    ...
    if (apiVersion==1) {
        ...
        //myIps['v4'] = await getPublicIpV4_apiv1();
        //myIps['v6'] = await getPublicIpV6_apiv1();
        if (ipV4 || ipV6) {
            myIps = {};
            myIps['v4'] = ipV4;
            myIps['v6'] = ipV6;
    ...
    if (apiVersion==2) {
        try{
            ...
            if (response.NetworkInterfaces && response.NetworkInterfaces.length) {
                ips={};
                ips['v4'] = response.NetworkInterfaces[0].Association.PublicIp;
                ips['v6'] = response.NetworkInterfaces[0].Ipv6Address;
            }
            return ips;
        } catch (error) {
                console.log(error);
