...
process.running.then(async ()=>{
    ...
    if (inAws) {
        ...
        srandmemberAsync = promisify(main.modules.rpscb.pub.srandmember).bind(main.modules.rpscb.pub);
        ...
        try {
            ...
            //If no numbers loaded, or qty_in_use = 0, reload
...
async function redisLoadE164s( e164Array) {
    try {
        //Load/Reset the E164s
        debug.enabled && debug("Re/Storing E164 numbers in redis", e164s);
        await delAsync( set_numbers_available );
        await delAsync( set_numbers_in_use );
        await saddAsync.apply(this,[ set_numbers_available ].concat(e164s));
...
async function reserveTrainingDID() {
    ...
    selectedE164 = await srandmemberAsync("numbers_available");
    if (selectedE164 == null) {
        debug.enabled && debug("No Training numbers_available in Redis")
        let numbers_loaded = await getAsync("numbers_loaded");
        consoldebug.enabled && debug("Redis Training numbers loaded=", numbers_loaded);
        if (!numbers_loaded || numbers_loaded=="0") {
            debug.enabled && debug("No Redis training numbers yet loaded when fetching - reloading")
            await redisLoadE164s(e164s);

            selectedE164 = await srandmemberAsync("numbers_available");
            debug.enabled && debug("In Load - Moving available to in_use:", selectedE164);
            await smoveAsync("numbers_available", "numbers_in_use", selectedE164);
            return selectedE164;
        }

    }
    if (selectedE164) {
        debug.enabled && debug("Outside Load - Moving available to in_use:", selectedE164);
        await smoveAsync("numbers_available", "numbers_in_use", selectedE164);
        return selectedE164;
    }
    return selectedE164;
...
function releaseTrainingDID ( e164Number ) {
    ...
    smoveAsync("numbers_in_use", "numbers_available", e164Number);
}
