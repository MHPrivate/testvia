...
async function reserveTrainingDID() {
    ...
    selectedE164 = await srandmemberAsync(set_numbers_available);
    if (selectedE164 == null) {
        debug.enabled && debug("No Training numbers_available in Redis")
        let numbers_loaded = await getAsync(key_numbers_loaded_flag);
        debug.enabled && debug("Redis Training numbers loaded=", numbers_loaded);
        if (!numbers_loaded || numbers_loaded=="0") {
            ...
            await smoveAsync(set_numbers_available, set_numbers_in_use, selectedE164);
            return selectedE164;
        }
