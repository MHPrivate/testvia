...
process.running.then(function apns() {
    ...
    exports.sub = fork(require.resolve('./apns-sub.js'), { execArgv: [], stdio: debug.enabled ? 'inherit' : 'ignore' }); // at launch - not dynamic
    // need to 'ignore' stdio to avoid obscure excessive diagnostics from @parse/node-apn
});
