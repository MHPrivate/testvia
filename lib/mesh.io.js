...
function onMesh(peer, uptime) { // _this_ is a Mesh server-side Socket
    debug.apply(null, [this.server ? 'S' : 'C', 'onMesh:', this.id].concat(Array.from(arguments)));
    ...
    debug.enabled && debug('onMesh:', this.id, 'stashed - count =', Object.keys(this.mesh.peers).length);
...
Mesh.fixClient = function fixClient(soc, peer) {
    if (soc.mesh.peer || soc.mesh.clients.includes(soc)) // already adopted by onMesh() OR fixClient() - no further fixup required
        return;
...
Mesh.connection = function onConnection(soc) { // _this_ is a Mesh listener Socket
    ...
    soc.conn.once('heartbeat', function () { // _this_ is the engine.io - cleanup 'mesh' listener if not received by 1st heartbeat
        ...
        debug.enabled && debug.apply(null, [soc.server ? 'S' : 'C', 'onHeartbeat:', soc.id, soc._eventsCount ? 'retain' : 'disconnect'].concat(Array.from(arguments)));
        ...
        if (soc.mesh.clients.includes(soc)) // already adopted by onMesh() OR fixClient() - no further fixup required
            return;
