...
Mesh.fixPeer = function fixPeer(soc, peer) {
    if (soc.mesh.peer) // fixup no necessary for a mesh.io based client
        return;
    soc.mesh.peer = peer;
    soc.mesh.peers[peer] = soc;
    debug.enabled && debug(soc.server ? 'S' : 'C', 'fixPeer:', soc.id, 'stashed - count =', Object.keys(soc.mesh.peers).length);
    for (var handler in Mesh.handlers)
