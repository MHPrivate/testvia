...
Mesh.handlers = { // _this_ is a Socket, client-side has no server-attribute AND server-side has no close-method
    ...
    disconnect: function onDisconnect(reason) { // both client/server
        debug.enabled && debug.apply(null, [this.server ? 'S' : 'C', 'onDisconnect:', this.mesh.id, this.mesh.peer].concat(Array.from(arguments)));
        ...
        if (this.mesh.peers[this.mesh.peer] === this) {
            ...
            debug.enabled && debug('onDisconnect:', this.mesh.id, 'unstashed - count =', Object.keys(this.mesh.peers).length);
        }
    },
    //disconnecting: function onDisconnecting(reason) {
    //    debug.enabled && debug.apply(null, [this.server ? 'S' : 'C', 'onDisconnecting:', this.id, this.mesh.peer].concat(Array.from(arguments)));
    //},
