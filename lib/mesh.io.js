...
function onceMesh(peer, uptime) { // _this_ is a Mesh server-side Socket
    debug.apply(null, [this.server ? 'S' : 'C', 'onceMesh:', this.id].concat(Array.from(arguments)));
...
Mesh.connection = function onConnection(soc) { // _this_ is a Mesh listener Socket
    ...
    soc.conn.once('heartbeat', function () { // _this_ is the engine.io - cleanup 'mesh' listener if not received by 1st heartbeat
        ...
        debug.enabled && debug.apply(null, [soc.server ? 'S' : 'C', 'onceHeartbeat:', soc.id, soc._eventsCount ? 'retain' : 'disconnect'].concat(Array.from(arguments)));
...
Mesh.handlers = { // _this_ is a Socket, client-side has no server-attribute AND server-side has no close-method
    connect: function onConnect() { // only client-side
        debug.enabled && debug.apply(null, [this.server ? 'S' : 'C', 'onConnect:', this.id, this.mesh.peer].concat(Array.from(arguments)));
    ...
    //    debug.enabled && debug.apply(null, [this.server ? 'S' : 'C', 'onDisconnect:', this.id, this.mesh.peer].concat(Array.from(arguments)));
    //},
    disconnecting: function onDisconnecting(reason) { // both client/server
        debug.enabled && debug.apply(null, [this.server ? 'S' : 'C', 'onDisconnecting:', this.mesh.id, this.mesh.peer].concat(Array.from(arguments)));
    ...
    //    debug.enabled && debug.apply(null, [this.server ? 'S' : 'C', 'onHeartbeat:', this.id, this.mesh.peer, this.conn.transport.__proto__.constructor.name].concat(Array.from(arguments)));
    //},
};
