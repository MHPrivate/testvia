...
function onceMesh(peer, uptime) { // _this_ is a Mesh server-side Socket
    debug.apply(null, [this.server ? 'S' : 'C', 'onceMesh:', this.id].concat(argsMap(arguments)));
    ...
    debug.enabled && debug('onceMesh:', this.id, 'stashed - count =', Object.keys(this.mesh.peers).length);
...
Mesh.fixClient = function fixClient(soc, peer) {
    if (soc.mesh.peer || soc.mesh.clients.includes(soc)) // already adopted by onceMesh() OR fixClient() - no further fixup required
        return;
    soc.conn.on('heartbeat', events.prototype.emit.bind(soc, 'heartbeat'));
    soc.mesh.clients.push(soc);
    if (peer) // is a peer - so add to peers dictionary
        soc.mesh.peers[soc.mesh.peer = peer] = soc;
    debug.enabled && debug(soc.server ? 'S' : 'C', 'fixClient:', 'clients =', soc.mesh.clients.length, soc.id, 'peers =', Object.keys(soc.mesh.peers).length);
    for (var handler in Mesh.handlers)
        soc.on(handler, Mesh.handlers[handler]);
}

Mesh.connection = function onConnection(soc) { // _this_ is a Mesh listener Socket
    debug(soc.server ? 'S' : 'C', 'onConnection:', soc.id, soc.handshake.address);
    ...
    Object.defineProperty(soc, 'mesh', { // server-side setup
        ...
        value: Object.create(soc.server.mesh, { // server-side setup
            ...
            id: { configurable: true, enumerable: true, value: soc.id, writable: true },
            peer: { configurable: true, enumerable: true, writable:true }, // placeholder
            server: { configurable: true, enumerable: true, value: true },
        }),
    });
    soc.once(soc.server.mesh.opts.meshEvent || 'mesh', onceMesh);
    soc.conn.once('heartbeat', function () { // _this_ is the engine.io - cleanup 'mesh' listener if not received by 1st heartbeat
        if (soc.mesh.peer) // valid client 'mesh' event OR fixClient call received so no further 'heartbeat' processing here
            return;
        soc.removeListener(soc.server.mesh.opts.meshEvent || 'mesh', onceMesh);
        debug.enabled && debug.apply(null, [soc.server ? 'S' : 'C', 'onceHeartbeat:', soc.id, soc._eventsCount ? 'retain' : 'disconnect'].concat(argsMap(arguments)));
        ...
        if (soc.mesh.clients.includes(soc)) // already adopted by onceMesh() OR fixClient() - no further fixup required
            return;
        console.log('WARNING: mesh.io catchall-fixup as fixClient() was not called when these handlers were attached:', JSON.stringify(Object.keys(soc._events)));
        soc.mesh.clients.push(soc);
        for (var handler in Mesh.handlers)
            soc.on(handler, Mesh.handlers[handler]);
    });
...
Mesh.handlers = { // _this_ is a Socket, client-side has no server-attribute AND server-side has no close-method
    connect: function onConnect() { // only client-side
        debug.enabled && debug.apply(null, [this.server ? 'S' : 'C', 'onConnect:', this.id, this.mesh.peer].concat(argsMap(arguments)));
        this.mesh.id = this.id;
        if (typeof this.mesh.opts.meshUptime === 'number') // diagnostic override
    ...
    connect_error: function onConnectError(err) { // only client-side
        debug.enabled
            ? debug.apply(null, [this.server ? 'S' : 'C', 'onConnectError:', this.mesh.id, this.mesh.peer, err.message])
            : console.log('mesh.io:', this.mesh.peer, err.message);
    },
    //disconnect: function onDisconnect(reason) {
    //    debug.enabled && debug.apply(null, [this.server ? 'S' : 'C', 'onDisconnect:', this.id, this.mesh.peer].concat(argsMap(arguments)));
    //},
    disconnecting: function onDisconnecting(reason) { // both client/server
        debug.enabled && debug.apply(null, [this.server ? 'S' : 'C', 'onDisconnecting:', this.mesh.id, this.mesh.peer].concat(argsMap(arguments)));
        if (this.subs) // will re-establish
            return;
        var idx = this.mesh.clients.indexOf(this);
        ~idx && this.mesh.clients.splice(idx, 1);
        if (this.mesh.peers[this.mesh.peer] === this) {
            delete this.mesh.peers[this.mesh.peer];
            debug.enabled && debug('onDisconnecting:', this.mesh.id, 'unstashed - count =', Object.keys(this.mesh.peers).length);
        }
    },
    //heartbeat: function onHeartbeat() { // only server-side - useful for discarding unvalidated clients
    //    debug.enabled && debug.apply(null, [this.server ? 'S' : 'C', 'onHeartbeat:', this.id, this.mesh.peer, this.conn.transport.__proto__.constructor.name].concat(argsMap(arguments)));
    //},
};
...
Mesh.listening = function listening() { // _this_ is a Mesh listener Socket
    ...
    debug.enabled && debug.apply(null, ['L', 'listening:', this.mesh.url].concat(argsMap(arguments)));
    return this;
}
...
function addPeer(url, opts) { // _this_ is a Mesh listener Socket
    debug.enabled && debug.apply(null, ['L', 'addPeer:'].concat(argsMap(arguments)));
    ...
    if (url === this.mesh.url) // silently prevent connection to ourself
        return;
    var soc = this.mesh.peers[url]; // preserve existing where present
    if (soc) // connection already present
        return;

    // connect to the new peer
    soc = this.mesh.peers[url] = cio(url, opts); // socket.io-client module
    debug.enabled && debug('addPeer:', soc.id, 'stashed - count =', Object.keys(this.mesh.peers).length);
...
function close(/* ... */) { // mesh.io - _this_ is a Mesh listener Socket
    debug.enabled && debug.apply(null, ['L', 'close:'].concat(argsMap(arguments)));
    ...
    for (var key in this.mesh.peers)
        if (this.mesh.peers[key].subs) { // active client-side connection to server-peer
            debug('closePeer:', this.mesh.peers[key].id, key);
...
function emit2peers(event /* , ... */) {
    debug.enabled && debug.apply(null, ['L', 'emit2peers:'].concat(argsMap(arguments)));
...
function listen(/* ... */) { // mesh.io - _this_ is a Mesh listener Socket
    debug.enabled && debug.apply(null, ['L', 'listen:'].concat(argsMap(arguments)));
