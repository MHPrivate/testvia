...
function iokNotify(hqData, cb) { // {endUtc, devices:{<uuid>:bool, ...}}, cb(err)
    ...
    var body = { // this is cloned for each request
        ...
        time_to_live: 0, // instantaneous delivery OR discard
        //notification: {
        //    endUtc: hqData.endUtc, // maybe undefined and therefore absent
        //    //iok: <boolean>,
        //},
    ...
    var locals = {
        ...
        uuids: Object.keys(hqData.devices),
        chanStates: [],
        tokenIds: {},
    };
    chain(cb, function () {
        ...
        var chanState;
        for (var n in apps) {
            locals.tokenIds[apps[n].token] = apps[n].id; // for failed-token clean-up
            for (var chan = locals.channels.length - 1; chan > 0; --chan) // 2, 1 or default 0
                if (locals.channels[chan].pattern && (apps[n].userAgent || '').includes(locals.channels[chan].pattern))
                    break;
            chanState = chan << 2; // chan:bool << 2
            chanState += hqData.devices[apps[n].aptUuid] === null ? 2 : +!!hqData.devices[apps[n].aptUuid]; // false:0 true:1 disabled:2
        ...
        while (chanState < locals.chanStates.length && !locals.chanStates[chanState])
            chanState++;
        var chan = chanState >> 2, tokens = locals.chanStates[chanState];
        ...
        json.data.iokNotify = (chanState & 3) > 1 ? {} : { endUtc: hqData.endUtc, iok: Boolean(chanState & 1) };
        //json.notification = json.data.iokNotify;
        ...
        chain(this, function () {
            ...
            debug.enabled && debug('iokNotify: options', JSON.stringify(options));
            request(options, this.noerror);
...
function voipNotify(hqData, cb) { // {devices:[<duid>, ...],?callerId,?hasVideo}, cb(err, delayMs)
    ...
    chain(cb, function () {
        ...
        chain(this, function () {
            ...
            debug.enabled && debug('voipNotify: options', JSON.stringify(options));
            request(options, this.noerror);
