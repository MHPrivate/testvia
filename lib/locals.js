...
async function local(req, res, next) {
    ...
    if (req.socket.remoteAddress === req.socket.address().address) // e.g. '::ffff:51.254.118.61'
        return next();
    //If AWS, check public IP
<<<<<<< HEAD
    if (main.aws.getIsAwsEnvironment()) {
        let awsPublicIps = await main.aws.getPublicIps()
        if (awsPublicIps && awsPublicIps.v4 && req.socket.remoteAddress == ('::ffff:' + awsPublicIps.v4))
=======
    if (main.modules.aws.getIsAwsEnvironment()) {
        let awsPublic = await main.modules.aws.getPublicIps()
        if (awsPublicIps && awsPublicIps.v4 && awsreq.socket.remoteAddress == ('::ffff:' + awsPublicIps.v4))
>>>>>>> 9c4e5247dd9c6d0688883261f2654cf3a407caf9
            return next();
    }
    next('route');
};
