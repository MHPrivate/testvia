...
process.running.then(init.bind(0, 'IPSETS' in process.env)); // force=false
function init(force) {
...
process.on('larcPresent', function _ipsets(ipstr, soc) { exports('larcs', 0, ipstr) }); // soc absent for heartbeats
process.on('larcDropped', function _ipsets(ipstr, soc) { exports('larcs', 30000, ipstr) }); // soc absent for heartbeats
function exports(base, ms, ipstr) {
    exports.sets[base] || (exports.sets[base] = { drops: {}, keeps: {} });
    var familySet = base + (ipstr.includes(':') ? '6' : '4');
    if (!ipstr) {
        debug('????: missing ipstr');
    } else if (ms) { // mark for discard
        debug('drop:', familySet, ipstr, ms);
        exports.sets[base].drops[ipstr] = new Date(Date.now() + ms);
        exports.timeout || (exports.timeout = main.running && setTimeout(onTimeout, ms));
    } else if (exports.sets[base].drops[ipstr]) { // was marked for discard - unmark it
        debug('keep:', familySet, ipstr);
        exports.sets[base].keeps[ipstr] = new Date;
        delete exports.sets[base].drops[ipstr];
    } else if (exports.sets[base].keeps[ipstr]) { // already known update last seen
        debug('here:', familySet, ipstr);
        exports.sets[base].keeps[ipstr] = new Date;
    } else { // unknown - make known
