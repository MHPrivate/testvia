...
//curl -X POST -H 'Content-Type: application/json' -d '{"section": ...}' http://localhost:8020
app.post('/', function (req, res, next) {
    ...
    console.log(JSON.stringify(extend(null, {
        ...
        'Caller-Caller-ID-Number': query['Caller-Caller-ID-Number'],
    ...
    chain(next, function () {
        ...
        var calledCaller = [query['Caller-Destination-Number'], query['Caller-Caller-ID-Number']];
        mysql('select * from sipFilters where called=substring(?,1,length(called)) and caller=substring(?,1,length(caller)) order by length(called) desc,length(caller) desc', calledCaller, this);

    }, function (sipFilters, meta) { // [{id,called,caller,flags}]
        locals.sipFilters = sipFilters;
        if (((sipFilters[0] || {}).flags || '-')[0] === '-')
            return res.type('text/xml').render(query.section, Object.assign(locals, { hangup: 'CALL_REJECTED' }));
        ...
        sipUsers.some(function (sipUser, idx, arr) {
            ...
            if (locals.utc < sipUser.value.expires)
                return Object.assign(locals, {
                    dialPrefix: sipUser.value.reg_user,
                    ...
                    appelloExpedite: jwt.sign({
                        ...
                        exp: locals.utc + 5, // 5sec lifetime
                        name: undefined,
                        numb: undefined,
                        prfx: sipUser.value.reg_user,
...
function directory(req, res, next) {
    ...
    chain(next, function () {
        ...
        if (user)
            mysql('select * from sipUsers where user in ("",?) or user=(select name from sipUsers where user=? and type="group")', [user, user], this);
        else
            mysql('select * from sipUsers where user in ("",?) or user in (select user from sipUsers where name="registration" and value like ?)', [user, `%${os.hostname()}%`], this);
...
function expedite(req, cb) {
    ...
    if (query.section !== 'dialplan'
        || !(query.variable_sofia_profile_name || '').startsWith('ext') // sip_profile - external, ext4udp, ext4tls, ...
