...
app.get('/', function (req, res, next) {
    var query = req.query;
    console.log(JSON.stringify(extend(null, { // skips undefined(s)
    ...
    chain(next, function () {
        ...
        mysql('select dialPrefix from schemes where scheme is not null and dialPrefix=substring(?, 1, length(dialPrefix)) order by length(dialPrefix) desc', [query['Caller-Destination-Number']], this);

    }, function (schemes, meta) {
        locals.dialPrefix = schemes && schemes[0] && schemes[0].dialPrefix;
        mysql('select * from config where nameSlashed=substring(?,1,length(nameSlashed)) order by length(nameSlashed) desc', ['/proxy/' + query['Caller-Destination-Number']], this);

    }, function (prefixes, meta) {
        try {
            locals.cone = new url.URL('x://' + prefixes[0].valueString).hostname;
