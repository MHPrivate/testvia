...
function onRoutingLookup(session, cb) {
    ...
    acquireToken().then(function (res) { // onRoutingLookup
                        ...
                        <link-entity name="sbr_scheme" from="sbr_schemeid" to="sbr_schemeid" alias="S" link-type="inner" visible="false">
                            ...
                            <filter type="and">
                                ...
                                <filter type="or">
                                    ...
                                    <condition attribute="sbr_groupedequipmentphone" operator="eq" value="${'+' + payload.e164}"/>
                                </filter>
                            </filter>
                        </link-entity>
                    </entity>
        ...
        (context.schemes = res.data.value).filter(function (scheme, idx, arr) {
            if (scheme['sbr_datasourceid@OData.Community.Display.V1.FormattedValue'] !== dataSource) // grouped dataSource
                return false; // discard any where the incoming dataSource does not match

            if (scheme['sbr_schemenumber'] !== schemenumber) // unmatched schemenumber, therefore must match the CLI
                return true;
