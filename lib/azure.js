...
// startup actions
process.running.then(function azureStartup() {
    var context = {};

    var hr = process.hrtime();
    acquireToken().then(function (res) { // azureStartup
        debug('accessToken:', hrMs(process.hrtime(hr)) + 'ms');
                ...
                fetchXml: `
                    <fetch version="1.0" output-format="xml-platform" mapping="logical" distinct="false">
                        <entity name="sbr_device">
                            <all-attributes/>
                            <link-entity name="sbr_scheme" from="sbr_schemeid" to="sbr_schemeid" visible="false" link-type="outer" alias="S">
                                <all-attributes/>
                            </link-entity>
                            <filter type="and">
                                <condition attribute="statecode" operator="eq" value="0"/>
        ...
        exports.defaultDevice = res.data.value[0]; // sbr_callcode, sbr_name, sbr_eventcodeid, S.sbr_name
        ...
        debug('atStartup:', err, err.response.data.error && ('\n' + err.response.data.error.message));
...
function onRoutingLookup(session, cb) {
    ...
    acquireToken().then(function (res) { // onRoutingLookup
        ...
        context.dataSourceClause = (context.dataSource || payload.dataSource) && `operator="eq" value="${context.dataSource || payload.dataSource}"`;
        if (lineType === 'Grouped') {
            context.deviceXml = `
                <fetch version="1.0" output-format="xml-platform" mapping="logical" distinct="true">
                    <entity name="sbr_device">
                        ...
                        <link-entity name="sbr_devicecharacteristic" from="sbr_deviceid" to="sbr_deviceid" visible="false" link-type="outer" alias="DC">
                            <attribute name="sbr_devicecharacteristicvalueidname"/>
                            <filter>
                                <condition attribute="sbr_devicechardictionaryidname" operator="eq" value="Data Source"/>
                            </filter>
                        </link-entity>
                        <filter type="and">
                            <condition attribute="statecode" operator="eq" value="0"/>
                            <condition attribute="sbr_devicenumber" operator="eq" value="${+payload.unit}"/>
                            <!-- Grouped -->
                            <condition attribute="sbr_linetype" operator="eq" value="858810000"/>
                            <condition entityname="DC" attribute="sbr_devicecharacteristicvalueidname" ${context.dataSourceClause || nullClause}/>
                            <condition entityname="S" attribute="sbr_schemenumber" operator="eq" value="${+payload.scheme}"/>
                        </filter>
                    </entity>
                </fetch>
            `.replace(/\r?\n\s*/g, '');
                ...
                <fetch version="1.0" output-format="xml-platform" mapping="logical" distinct="false">
                    <entity name="sbr_device">
                        ...
                        <filter type="and">
                            ...
                            <filter type="or">
                                <filter type="and">
                                    <condition attribute="sbr_devicenumber" operator="eq" value="${payload.originUser}"/>
                                    <condition entityname="DC" attribute="sbr_devicecharacteristicvalueidname" ${context.dataSourceClause || nullClause}/>
                                </filter>
                                <filter type="and">
        ...
        hr = process.hrtime();
        return context.deviceXml && axios({
            method: 'get',
            baseURL: main.secrets.appello.azureBaseUrl + main.secrets.appello.azureDataUrl,
            httpsAgent: main.cache.keepaliveagent,
            headers: {
        ...
        context.device = (res && res.data.value[0]) || exports.defaultDevice;
        debug(session.sid, 'device:', context.deviceMs + 'ms', JSON.stringify(context.device));
        ...
        if (err.response.data.error)
            debug(communicator.session.sid, 'onRoutingLookup:', xml, '\n' + err.response.data.error.message);
        else
            debug(communicator.session.sid, 'onRoutingLookup:', xml, err);
...
function onBsiaSpawnCalls(catalogue, communicator, cb) {
    ...
    acquireToken().then(function (res) { // bsiaSpawnCalls
                ...
                fetchXml: xml = context.deviceXml = `
                    <fetch version="1.0" output-format="xml-platform" mapping="logical" distinct="false">
                        <entity name="sbr_device">
                            ...
                            <link-entity name="sbr_devicecharacteristic" from="sbr_deviceid" to="sbr_deviceid" visible="false" link-type="outer">
                                <!-- many-to-many joins cannot contribute attributes -->
                                <filter>
                                    <condition attribute="sbr_devicechardictionaryidname" operator="eq" value="Data Source"/>
                                    <condition attribute="sbr_devicecharacteristicvalueidname" ${context.dataSourceClause || nullClause}/>
                                </filter>
                            </link-entity>
                            <filter type="and">
                                <condition attribute="statecode" operator="eq" value="0"/>
                                <filter type="or">
                                    <filter type="and">
                                        <condition attribute="sbr_devicenumber" operator="eq" value="${payload.originUser}"/>
                                        <condition attribute="sbr_devicerole" operator="eq" value="858810003"/><!-- SecurityDialler -->
                                    </filter>
                                    <filter type="and">
                                        <condition attribute="sbr_equipmentphone" operator="eq" value="${'+' + payload.e164}"/>
                                        <condition attribute="sbr_linetype" operator="eq" value="858810001"/><!-- Dispersed -->
                                    </filter>
                                </filter>
                            </filter>
                        </entity>
        ...
        context.deviceMs = hrMs(process.hrtime(hr));
        if (res.data.value.some(function (row, idx, arr) {
            return (row.sbr_devicenumber === payload.originUser) && (context.device = row);
        })) // found a device with a matching devicenumber
            null;
        else if (res.data.value.length === 1) // exactly one device with matching CLI
            context.device = res.data.value[0];
        else // NOT precisely one device with matching CLI
            context.device = exports.defaultDevice;
        debug(communicator.session.sid, 'device:', context.deviceMs + 'ms', JSON.stringify(context.device));
        ...
        context.authorityClause = context.device['S._sbr_authorityid_value'] && `operator="eq" value="${context.device['S._sbr_authorityid_value']}"`;
        context.protocolClause = exports.protocols[payload.protocol] && `operator="eq" value="${exports.protocols[payload.protocol].sbr_protocolid}"`;
        hr = process.hrtime();
        return axios({
            method: 'get',
        ...
        if (err.response.data.error)
            debug(communicator.session.sid, 'onBsiaSpawnCalls:', xml, '\n' + err.response.data.error.message);
        else
            debug(communicator.session.sid, 'onBsiaSpawnCalls:', xml, err);
...
function onWriteCallData(session, cb) {
    var context = session.context,
        ...
        now = new Date,
        payload = session.payload;
    if (!context.accessToken || context.cdr) { // SBR not relevant for this call OR already written
        debug(session.sid, 'onWriteCallData:', JSON.stringify({ accessToken: !!context.accessToken, cdr: !!context.cdr }));
        return cb && cb();
    }

    context.protocolId = (exports.protocols[payload.protocol] || exports.protocols['Unknown']).sbr_protocolid;
    Object.assign(context, context.truncate
        ? {
    ...
    if (Array.isArray(payload.bsia)) { // BSIA parent call
        ...
        if (!context.cdr.sbr_extendedevent)
    ...
    hr = process.hrtime();
    return context.cdr && axios({ // onWriteCallData
        method: 'post',
        baseURL: main.secrets.appello.azureBaseUrl + main.secrets.appello.azureDataUrl,
        headers: {
        ...
        if (err.response.data.error)
            debug(session.sid, 'onWriteCallData:', JSON.stringify(context.cdr), '\n' + err.response.data.error.message);
        else
            debug(session.sid, 'onWriteCallData:', JSON.stringify(context.cdr), err);
