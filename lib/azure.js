...
function onRoutingLookup(session, cb) {
    ...
    acquireToken().then(function (res) { // onRoutingLookup
        ...
        return axios(azure.axios = {
            ...
            url: 'sbr_callevents',
            params: {
                fetchXml: xml = azure.calleventsXml,
            },
        });
        ...
        (payload.events || [payload.event]).forEach(function (event, idx, arr) {
            ...
            if (eventCode)
                context.truncate = context.truncate || eventCode._sbr_calleventid_value in azure.callevents; // aka autoAnswer
        });
        ...
        return axios(azure.axios = {
            ...
            params: {
                fetchXml: xml = azure.callroutingsXml,
            },
        });
        ...
        return axios(azure.axios = {
            ...
            params: {
                fetchXml: xml = azure.routingendpointsXml,
            },
        });
    ...
    acquireToken().then(function (res) { // bsiaSpawnCalls
        ...
        for (row in catalogue) { // array of {account,channels,status} e.g. [{ account: '717273', channels: '65555555', status: '9' }]
            ...
            for (chan = 0; chan < catalogue[row].channels.length; ++chan) { // string of 8, 16 or 24 digits e.g. '65555555'
                ...
                eventKey = [payload.protocol, status, azure.device['sbr_linetype@OData.Community.Display.V1.FormattedValue'] || 'Dispersed'].join('/');
                eventCode = exports.eventCodes[eventKey] || {};
                truncate = context.truncate || eventCode._sbr_calleventid_value in azure.callevents; // aka autoAnswer
                debug(communicator.session.sid, UTIL.stringify({ chan: chan + 1, status: status, tails: tails, col: azure.device[col + '@OData.Community.Display.V1.FormattedValue'], eeellpss: eeellpss, truncate: truncate }));
            ...
            eventCode = exports.eventCodes[eventKey] || {};
            truncate = context.truncate || eventCode._sbr_calleventid_value in azure.callevents; // aka autoAnswer
            debug(communicator.session.sid, UTIL.stringify({ chan: 25, status: status, tails: exports.statusNowipTails, col: azure.device[col + '@OData.Community.Display.V1.FormattedValue'], eeellpss: eeellpss, truncate: truncate }));
            eeellpss && children.push({
                account: catalogue[0].account,
                channel: '' + 25,
        ...
        for (var child in children) {
            ...
            debug(communicator.session.sid, 'spawn: child session', session.sid);
    ...
    acquireToken().then(function (res) { // onWriteCallData
        ...
        if (Array.isArray(payload.bsia)) { // BSIA parent call
            ...
            context.cdr = azure.device && {
                ...
                sbr_devicecallcode: payload.event,
                'sbr_deviceid@odata.bind': azure.device.sbr_deviceid && `/sbr_devices(${azure.device.sbr_deviceid})`,
                sbr_deviceident: payload.grouped ? +payload.unit : (payload.originUser || azure.device.sbr_devicenumber),
                sbr_diagnostics: diagnostics,
                sbr_direction: payload.outbound ? 858810002 : 858810000, // B-party Outdial or In
                sbr_endpoint: false, // Offsite
