...
function onRoutingLookup(session, cb) {
    ...
    acquireToken().then(function (res) { // onRoutingLookup
        ...
        (payload.events || [payload.event]).forEach(function (event, idx, arr) {
            ...
            // When we are happy that DS specific event codes can be safely loaded w/o implacting live traffic, we can uncomment the following to enable..
            //dsEventKey = dataSource + '/' + eventKey; // Allows support for DS specific event codes
