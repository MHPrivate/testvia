...
function onRoutingLookup(session, cb) {
    ...
    acquireToken().then(function (res) { // onRoutingLookup
        ...
        debug(session.sid, 'rl-device:', res.data.value.length, 'item(s)', context.deviceMs + 'ms', JSON.stringify(context.device));
        ...
        debug(session.sid, 'rl-scheme:', res.data.value.length, 'item(s)', context.schemeMs + 'ms', JSON.stringify(context.scheme));
        ...
        debug(session.sid, 'rl-callevents:', context.calleventsMs + 'ms', JSON.stringify(context.callevents));
        ...
        debug(session.sid, 'rl-callroutings:', context.callroutingsMs + 'ms', JSON.stringify(context.callroutings));
        ...
        debug(session.sid, 'rl-routingendpoints:', context.routingendpointsMs + 'ms', JSON.stringify(context.routingendpoints));

    }).then(function () { // onRoutingLookup
        if (context.truncate) // allow mysql configJson to prevail if present
            return;

        debug(session.sid, 'rl-callCodes:', JSON.stringify(context.callCodes));
...
function onBsiaSpawnCalls(catalogue, communicator, cb) {
    var cli,
        context = communicator.session.context,
        devicenumber,
        payload = communicator.session.payload;

    debug(communicator.session.sid, 'bsia: spawnOffspring', JSON.stringify({ catalogue: catalogue, context: context })); // [{ account, channels, status }, ...]
    ...
    acquireToken().then(function (res) { // bsiaSpawnCalls
        ...
        if (!context.device)
            context.device = cliDevices.length === 1 ? cliDevices[0] : (exports.defaultDevices[dataSource] || exports.defaultDevices[null]);
        debug(communicator.session.sid, 'sp-device:', res.data.value.length, 'item(s)', context.deviceMs + 'ms', JSON.stringify(context.device));
        ...
        debug(communicator.session.sid, 'sp-callevents:', res.data.value.length, 'item(s)', context.calleventsMs + 'ms', JSON.stringify(context.callevents));
        ...
        context.callroutingsMs = hrMs(process.hrtime(hr));
        context.callroutings = res.data.value;
        debug(communicator.session.sid, 'sp-callroutings:', res.data.value.length, 'item(s)', context.callroutingsMs + 'ms', JSON.stringify(context.callroutings));
        ...
        debug(communicator.session.sid, 'sp-routingendpoints:', res.data.value.length, 'item(s)', context.routingendpointsMs + 'ms', JSON.stringify(context.routingendpoints));
        debug(communicator.session.sid, 'sp-callCodes:', JSON.stringify(context.callCodes));
        ...
        for (row in catalogue) { // array of {account,channels,status} e.g. [{ account: '717273', channels: '65555555', status: '9' }]
            debug(communicator.session.sid, 'row:', JSON.stringify({ row: catalogue[row], context: context }));
            for (chan = 0; chan < catalogue[row].channels.length; ++chan) { // string of 8, 16 or 24 digits e.g. '65555555'
                ...
                debug(communicator.session.sid, JSON.stringify({ chan: chan + 1, status: status, tails: tails, col: context.device[col + '@OData.Community.Display.V1.FormattedValue'], eeellpss: eeellpss, truncate: truncate }));
            ...
            eventCode = exports.eventCodes[eventKey] || {};
            truncate = context.truncate || eventCode._sbr_calleventid_value in context.callevents; // aka autoAnswer
            debug(communicator.session.sid, JSON.stringify({ chan: 25, status: status, tails: exports.statusNowipTails, col: context.device[col + '@OData.Community.Display.V1.FormattedValue'], eeellpss: eeellpss, truncate: truncate }));
            eeellpss && children.push({
                account: catalogue[0].account,
                channel: '' + 25,
...
function onWriteCallData(session, cb) {
    ...
    if (context.cdr) { // CDR already written
        ...
        debug(session.sid, 'onWriteCallData:', JSON.stringify({ device: null, protocol: payload.protocol }));
        return cb && cb();
    }
    ...
    acquireToken().then(function (res) { // onWriteCallData
        ...
        if (!context.scheme && cliSchemes.length === 1)
            context.scheme = cliSchemes[0];
        debug(session.sid, 'wd-scheme:', res.data.value.length, 'item(s)', context.schemeMs + 'ms', JSON.stringify(context.scheme));
        ...
        debug(session.sid, 'sbr_call:', context.cdr.$, context.cdrMs + 'ms', JSON.stringify(context.cdr));
        ...
        if (err.response && err.response.data.error)
            debug(session.sid, 'onWriteCallData:', hint, JSON.stringify(context.axios), '\n' + err.response.data.error.message);
        else
            debug(session.sid, 'onWriteCallData:', hint, JSON.stringify(context.axios), err);
