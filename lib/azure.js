...
function onRoutingLookup(session, cb) {
    ...
    acquireToken().then(function (res) { // onRoutingLookup
        ...
        debug(session.sid, 'deviceXml:', context.deviceXml);
        hr = process.hrtime();
        ...
        var cliDevices = (context.devices = res.data.value).filter(function (device, idx, arr) {
            if ((device['sbr_equipmentphone'] || device['S.sbr_groupedequipmentphone']) === cli)
                return true;

            if ((device['_sbr_datasourceid_value@OData.Community.Display.V1.FormattedValue'] // dispersed device dataSource
                || device['S.sbr_datasourceid@OData.Community.Display.V1.FormattedValue'] // grouped device dataSource
            ) !== dataSource) // either/or - none OR only one of the dispersed/grouped dataSources _may_ be defined on the row
                return false; // discard any where the incoming dataSource does not match
        ...
        if (!session.communicator.grouped || +context.device.sbr_devicenumber !== -2)
            return;
        ...
        `.replace(/\r?\n\s*/g, '');;
        debug(session.sid, 'schemeXml:', context.schemeXml);
        hr = process.hrtime();
        return axios(context.axios = {
            method: 'get',
            baseURL: main.secrets.appello.azureBaseUrl + main.secrets.appello.azureDataUrl,
        ...
        (context.schemes = res.data.value).filter(function (scheme, idx, arr) {
            if (scheme['sbr_groupedequipmentphone'] === cli)
                return true;

            if (scheme['_sbr_datasourceid_value@OData.Community.Display.V1.FormattedValue'] !== dataSource) // grouped dataSource
                return false; // discard any where the incoming dataSource does not match

            context.scheme = scheme; // match on schemenumber takes precedence
        });
        debug(session.sid, 'scheme:', res.data.value.length, 'item(s)', context.schemeMs + 'ms', JSON.stringify(context.scheme));
        ...
        `.replace(/\r?\n\s*/g, '');
        debug(session.sid, 'calleventsXml:', context.calleventsXml);
        hr = process.hrtime();
        return axios(context.axios = {
            method: 'get',
            baseURL: main.secrets.appello.azureBaseUrl + main.secrets.appello.azureDataUrl,
        ...
        debug(session.sid, 'callevents:', context.calleventsMs + 'ms', JSON.stringify(context.callevents));
        ...
        `.replace(/\r?\n\s*/g, '');
        debug(session.sid, 'callroutingsXml:', context.callroutingsXml);
        hr = process.hrtime();
        return axios(context.axios = {
            method: 'get',
            baseURL: main.secrets.appello.azureBaseUrl + main.secrets.appello.azureDataUrl,
        ...
        `.replace(/\r?\n\s*/g, '');
        debug(session.sid, 'routingendpointsXml:', context.routingendpointsXml);
        hr = process.hrtime();
        return axios(context.axios = {
            method: 'get',
            baseURL: main.secrets.appello.azureBaseUrl + main.secrets.appello.azureDataUrl,
...
function onBsiaSpawnCalls(catalogue, communicator, cb) {
    ...
    acquireToken().then(function (res) { // bsiaSpawnCalls
        ...
        `.replace(/\r?\n\s*/g, '');
        debug(communicator.session.sid, 'deviceXml:', context.deviceXml);
        hr = process.hrtime();
        return axios(context.axios = {
            method: 'get',
            baseURL: main.secrets.appello.azureBaseUrl + main.secrets.appello.azureDataUrl,
        ...
        var dataSource = context.dataSource || payload.dataSource || undefined;
        var cliDevices = (context.devices = res.data.value).filter(function (device, idx, arr) {
            if (device['sbr_equipmentphone'] === cli)
                return true;
        ...
        context.calleventsXml = `
                ...
                `.replace(/\r?\n\s*/g, '');
        debug(communicator.session.sid, 'calleventsXml:', context.calleventsXml);
        hr = process.hrtime();
        return axios(context.axios = {
            method: 'get',
            baseURL: main.secrets.appello.azureBaseUrl + main.secrets.appello.azureDataUrl,
        ...
        debug(communicator.session.sid, 'callevents:', context.calleventsMs + 'ms', JSON.stringify(context.callevents));
        ...
        `.replace(/\r?\n\s*/g, '');
        debug(communicator.session.sid, 'callroutingsXml:', context.callroutingsXml);
        hr = process.hrtime();
        return axios(context.axios = {
            method: 'get',
            baseURL: main.secrets.appello.azureBaseUrl + main.secrets.appello.azureDataUrl,
        ...
        `.replace(/\r?\n\s*/g, '');
        debug(communicator.session.sid, 'routingendpointsXml:', context.routingendpointsXml);
        hr = process.hrtime();
        return axios(context.axios = {
            method: 'get',
            baseURL: main.secrets.appello.azureBaseUrl + main.secrets.appello.azureDataUrl,
...
function onWriteCallData(session, cb) {
    ...
    if (context.cdr) { // CDR already written
        ...
        return cb && cb();
    } else if (payload.protocol && !context.device) {
        debug(session.sid, 'onWriteCallData:', JSON.stringify({ protocol: payload.protocol, device: null }));
        return cb && cb();
    }
    ...
    acquireToken().then(function (res) { // onWriteCallData
        ...
        var cliDevices = (context.devices = res.data.value).filter(function (device, idx, arr) {
            ...
            if (device['_sbr_datasourceid_value@OData.Community.Display.V1.FormattedValue'] !== dataSource)
                return false; // discard any with mis-matched dataSource
        ...
        context.schemeMs = hrMs(process.hrtime(hr));
        (context.schemes = res.data.value).filter(function (scheme, idx, arr) {
            if (device['sbr_groupedequipmentphone'] !== cli)
                return true;

            if (scheme['_sbr_datasourceid_value@OData.Community.Display.V1.FormattedValue'] !== dataSource) // grouped dataSource
                return false; // discard any where the incoming dataSource does not match

            context.scheme = scheme; // match on CLI
        });
        debug(session.sid, 'scheme:', res.data.value.length, 'item(s)', context.schemeMs + 'ms', JSON.stringify(context.scheme));
