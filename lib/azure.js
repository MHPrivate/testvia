...
function onRoutingLookup(session, cb) {
    ...
    var bridgePlatform = context.bridgePlatform || payload.bridgePlatform || undefined;
    var hr = process.hrtime();
        ...
        azure.orClauses = azure.callroutings.map(function (row, idx, arr) {
            return `
                <filter type="and">
                    ...
                    <condition attribute="sbr_bridgeplatformid" ${bridgePlatform ? `operator="eq" value="${exports.platforms[bridgePlatform]}"` : nullClause}/>
                </filter>
            `.replace(/\r?\n\s*/g, '');
        });
