...
process.running.then(function azureStartup() {
    ...
    acquireToken().then(function (res) { // azureStartup - accessToken
                ...
                fetchXml: `
                    <fetch version="1.0" output-format="xml-platform" mapping="logical" distinct="false">
                        <entity name="sbr_device">
                            ...
                            <link-entity name="sbr_devicecharacteristic" from="sbr_deviceid" to="sbr_deviceid" alias="DC" link-type="outer" visible="false">
                                <all-attributes/>
                                <filter type="and">
                                    <condition attribute="sbr_name" operator="eq" value="Data Source"/>
                                </filter>
                                <link-entity name="sbr_devicecharacteristicvalue" from="sbr_devicecharacteristicvalueid" to="sbr_devicecharacteristicvalueid" alias="CV" link-type="outer" visible="false">
                                    <all-attributes/>
                                    <!-- All DataSources -->
                                </link-entity>
                            </link-entity>
        ...
        res.data.value.forEach(function (device, idx, arr) { // sbr_callcode, sbr_name, sbr_eventcodeid, S.sbr_name
            exports.defaultDevices[device['DV.sbr_name'] || null] = device;
        });
...
function onRoutingLookup(session, cb) {
    ...
    acquireToken().then(function (res) { // onRoutingLookup
            ...
            context.deviceXml = `
                <fetch version="1.0" output-format="xml-platform" mapping="logical" distinct="false">
                    <entity name="sbr_device">
                        ...
                        <link-entity name="sbr_devicecharacteristic" from="sbr_deviceid" to="sbr_deviceid" alias="DC" link-type="outer" visible="false">
                            <order attribute="sbr_name" descending="true"/>
                            <filter type="and">
                                <condition attribute="sbr_name" operator="eq" value="Data Source"/>
                            </filter>
                            <link-entity name="sbr_devicecharacteristicvalue" from="sbr_devicecharacteristicvalueid" to="sbr_devicecharacteristicvalueid" alias="CV" link-type="outer" visible="false">
                                <attribute name="sbr_name"/>
                                <filter type="and">
                                    <condition attribute="sbr_name" operator="eq" value="${context.dataSource || payload.dataSource || ''}"/>
                                </filter>
                            </link-entity>
                        </link-entity>
                    </entity>
        ...
        var cliDevices = (context.devices = res.data.value).filter(function (device, idx, arr) {
            if (device['CV.sbr_name'] !== dataSource) // discard any with mis-matched dataSource
                return false;
...
function onWriteCallData(session, cb) {
    ...
    acquireToken().then(function (res) { // onWriteCallData
        ...
        if (payload.protocol || !res) // should already have the relevant device
            return;

        context.deviceMs = hrMs(process.hrtime(hr));
        var dataSource = context.dataSource || payload.dataSource || undefined;
        var cliDevices = (context.devices = res.data.value).filter(function (device, idx, arr) {
            return device['CV.sbr_name'] === dataSource; // discard any with mis-matched dataSource
        });
        if (!context.device)
            context.device = cliDevices.length === 1 ? cliDevices[0] : (exports.defaultDevices[dataSource] || exports.defaultDevices[null]);
