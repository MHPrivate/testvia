...
function onWriteCallData(session, cb) {
    ...
    acquireToken().then(function (res) { // onWriteCallData
        ...
        if (err.response && err.response.data.error)
            debug(session.sid, 'onWriteCallData:', context.cdr.$, JSON.stringify(context.axios), '\n' + err.response.data.error.message);
        else
            debug(session.sid, 'onWriteCallData:', context.cdr.$, JSON.stringify(context.axios), err);
