...
function onWriteCallData(session, cb) {
    ...
    return new Promise(function (resolve, reject) {
        hr = process.hrtime();
        return context.accessToken ? resolve() : resolve(acquireToken());
        ...
        if (payload.protocol || !res) // already have the relevant device
            return;

        context.deviceMs = hrMs(process.hrtime(hr));
        context.device = res.data.value.length === 1 ? res.data.value[0] : exports.defaultDevice;
        debug(session.sid, 'device:', context.deviceMs + 'ms', JSON.stringify(context.device));
