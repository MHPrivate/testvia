...
function onRoutingLookup(session, cb) {
    ...
    acquireToken().then(function (res) { // onRoutingLookup
        ...
        var cliDevices = (context.devices = res.data.value).filter(function (device, idx, arr) {
            if (device['_sbr_datasourceid_value@OData.Community.Display.V1.FormattedValue'] !== dataSource // dispersed device dataSource mismatch
                && device['S._sbr_datasourceid_value@OData.Community.Display.V1.FormattedValue'] !== dataSource) // grouped device dataSource mismatch
                return false; // discard any with fully mis-matched dataSource
