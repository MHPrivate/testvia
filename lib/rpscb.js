...
process.running.wait.push(process.running.then(function () {
    ...
    process.once('terminate', function _rpscb() { // register for shutdown
...
function node() { // prepare mac based options for uuid.v1()
    ...
    Object.keys(ifaces).some(function (iface, idx, arr) {
        return this[iface].filter(function (address, idx, arr) {
            ...
            node = Buffer.from(address.mac.replace(/:/g, ''), 'hex'); // used as uuid.v1(uuid)
...
function ready(resolve, reject) { // Promise executor and resolver callback
    ...
    debug(this + ': ready', ready.pending.size, 'pending', typeof this, this);
    ready.pending.delete(this.toString()); // in this instance, nodejs-v10.19.0 seems to wrap a string as an Object
