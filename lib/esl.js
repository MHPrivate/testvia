...
function atm(evt, o, cb) { // evt, { ?type, ?data, ?blocking, ?wgs }, cb(err, evt);
    ...
    return msg(evt, 'text/plain', o.xml, o.blocking, o.expected_cli, cb);
...
function mrs(evt, o, cb) { // evt, { ?type, ?data, ?blocking, ?wgs }, cb(err, evt);
    ...
    return msg(evt, 'application/scaip+xml', o.xml, o.blocking, o.expected_cli, cb);
...
function msg(evt, type, body, blocking, expected_cli, cb) { // evt: MESSAGE OR CHANNEL_CREATE OR undefined, type: plain/text OR application/scaip+xml OR ..., body: string, blocking: boolean, cb(err)
    ...
    if (evt.type === 'MESSAGE') { // only respond to outside-dialog MESSAGEs
        ...
        expected_cli && event.addHeader('sip_h_X-Expected-CLI', expected_cli);
        event.addBody(body);
