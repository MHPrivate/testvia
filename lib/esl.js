...
module.exports = Object.assign(exports, { // connection is dynamically assigned as export's Prototype
    ...
    sendDtmf: sendDtmf,                     // method to send DTMF while configurably disabling/enablng drop_dtmf
...
Object.assign(dtmf2stmf, {
    '1': [697, 1209],
    '2': [697, 1336],
    '3': [697, 1477],
    'A': [697, 1633], 'a': [697, 1633],
    '4': [770, 1209],
    '5': [770, 1336],
    '6': [770, 1477],
    'B': [770, 1633], 'b': [770, 1633],
    '7': [852, 1209],
    '8': [852, 1336],
    '9': [852, 1477],
    'C': [852, 1633], 'c': [852, 1633],
    '*': [941, 1209],
    '0': [941, 1336],
    '#': [941, 1477],
    'D': [941, 1633], 'd': [941, 1633],
});
function dtmf2stmf(dtmf) {
    return dtmf.split(/\++/g).reduce(function (wksp, part, idx, arr) { // split on '+' giving array of [digits@durationMs, ...]
        ...
        return parts[0].match(/./g).reduce(function (wksp, digit, idx, arr) {
            var f = dtmf2stmf[digit];
            return wksp + `%(${parts[1] - 5},5,${f[0]});%(${parts[1] - 5},5,${f[1]});`
        }, wksp);
    }, '');
}

function dtmfMs(dtmf, pauseMs) { // calculate the transmission time for a f/s digit-string
    ...
    return dtmf.split(/\++/g).reduce(function (wksp, part, idx, arr) { // split on '+' giving array of [digits@durationMs, ...]
        ...
        for (var i in parts[0]) switch (parts[0][i]) { // collect part durations
            ...
            default: wksp += +parts[1] + pauseMs;
        }
        return wksp;
    }, 0);
}

function sendDtmf(dtmf, uuid, lock, cb) {
    //          after
    //      N           Y
    //
    // b  N  neither    lock
    ...
    // e
    //
    var debug = typeof this === 'function' && this.name.endsWith('debug') && this; // debug.bind(null, sid, ...)
    chain(cb, function () {
        if ([undefined, true].includes(lock))
            return this();

        debug && debug('allow_dtmf');
        exports.executeAsyncX('eval', ['${uuid_drop_dtmf ${uuid} off mask_digits -}'], uuid, this);

    }, function () {
        debug && debug('send_dtmf', dtmf, new Date);
        exports.executeAsyncX('send_dtmf', [dtmf], uuid, this);

    }, function () {
        if ([undefined, false].includes(lock))
            return this();

        debug && debug('block_dtmf');
        exports.executeAsyncX('eval', ['${uuid_drop_dtmf ${uuid} on mask_digits -}'], uuid, this);
