...
module.exports = extend(exports, {
    api: null,                              // placeholder for re-packaged modesl method to use callbacks with err first argument
    auth: null,                             // placeholder for re-packaged modesl method to use callbacks with err first argument
    bgapi: null,                            // placeholder for re-packaged modesl method to use callbacks with err first argument
    execute: null,                          // placeholder for re-packaged modesl method to use callbacks with err first argument
    executeAsync: null,                     // placeholder for re-packaged modesl method to use callbacks with err first argument
    history: extend([], { limit: 100 }),    // limited history of recent freeswitch events
    message: null,                          // placeholder for re-packaged modesl method to use callbacks with err first argument
    originate: null,                        // placeholder for re-packaged modesl method to use callbacks with err first argument
    ...
    show: null,                             // placeholder for re-packaged modesl method to use callbacks with err first argument
    subscribe: null,                        // placeholder for re-packaged modesl method to use callbacks with err first argument
});
...
function parseEvt(evt) { // transform evt.headers to dictionary "evt = parseEvt.apply(this, arguments);"
    if (evt && evt.headers && !Array.isArray(evt.headers)) // already parsed
        return evt
    exports.history.push(evt = extend(evt || { headers: [] }, { event: this ? this.event : null }));
...
['auth', 'show'].forEach(function (name, idx, arr) {
    exports[name] = setFunctionName(name, function (/* ..., */ cb) { // already expects callbacks taking err as a first argument
...
['api', 'bgapi', 'execute', 'executeAsync', 'message', 'originate', 'subscribe'].forEach(function (name, idx, arr) {
    exports[name] = setFunctionName(name, function (/* ..., */ cb) { // expects callbacks not taking err as a first argument
        var args = Array.from(arguments);
        cb = typeof args.slice(-1)[0] === 'function' ? args.pop() : function () { };
        args.push(function (evt /* , hdrs, body */) {
            if (!evt) // aborted
                return cb(new Error(name + 'aborted'));
            parseEvt.apply(this, arguments);
