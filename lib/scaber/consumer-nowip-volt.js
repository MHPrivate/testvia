...
var consumerState = {// _this_ of all methods is the StateMachine instance
    ...
    activate: function onConsumerNowipVoltActivate(err, configs, meta) { // multiple activations by session each time round the list of consumers
        ...
        var sm = this, origin = (this.session.payload.ATM || { data: [''] }).data[0].slice(4, 16).replace(/^0+/, '');
        chain(this.signal.bind(this, 'parked'), function () {
            if (!sm.session.payload.ATM)
                return this();
            esl.executeAsyncX('bgsystem', ['$${conf_dir}/bin/add-suffix.sh ${record_file_path} ' + sm.session.payload.ATM.data], sm.session.communicator.uuid, this);
            //esl.executeAsyncX('set', ['record_post_process_exec_app=bgsystem:$${conf_dir}/bin/add-suffix.sh ${record_file_path} ' + sm.session.payload.ATM.data], sm.session.communicator.uuid, this);

        }, function (evt) {
            esl.bgapiX('originate', [esl.nvp({
                    //absolute_codec_string: 'PCMU\\,PCMA\\,H264', // fails to include video within INVITE
                    appello_consumer: !undefined, // true as we is a consumer leg
                    ...
                    origination_caller_id_number: origin || sm.session.origin,
                }, '{}' + sm.uris), '&park'], this);
