...
var consumerState = {// _this_ of all methods is the StateMachine instance
        ...
        if (configs) { // [{id,nameSlashed,valueBoolean,valueNumber,valueString:JSON}] response from mysql - see 12 lines below
            ...
            this.session.payload.ATM = {
                data: [nowip.stringify(Object.assign({
                        controllerunit: (this.session.origin + '000000000000').slice(0, 12),
                        status: 'Non-speech manually operated',
                    }, atm))], // JSON:{event:number,location:number}
            };
            ...
            this.dty = ('0000' + +(mrq.dty || ['0'])[0]).slice(-4);
            this.stc = ('0000' + +(mrq.stc || ['10'])[0]).slice(-4);
            this.lco = ('000' + +(mrq.lco || ['0'])[0]).slice(-3);
            wheres.push(
            ...
            debug.enabled && debug(this.session.sid, 'activate', JSON.stringify({ dty: this.dty, lco: this.lco }));
            var cb = Object.assign(this.signal.bind(this, 'activate'), { index: this.session.sid });
            return mysql('select * from config where schemeId=0 and nameSlashed in (' + mysql.qmks(wheres) + ') order by length(nameSlashed) asc', wheres, cb);
        } else {
    ...
    CHANNEL_: function onConsumerNowipVoltChannel(evt, first) { // miscellaneous CHANNEL_*** events
        ...
        this.sent = true;
        var atm = { type: 1, data: this.session.payload.ATM.data[0], wgs: (this.session.payload.ATM.wgs || [])[0], blocking: true };
        esl.atm(evt, atm);
