...
Object.assign(ConsumerNowipVolt, {// _this_ of all methods is the StateMachine instance
    ...
    activate: function onConsumerNowipVoltActivate() { // multiple activations by session each time round the list of consumers
        ...
        chain(function cleanup(err, evt) {
            ...
            var originate = esl.nvp({
                ...
                drop_dtmf: true,
                fs_send_unsupported_message: ConsumerNowipVolt.inDialog || undefined, // enables uuid_send_message
            ...
            /// unnecessary as CareNet sends in-dialog SIP-MESSAGEs even though it won't accept in-dialog SIP-MESSAGEs
            //sm.session.signal('route', [originUser]); // to receive any SIP-MESSAGEs for this Consumer channel
    ...
    CHANNEL_: function onConsumerNowipVoltChannel(evt, first) { // miscellaneous CHANNEL_*** events
        ...
        if (!ConsumerNowipVolt.inDialog) { // outDialog SIP-MESSAGE
            var atm = {
                type: 1,
            ...
            var xml = js2xml.buildObject({ // uuid_send_message *** REQUIRES *** fs_send_unsupported_message=true on the originate
                ATM: {
                    version: ['1.5'],
                    type: ['1'],
