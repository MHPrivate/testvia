...
Object.assign(ConsumerNowipVolt, {// _this_ of all methods is the StateMachine instance
    ...
    activate: function onConsumerNowipVoltActivate() { // multiple activations by session each time round the list of consumers
        ...
        chain(function cleanup(err, evt) {
            ...
            if (!sm.session.communicator.uuid) { // no Communicator - THEN based on having an atm.event ...
                if ((payload.atm || {}).event) // have a non-zero atm.event
                    return this(); // proceed with originate

                console.log('onConsumerNowipVoltActivate: zero ATM.event - ABORTING Consumer', JSON.stringify(payload));
                return sm.signal('cleanup');
            }
            else if (!sm.session.payload.ATM) // have Communicator but no ATM
                return this(); // proceed with originate
