...
Object.assign(exports, {// _this_ of all methods is the StateMachine instance
	...
	// IPv6, expect evt_headers['login'] to be standard v4 sip URI e.g. sip:user@[xxxx:xxxx:xxxx:xxxx:xxxx:xxxx:xxxx:xxxx]:5060
	// 
	let non_tls = ['int6tcp','int6udp','int4tcp','int4udp','ext4tcp','ext4udp','ext6tcp','ext6udp'];
	if (!crds['sips:'] && non_tls.includes(evt.headers['sip_profile'])) {
		...
		let tnu_method = "sip:";
		let tnu_address = "";
		if (evt.headers['sip_profile'].slice(3,1)=='6') {
			// for IPv6
