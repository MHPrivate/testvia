...
Object.assign(exports, {// _this_ of all methods is the StateMachine instance
    ...
    leave: function onCommunicatorScaipLeave() {
        ...
        debug(this.session.sid, 'leave:', this.uuid === '' ? 'wasted training DDI reservation' : '');
	...
	// received MESSAGE body doesn't contain SCAIP <mrq> so abandon without writing CDR.
	if (!evt.parsed.mrq) {
		this.session.payload.noCdr = true;
    		return debug(this.session.sid, evt.type + ': invalid SCAIP message');
	}
        ...
        switch (mrq.mty[0]) {
            case 'ME':// message - alarm notify
                if (evt.headers['from_user'] !== mrq.cid[0])
                    ...
                    crds = Object.assign(mrs, { snu: 0 }) && {}; // immediate cleanup
                else if (crds['tel:'] || crds['gsm:'] || crds['sip:'] || crds['sips:']) // potential media-leg
        ...
        // Generating the TNU response for a sip/sips requested call.
        //For crd="sips:", need to assign the correct destination port and transport for the mrs message.
        // sips: requested, will get a response URI to an encrypted endpoint, irrespective of how the message was received.
        // Using the old 'login' event header method, an inbound <mrq> received over ports 5061/5071 (tls) will return port 5060, which cannot be used.
        // We do however need to retain that 'login' header mechanism for non-TLS cases as it suits the upcoming AWS implementation.
        //
        // evt.headers[sip_profile] contains the name of the sofia profile where the message was received, from here we can determine if received over TLS.
        // e.g. int4usp, int4tls, ext4udp, ext4tls, etc.
        // int = port 5070/1
        // ext = port 5060/1
        // So, if received over non-tls, and not sips: requested, use the original 'login' header method (good for AWS).
        // If received over tls
        //	if sip requested, assign ports 5060/5070 corresponding to 5061/5071 received
        //	if sips requested, assign ports 5061/5071 as received.
        //
        // When used in AWS, the mrq message will always be received here without TLS.
        //
        // Parsing local address.
        // IPv4, expect evt_headers['login'] to be standard v4 sip URI e.g. sip:user@xx.xx.xx.xx:5060
        // IPv6, expect evt_headers['login'] to be standard v6 sip URI e.g. sip:user@[xxxx:xxxx:xxxx:xxxx:xxxx:xxxx:xxxx:xxxx]:5060
        // 
        // The code expects that Freeswitch profile names will continue to use the current format e.g.
        // int6tcp, int6udp, int4tcp, int4udp, ext4tcp, ext4udp, ext6tcp, ext6udp, int4tls, int6tls, ext4tls, etx6tls
        //
        if (!crds['sips:'] && !evt.headers['sip_profile'].includes('tls')) {
            // no sips and request received over non-tls - use original method
            this.tnu = [evt.headers['login'].replace('mod_sofia', evt.headers['to_user'])];
        } else {
            let tnu_method = "sip:";
            let tnu_address = "";
            if (evt.headers['sip_profile'].includes('6')) {
                // for IPv6
                let matches = evt.headers['login'].match(/\[(.*?)\]/);
                tnu_address = (matches) ? matches[0] : "badIPv6";
            } else {
                // for IPv4
                if (evt.headers['login'].includes(':') && evt.headers['login'].includes('@'))
                    tnu_address = evt.headers['login'].split(":")[1].split("@")[1];
                else
                    tnu_address = "badIPv4";
            }

            let tnu_transport = (crds['sips:']) ? ";transport=tls" : "";
            let tnu_port;
            // if sips requested, will need 5061/5071, depending on received sip_profile
            if (crds['sips:'])
                tnu_port = (evt.headers['sip_profile'].slice(0,3)=='ext') ? "5061" : "5071";
            else
                tnu_port = (evt.headers['sip_profile'].slice(0,3)=='ext') ? "5060" : "5070";
            
            this.tnu = tnu_method + evt.headers['to_user'] + "@" + tnu_address + ":" + tnu_port + tnu_transport;
        }

        if (+mrq.mreq[0] !== 4 && (this.callerId = (crds['sip:'] || crds['sips:']))) {
            ...
            chain(function cleanup(err) {
            ...
            }, function (tnu) { // 'gsm:+e164' //Object tnu.tnu 'gsm:+e164' tnu.cli is expected inbound cli or doesn't exist (training scenario)
                ...
                sm.tnu.splice(0, main.hack.multiTnu ? 0 : sm.tnu.length, tnu.tnu); // multiple OR single <tnu> nodes (multiple not currently supported by Essence & Neat)
                Object.assign(mrs, { tnu: sm.tnu });
                (tnu.cli) && Object.assign(mrs, { expected_cli: tnu.cli });
                sm.uuid = ''; // expect a training call
