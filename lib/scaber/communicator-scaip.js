    ...
    MESSAGE: function onCommunicatorScaipMessage(evt, first) {
        ...
        switch (mrq.mty[0]) {
            ...
            case 'RE': // reset - alarm withdrawn
                this.session.payload.noCdr = true; // prevent CDRs for alarm-resets
                if (evt.headers['from_user'] !== mrq.cid[0])
                    crds = Object.assign(mrs, { ste: 'from_user & cid must be identical' }) && undefined; // immediate cleanup
                else
            ...
            case 'PI': // hearbeat
                this.session.payload.noCdr = exports.noCdrPI; // prevent CDRs for keepalive-pings
                if (evt.headers['from_user'] !== mrq.cid[0])
                    crds = Object.assign(mrs, { ste: 'from_user & cid must be identical' }) && undefined; // immediate cleanup
                else
            ...
            default:
                ...
                crds = undefined;
                this.session.payload.noCdr = true; // prevent CDRs for unknown message-types
                break;
        }

        //debug.enabled && debug(this.session.sid, evt.type + ':', UTIL.stringify({ mrs: mrs, crds: crds, callerId: this.callerId }));
        if (this.session.payload.noCdr) { // non-call SCAIP message
            esl.mrs(evt, mrs);
        ...
        switch (type) {
            ...
            case 'control': // ('control', match, control) - control is a string
                switch (arguments[2]) {
                    ...
                    case 'releaseKeysafe':
                    case 'releaseAll':
                        return worker.legDtmf(this, exports.releaseDtmf, null, 'ctl', () => ok()); // allow+send+block
            ...
            case 'quick': // ('quick', match, quick) - quick is a string
                switch (arguments[2]) {
                    case 'speak': // ack: B
                        ...
                        return worker.legDtmf(this, exports.speakDtmf, null, 'spk', () => ok()); // allow+send+block
                    ...
                    case 'listen': // ack: B
                        ...
                        return worker.legDtmf(this, exports.listenDtmf, null, 'lsn', () => ok()); // allow+send+block
                    ...
                    case 'clear': // status: A000000#
                        break;
                    case 'close': // ack: B
                        return worker.legDtmf(this, exports.closeDtmf, false, 'cls', ()=>ok()); // allow+send
                    ...
                    case 'null': // ack: B ????
                        ...
                        return worker.legDtmf(this, dtmf || exports.duplexDtmf, null, 'nul', () => ok()); // allow+send+block
            ...
            case 'speech': // ('speech', match, speech) - speech is a string
                switch (arguments[2]) {
                    ...
                    case 'volume3': // ack: B
                        break;
                    case 'volumeUp': // ack: B
                        return worker.legDtmf(this, exports.volUpDtmf, null, 'vUp', () => ok()); // allow+send+block
                        //return esl.sendDtmf.call(debug.bind(null, this.session.sid, arguments[2]), exports.volUpDtmf, this.uuid, null, ()=>ok());
                    case 'volumeDn': // ack: B
                        return worker.legDtmf(this, exports.volDnDtmf, null, 'vDn', () => ok()); // allow+send+block
                    ...
                    case 'duplex': // ack: B
                        ...
                        return worker.legDtmf(this, exports.duplexDtmf, null, 'dpx', () => ok()); // allow+send+block
                        //return esl.sendDtmf.call(debug.bind(null, this.session.sid, arguments[2]), exports.duplexDtmf, this.uuid, null, ()=>ok());
                    case 'simplex': // ack: B
                        this.speech = undefined;
                        this.direction = 'listen';
                        return worker.legDtmf(this, exports.listenDtmf, null, 'spx', () => ok()); // allow+send+block
                        //return esl.sendDtmf.call(debug.bind(null, this.session.sid, arguments[2]), exports.listenDtmf, this.uuid, null, ()=>ok());
                }
                break;
            ...
            case 'dtmf':
                if (!this.session.context.passDtmf && exports.arcDtmf[arguments[2]])
                    return worker.legDtmf(this, exports.arcDtmf[arguments[2]], null, 'dtm', () => ok()); // allow+send+block
                break;
        }
