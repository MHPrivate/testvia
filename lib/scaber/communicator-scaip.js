...
process.on('sipMessagePreProcess', function onSipMessageProProcessScaip(evt) {
    ...
    evt.scaber = {
        origin: evt.headers['from_user'] + '9999',
...
var communicatorState = {// _this_ of all methods is the StateMachine instance
    ...
    answer: function onCommunicatorScaipAnswer() { // request to answer a-leg if not already answered
        ...
        this.interval = clearInterval(this.interval) || setInterval(this.signal.bind(this, 'interval'), 60000);
        return this;
    },
    clear: function onCommunicatorScaipClear() {
        debug(this.session.sid, 'clear:', callsites()[2].toString());
        if (!this.uuid)
            return this.enter(null) || this;
        this.interval = clearInterval(this.interval);
        this.timeout = clearTimeout(this.timeout) || setTimeout(this.signal.bind(this, 'timeout'), 5000); // allow the scaip device upto 5s to disconnect
        var sm = this;
        chain(function cleanup(err) {
            err && console.log(sm.session.sid, 'onCommunicatorScaipClear:', err);
    ...
    interval: function onCommunicatorScaipInterval() { // send SCAIP keepalive DTMF
        debug(this.session.sid, 'interval:');
        var sm = this;
        chain(function cleanup(err) {
            err && console.log(sm.session.sid, 'onCommunicatorScaipInterval:', err);

        }, function () {
            esl.bgapiX('uuid_drop_dtmf', [sm.uuid, 'off mask_digits ""'], this);

        }, function (evt) {
            esl.bgapiX('uuid_send_dtmf', [sm.uuid, '4@150'], this);
    ...
    timeout: function onCommunicatorScaipTimeout() { // forced release timeout
    ...
    CHANNEL_CREATE: function onCommunicatorScaipChannelCreate(evt, first) {
        ...
        this.timeout = clearTimeout(this.timeout);
        if (reReq.test(evt.headers['variable_sip_req_params'])) // appello=scaber indicates PSTN call
        ...
        chain(function cleanup(err) {
            ...
            debug(sm.session.sid, 'CHANNEL_CREATE:', evt.headers['Unique-ID'], 'ringing');
        ...
        switch (mrq.mty[0]) {
            case 'ME':// message - alarm notify
                if (stc === 19) // 19='Cancel'
                    ...
                    crds = Object.assign(mrs, { snu: 0 }) && undefined;
                else if (Object.assign(mrs, { snu: 0, mre: 1, cre: 62 })) // always true
