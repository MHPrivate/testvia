...
Object.assign(exports, {// _this_ of all methods is the StateMachine instance
    ...
    clear: function onCommunicatorScaipClear(err, evt) {
        ...
        this.keepalive = worker.resetInterval.call(this.session.sid, this.keepalive);
        esl.sendDtmf.call(debug.bind(null, this.session.sid, 'close'), '0@200', this.uuid, false);
        this.timeout = worker.resetTimeout.call(this.session.sid, this.timeout, this.signal.bind(this, 'release'), 10000); // allow the scaip device upto 5s to disconnect
        return this;
    },
    ...
    CHANNEL_CREATE: function onCommunicatorScaipChannelCreate(evt, first) {
        ...
        chain(function cleanup(err) {
            ...
            debug(sm.session.sid, 'CHANNEL_CREATE.1:', 'record_session', main.hack.record);
            if (!main.hack.record)
                return this();
    ...
    MESSAGE: function onCommunicatorScaipMessage(evt, first) {
        ...
        if (this.callerId = crds['sip:']) {
            ...
            this.session.signal('route', [evt.headers['from_user']]); // expect SIP-INVITE from same origin as SIP-MESSAGE (which is also the SCAIP.cid)
            ...
            debug(this.session.sid, evt.type + ':', evt.headers['Event-Sequence'], '???:', mrs.xml);
            return this.session.signal('consume', undefined) || this;
        ...
        switch (type) {
            ...
            case 'control': // ('control', match, control) - control is a string
                switch (arguments[2]) {
                    ...
                    case 'releaseKeysafe':
                    case 'releaseAll':
                        return esl.sendDtmf.call(debug.bind(null, this.session.sid, arguments[2]), '9@200', this.uuid, null, ()=>cb());
                    case 'undefined':
                    case 'relay1On':
                    case 'relay1Off':
                    case 'relay2On':
            ...
            case 'quick': // ('quick', match, quick) - quick is a string
                switch (arguments[2]) {
                    case 'speak': // ack: B
                        ...
                        return esl.sendDtmf.call(debug.bind(null, this.session.sid, arguments[2]), '8@200', this.uuid, null, ()=>cb());
                        break;
                    case 'listen': // ack: B
                        this.speech = undefined;
                        this.direction = 'listen';
                        return esl.sendDtmf.call(debug.bind(null, this.session.sid, arguments[2]), '7@200', this.uuid, null, ()=>cb());
                        break;
                    case 'clear': // status: A000000#
                        break;
                    case 'close': // ack: B
                        return esl.sendDtmf.call(debug.bind(null, this.session.sid, arguments[2]), '0@200', this.uuid, false, ()=>cb());
                        break;
                    case 'null': // ack: B ????
                        var dtmf = { duplex: '4@200', listen: '7@200', speak: '8@200' }[this.speech || this.direction];
                        return esl.sendDtmf.call(debug.bind(null, this.session.sid, arguments[2]), dtmf || '4@200', this.uuid, null, ()=>cb());
                        break;
                }
                break;
            ...
            case 'speech': // ('speech', match, speech) - speech is a string
                switch (arguments[2]) {
                    ...
                    case 'volume3': // ack: B
                        break;
                    case 'volumeUp': // ack: B
                        return esl.sendDtmf.call(debug.bind(null, this.session.sid, arguments[2]), '3@200', this.uuid, null, ()=>cb());
                        break;
                    case 'volumeDn': // ack: B
                        return esl.sendDtmf.call(debug.bind(null, this.session.sid, arguments[2]), '1@200', this.uuid, null, ()=>cb());
                        break;
                    case 'speaker1': // ack: B
                        break;
                    case 'speaker2': // ack: B
                    ...
                    case 'duplex': // ack: B
                        ...
                        return esl.sendDtmf.call(debug.bind(null, this.session.sid, arguments[2]), '4@200', this.uuid, null, ()=>cb());
                        break;
                    case 'simplex': // ack: B
                        this.speech = undefined;
                        this.direction = 'listen';
