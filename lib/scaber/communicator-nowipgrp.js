...
require('util').inherits(module.exports = exports = CommunicatorNowip2, require('../state-machine'));
function CommunicatorNowip2(session, evt) {
    if (this instanceof CommunicatorNowip2 === false)
        throw new Error('Constructor CommunicatorNowip2 requires \'new\'');

    CommunicatorNowip2.super_.call(this, CommunicatorNowip2, {
...
Object.assign(CommunicatorNowip2, {// _this_ of all methods is the StateMachine instance
    advisee: 1,         // luc
    ...
    enter: function onCommunicatorNowip2Enter(evt) { // websvc accepted our offer to handle
        debug(this.session.sid, 'enter:', evt.type);
        return this;
    },
    leave: function onCommunicatorNowip2Leave() {
    ...
    release: function onCommunicatorNowip2Release() { // forced release timeout - from close/closed/establish
    ...
    clear: function onCommunicatorNowip2Clear() {
    ...
    answer: function onCommunicatorNowip2Answer() { // request to answer a-leg if not already answered
    ...
    CHANNEL_CREATE: function onCommunicatorNowip2ChannelCreate(evt, first) {
        ...
        this.timeout = worker.resetTimeout.call(this.session.sid, this.timeout, this.signal.bind(this, 'clear'), CommunicatorNowip2.messageMs);
    ...
    CHANNEL_: function onCommunicatorNowip2Channel(evt, first) { // miscellaneous CHANNEL_*** events
    ...
    CHANNEL_DESTROY: function onCommunicatorNowip2ChannelDestroy(evt, first) { // a-leg has ended
    ...
    CUSTOM: function onCommunicatorNowip2Custom(evt, first) {
        debug(this.session.sid, 'CUSTOM:', evt.subclass);
        return this;
    },
    DETECTED_TONE: function onCommunicatorNowip2Tone(evt, first) { // received tone
        debug(this.session.sid, 'TONE:', evt.headers['Detected-Tone']);
        return this;
    },
    DTMF: function onCommunicatorNowip2Dtmf(evt, first) {
        debug(this.session.sid, 'DTMF:', evt.headers['DTMF-Digit'] + '@' + (evt.headers['DTMF-Duration'] / 8));
        return this;
    },
    MESSAGE: function onCommunicatorNowip2Message(evt, first) { // received NOWIP message
    ...
    atmResponder: function onCommunicatorNowip2AtmResponder(timeoutMs, responseCb) {
    ...
    transaction: function onCommunicatorNowip2Transaction(type, match, arg2, arg3, arg4/* ..., cb */) { // consumer-to-communicator transaction
        ...
        switch (type) {
            ...
            case 'catalogue': // ('catalogue', match, unit) - A000000000000#
                ...
                return this.signal('atmResponder', exports.catalogueMs, function catalogueCb(err, type, data) { // _this_ is this communicator
                    ...
                    debug(this.session.sid, 'onCommunicatorNowip2TransactionCatalogueCb:', data);
                    bs8521 = nowip.parse(data, nowip.parse.catsel);
                    esl.atm(this.session.firstEvt, atm = { type: 'A', data: '.' }); // acknowledge
                    debug(this.session.sid, 'onCommunicatorNowip2TransactionCatalogueCb:', atm.xml);
            ...
            case 'control': // ('control', match, control) - control is a string
                ...
                return this.signal('atmResponder', exports.controlMs, function controlCb(err, type, data) { // _this_ is this communicator
                    if (err || !['5', '6', '7', 'A'].includes(type)) // rejected/busy/success/acknowledgement
                        return cb ? ko() : console.log(this.session.sid, 'onCommunicatorNowip2TransactionControlCb:', err || type);

                    debug(this.session.sid, 'onCommunicatorNowip2TransactionControlCb:', data);
            ...
            case 'paramGet': // ('paramGet', match, param) - param is a string
                ...
                return this.signal('atmResponder', exports.paramGetMs, function paramGetCb(err, type, data) { // _this_ is this communicator
                    if (err || !['9'].includes(type)) // reponse
                        return cb ? ko() : console.log(this.session.sid, 'onCommunicatorNowip2TransactionParamGetCb:', err || type);

                    debug(this.session.sid, 'onCommunicatorNowip2TransactionParamGetCb:', data);
            ...
            case 'paramSet': // ('paramSet', match, param, value) - param is a string, value is a digit-string
                ...
                return this.signal('atmResponder', exports.paramSetMs, function paramSetCb(err, type, data) { // _this_ is this communicator
                    if (err || !['9'].includes(type)) // reponse
                        return cb ? ko() : console.log(this.session.sid, 'onCommunicatorNowip2TransactionParamSetCb:', err || type);

                    debug(this.session.sid, 'onCommunicatorNowip2TransactionParamSetCb:', data);
                    bs8521 = nowip.parse(data, nowip.parse.paramget);
                    if (cb) // consumer-transaction
            ...
            case 'program': // ('program', match, pin) - ignores ACK - pin is numeric
                ...
                return this.signal('atmResponder', exports.programMs, function programCb(err, type, data) { // _this_ is this communicator
                    if (err || !['5', '6', '7', 'A'].includes(type)) // rejected/busy/success/acknowledgement
                        return cb ? ko() : console.log(this.session.sid, 'onCommunicatorNowip2TransactionProgramCb:', err || type);

                    debug(this.session.sid, 'onCommunicatorNowip2TransactionProgramCb:', data);
                    bs8521 = nowip.parse(data, nowip.parse.control);
                    if (cb) // consumer-transaction
                        return bs8521 ? ok(data) : ko(data);
                }.bind(this));
            ...
            case 'quick': // ('quick', match, quick) - quick is a string
                ...
                return this.signal('atmResponder', exports.quickMs, function quickCb(err, type, data) { // _this_ is this communicator
                    if (err || !['5', '6', '7', 'A'].includes(type)) // rejected/busy/success/acknowledgement
                        return cb ? ko() : console.log(this.session.sid, 'onCommunicatorNowip2TransactionQuickCb:', err || type);

                    if (arg2 === 'clear' && type === '7') // quick.clear was succesful
                        this.selected = null;

                    debug(this.session.sid, 'onCommunicatorNowip2TransactionQuickCb:', data);
            ...
            case 'select': // ('select', match, unit) - unit is numeric - A000000000000#
                ...
                return this.signal('atmResponder', exports.selectMs, function selectCb(err, type, data) { // _this_ is this communicator
                    if (err || !['5', '6', '7', 'A'].includes(type)) // rejected/busy/success/acknowledgement
                        return cb ? ko() : console.log(this.session.sid, 'onCommunicatorNowip2TransactionSelectCb:', err || type);

                    debug(this.session.sid, 'onCommunicatorNowip2TransactionSelectCb:', data);
                    bs8521 = nowip.parse(data, nowip.parse.catsel);
            ...
            case 'speech': // ('speech', match, speech) - speech is a string
                ...
                return this.signal('atmResponder', exports.speechMs, function speechCb(err, type, data) { // _this_ is this communicator
                    if (err || !['5', '6', '7', 'A'].includes(type)) // rejected/busy/success/acknowledgement
                        return cb ? ko() : console.log(this.session.sid, 'onCommunicatorNowip2TransactionSpeechCb:', err || type);

                    debug(this.session.sid, 'onCommunicatorNowip2TransactionSpeechCb:', data);
                    if (cb) // consumer-transaction
                        return ok(data);
                }.bind(this));
    ...
    atmCommandControlRelease1: function onCommunicatorNowip2ControlRelease1() {
        return this.signal('transaction', 'control', {}, 'release1') || this;
    },
    atmCommandControlRelease2: function onCommunicatorNowip2ControlRelease2() {
        return this.signal('transaction', 'control', {}, 'release2') || this;
    },
    atmCommandControlReleaseKeysafe: function onCommunicatorNowip2ControlReleaseKeysafe() {
        return this.signal('transaction', 'control', {}, 'releaseKeysafe') || this;
    },
    atmCommandControlReleaseAll: function onCommunicatorNowip2ControlReleaseAll() {
