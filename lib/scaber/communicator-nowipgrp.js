...
var argsMap = require('../args-map'),
    ...
    debug = require('debug')('communicator:nowipgrp'),
    esl = require('../esl'),
...
require('util').inherits(module.exports = exports = CommunicatorNowipgrp, require('../state-machine'));
function CommunicatorNowipgrp(session, evt) {
    if (this instanceof CommunicatorNowipgrp === false)
        throw new Error('Constructor CommunicatorNowipgrp requires \'new\'');

    CommunicatorNowipgrp.super_.call(this, CommunicatorNowipgrp, {
...
Object.assign(CommunicatorNowipgrp, {// _this_ of all methods is the StateMachine instance
    advisee: 1,         // luc
    ...
    enter: function onCommunicatorNowipgrpEnter(evt) { // websvc accepted our offer to handle
        debug(this.session.sid, 'enter:', evt.type);
        return this;
    },
    leave: function onCommunicatorNowipgrpLeave() {
    ...
    release: function onCommunicatorNowipgrpRelease() { // forced release timeout - from close/closed/establish
    ...
    clear: function onCommunicatorNowipgrpClear() {
    ...
    answer: function onCommunicatorNowipgrpAnswer() { // request to answer a-leg if not already answered
        debug(this.session.sid, 'answer:', (this.session.firstEvt || {}).type, callsites()[2].toString());
        if (!this.uuid)
            return this.enter(null) || this;

        debug(this.session.sid, 'answer:', 'answer');
        esl.executeAsyncX('answer', [], this.uuid); // XT2 needs answer before select
        return this;
    },
    CHANNEL_CREATE: function onCommunicatorNowipgrpChannelCreate(evt, first) {
        ...
        this.timeout = worker.resetTimeout.call(this.session.sid, this.timeout, this.signal.bind(this, 'clear'), CommunicatorNowipgrp.messageMs);
    ...
    CHANNEL_: function onCommunicatorNowipgrpChannel(evt, first) { // miscellaneous CHANNEL_*** events
        ...
        if (evt.type === 'CHANNEL_ANSWER') {
            ...
            this.signal('transaction', 'select', '{}', +this.grouped.unit );
        } 
        return this;
    },
    CHANNEL_DESTROY: function onCommunicatorNowipgrpChannelDestroy(evt, first) { // a-leg has ended
    ...
    CUSTOM: function onCommunicatorNowipgrpCustom(evt, first) {
        debug(this.session.sid, 'CUSTOM:', evt.subclass);
        return this;
    },
    DETECTED_TONE: function onCommunicatorNowipgrpTone(evt, first) { // received tone
        debug(this.session.sid, 'TONE:', evt.headers['Detected-Tone']);
        return this;
    },
    DTMF: function onCommunicatorNowipgrpDtmf(evt, first) {
        debug(this.session.sid, 'DTMF:', evt.headers['DTMF-Digit'] + '@' + (evt.headers['DTMF-Duration'] / 8));
        return this;
    },
    MESSAGE: function onCommunicatorNowipgrpMessage(evt, first) { // received NOWIP message
    ...
    atmResponder: function onCommunicatorNowipgrpAtmResponder(timeoutMs, responseCb) {
    ...
    transaction: function onCommunicatorNowipgrpTransaction(type, match, arg2, arg3, arg4/* ..., cb */) { // consumer-to-communicator transaction
        ...
        switch (type) {
            ...
            case 'catalogue': // ('catalogue', match, unit) - A000000000000#
                ...
                return this.signal('atmResponder', exports.catalogueMs, function catalogueCb(err, type, data) { // _this_ is this communicator
                    if (err || !['5', '6', '7'].includes(type))  // rejected/busy/success
                        return cb ? ko() : this.signal('close');
                    
                    debug(this.session.sid, 'onCommunicatorNowipgrpTransactionCatalogueCb:', data);
                    bs8521 = nowip.parse(data, nowip.parse.catsel);
                    esl.atm(this.session.firstEvt, atm = { type: 'A', data: '.' }); // acknowledge
                    debug(this.session.sid, 'onCommunicatorNowipgrpTransactionCatalogueCb:', atm.xml);
            ...
            case 'control': // ('control', match, control) - control is a string
                ...
                return this.signal('atmResponder', exports.controlMs, function controlCb(err, type, data) { // _this_ is this communicator
                    if (err || !['5', '6', '7', 'A'].includes(type)) // rejected/busy/success/acknowledgement
                        return cb ? ko() : console.log(this.session.sid, 'onCommunicatorNowipgrpTransactionControlCb:', err || type);

                    debug(this.session.sid, 'onCommunicatorNowipgrpTransactionControlCb:', data);
            ...
            case 'paramGet': // ('paramGet', match, param) - param is a string
                ...
                return this.signal('atmResponder', exports.paramGetMs, function paramGetCb(err, type, data) { // _this_ is this communicator
                    if (err || !['9'].includes(type)) // reponse
                        return cb ? ko() : console.log(this.session.sid, 'onCommunicatorNowipgrpTransactionParamGetCb:', err || type);

                    debug(this.session.sid, 'onCommunicatorNowipgrpTransactionParamGetCb:', data);
            ...
            case 'paramSet': // ('paramSet', match, param, value) - param is a string, value is a digit-string
                ...
                return this.signal('atmResponder', exports.paramSetMs, function paramSetCb(err, type, data) { // _this_ is this communicator
                    if (err || !['9'].includes(type)) // reponse
                        return cb ? ko() : console.log(this.session.sid, 'onCommunicatorNowipgrpTransactionParamSetCb:', err || type);

                    debug(this.session.sid, 'onCommunicatorNowipgrpTransactionParamSetCb:', data);
                    bs8521 = nowip.parse(data, nowip.parse.paramget);
                    if (cb) // consumer-transaction
            ...
            case 'program': // ('program', match, pin) - ignores ACK - pin is numeric
                ...
                return this.signal('atmResponder', exports.programMs, function programCb(err, type, data) { // _this_ is this communicator
                    if (err || !['5', '6', '7', 'A'].includes(type)) // rejected/busy/success/acknowledgement
                        return cb ? ko() : console.log(this.session.sid, 'onCommunicatorNowipgrpTransactionProgramCb:', err || type);

                    debug(this.session.sid, 'onCommunicatorNowipgrpTransactionProgramCb:', data);
                    bs8521 = nowip.parse(data, nowip.parse.control);
                    if (cb) // consumer-transaction
                        return bs8521 ? ok(data) : ko(data);
                }.bind(this));
            ...
            case 'quick': // ('quick', match, quick) - quick is a string
                ...
                return this.signal('atmResponder', exports.quickMs, function quickCb(err, type, data) { // _this_ is this communicator
                    if (err || !['5', '6', '7', 'A'].includes(type)) // rejected/busy/success/acknowledgement
                        return cb ? ko() : console.log(this.session.sid, 'onCommunicatorNowipgrpTransactionQuickCb:', err || type);

                    if (arg2 === 'clear' && type === '7') // quick.clear was succesful
                        this.selected = null;

                    debug(this.session.sid, 'onCommunicatorNowipgrpTransactionQuickCb:', data);
            ...
            case 'select': // ('select', match, unit) - unit is numeric - A000000000000#
                ...
                return this.signal('atmResponder', exports.selectMs, function selectCb(err, type, data) { // _this_ is this communicator
                    if (err || !['5', '6', '7', 'A'].includes(type)) // rejected/busy/success/acknowledgement
                        return cb ? ko() : console.log(this.session.sid, 'onCommunicatorNowipgrpTransactionSelectCb:', err || type);

                    debug(this.session.sid, 'onCommunicatorNowipgrpTransactionSelectCb:', data);
                    bs8521 = nowip.parse(data, nowip.parse.catsel);
            ...
            case 'speech': // ('speech', match, speech) - speech is a string
                ...
                return this.signal('atmResponder', exports.speechMs, function speechCb(err, type, data) { // _this_ is this communicator
                    if (err || !['5', '6', '7', 'A'].includes(type)) // rejected/busy/success/acknowledgement
                        return cb ? ko() : console.log(this.session.sid, 'onCommunicatorNowipgrpTransactionSpeechCb:', err || type);

                    debug(this.session.sid, 'onCommunicatorNowipgrpTransactionSpeechCb:', data);
                    if (cb) // consumer-transaction
                        return ok(data);
                }.bind(this));
    ...
    atmCommandControlRelease1: function onCommunicatorNowipgrpControlRelease1() {
        return this.signal('transaction', 'control', {}, 'release1') || this;
    },
    atmCommandControlRelease2: function onCommunicatorNowipgrpControlRelease2() {
        return this.signal('transaction', 'control', {}, 'release2') || this;
    },
    atmCommandControlReleaseKeysafe: function onCommunicatorNowipgrpControlReleaseKeysafe() {
        return this.signal('transaction', 'control', {}, 'releaseKeysafe') || this;
    },
    atmCommandControlReleaseAll: function onCommunicatorNowipgrpControlReleaseAll() {
