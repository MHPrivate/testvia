...
function Session(evt, communicatorName, arg) { // minimal evt is { headers: { 'Caller-Caller-ID-Number', variable_sip_call_id } }
    ...
    Object.assign(this, {
        ...
        payload: { // { service, ?ATM:{version,type,data,time,mac} }, ?mrq:{ref,cid,dty,...} } - NOWIP/SCAIP payload
            protocol: 'Unknown', // overwritten once known
            // service
    ...
    Session.super_.call(this, sessionState, undefined, evt); // attach enter+signal methods and enter initial state
...
Session.adviseMap = {
    ...
    'quick.clear': '', // prevent this command
    'quick.close': '', // prevent this command
    'select': '', // prevent this command
};
...
var sessionState = {// _this_ of all methods is the StateMachine instance
    ...
    advise: function onSessionAdvise(channel, command /*, ..., cb */) { // rpscb-to-communicator transaction
        ...
        var args = Array.from(arguments); // rpscb('bridge:evoid:<evoid>', 'quick.speak', cb)
        ...
        if (command === 'command.close' && consumer && consumer.signal('cleanup'))
            return cb && cb();

        command = command.split('.'); // e.g. quick.speak => ['quick', 'speak']
        if (command[0] in exports.adviseMap)
            command[0] = exports.adviseMap[command[0]];
        args.push(function onSessionAdviseCb(err) {
            cb ? cb.apply(this, arguments) : err && console.log(this.sid, 'onSessionAdviseCb:', err);
        }.bind(this));
        var communicator = this.communicator;
        (!communicator || !communicator.signal.apply(communicator, ['transaction', command[0], [], command[1]].concat(args))) && cb && cb(null, '');
    },
    ...
    communicator: function onSessionCommunicator(signal, type, match /* , ..., ?cb */) { // consumer-to-communicator transaction
        var args = Array.from(arguments),
            cb = (typeof args[args.length - 1] === 'function') && args.pop();
        args.push(function onSessionCommunicatorCb(err) {
            cb ? cb.apply(this, arguments) : err && console.log(this.sid, 'onSessionCommunicatorCb:', err);
        }.bind(this));
        var communicator = this.communicator;
        (!communicator || !communicator.signal.apply(this.communicator, ['transaction'].concat(args))) && cb && cb(null, '');
    },
