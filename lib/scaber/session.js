...
var sessionState = {// _this_ of all methods is the StateMachine instance
    ...
    advise: function onSessionAdvise(channel, command /*, ..., cb */) { // rpscb-to-communicator transaction
        ...
        var adviseCb = function onSessionAdviseCb(err) {
            ...
            if (command === 'command.close' && this.consumers[this.consumers.index])
                this.consumers[this.consumers.index].signal('cleanup');
        }.bind(this);
        args.push(adviseCb);
        var communicator = this.communicator;
        if (!communicator || !communicator.signal.apply(communicator, ['transaction',  cmds[0], [], cmds[1]].concat(args)))
            adviseCb(null, '');
    },
