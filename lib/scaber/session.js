...
Object.assign(Session, { // _this_ of all methods is the StateMachine instance
    ...
    reaperMs: 300000, // a session that has not bean consumed within 5 minutes will be killed
    ...
    reaper: function onSessionReaper() {
        debug(this.sid, 'reaper: session not consumed - reaping...');
        if (this.communicator) // For phase 2 remove the following debugs and uncomment the returns
            debug(this.sid, 'reaper: in phase 2 will ask the communicator to cleanup the session');
            //return this.communicator.enter(null);
        else
            debug(this.sid, 'reaper: in phase 2 will directly cleanup the session');
