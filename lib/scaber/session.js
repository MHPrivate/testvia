...
var sessionState = {// _this_ of all methods is the StateMachine instance
    ...
    communicator: function onSessionCommunicator(signal/* , ..., ?cb */) {
        ...
        (!communicator || !communicator.signal.apply(this.communicator, args)) && cb && cb();
    },
    consumer: function onSessionConsumer(signal/* , ..., ?cb */) {
        var args = Array.from(arguments);
        var cb = (typeof args[args.length - 1] === 'function') && args.pop();
        ...
        (!consumer || !consumer.signal.apply(consumer, args)) && cb && cb();
    },
