...
var sessionState = {// _this_ of all methods is the StateMachine instance
    ...
    advise: function onSessionAdvise(channel /*, ..., cb */) {
        debug.enabled && debug(this.sid, 'advise:', channel, argsMap(arguments).slice(1));
        // need similar implementation to onSessionCommunicator method
    ...
    communicator: function onSessionCommunicator(signal, type, match /* , ..., ?cb */) { // consumer-to-communicator transaction
        ...
        (!communicator || !communicator.signal.apply(this.communicator, ['transaction'].concat(args))) && cb && cb();
    },
    ...
    //    (!consumer || !consumer.signal.apply(consumer, ['transaction'].concat(args))) && cb && cb();
    //},
