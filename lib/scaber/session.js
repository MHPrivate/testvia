...
var sessionState = {// _this_ of all methods is the StateMachine instance
    ...
    advise: function onSessionAdvise(channel, command /*, ..., cb */) { // rpscb-to-communicator transaction
        ...
        var adviseCb = function onSessionAdviseCb(/* err, outcome, ... */) {
            ...
            if ((args[1] || '').startsWith('+')) // success indication
                this.keepalived = new Date(); // note latest timestamp for keepalive timing
            cb && cb.apply(this, args);
        }.bind(this);
        args.push(adviseCb);
        var communicator = this.communicator, consumer = this.consumers[this.consumers.index];
        if (cmds[0] === 'consumer') {
            var keepalived = this.keepalived;
    ...
    communicator: function onSessionCommunicator(type, match, subtype /* , ..., ?cb */) { // consumer-to-communicator transaction
        ...
        var communicatorCb = function (err) {
            debug.enabled && debug(this.sid, 'communicatorCb:', argsMap(arguments));
            if (Array.isArray(match) ? arguments[1] !== '' : (arguments[1] || '').startsWith('+'))
                this.keepalived = new Date; // note latest timestamp for keepalive timing
            cb ? cb.apply(this, arguments) : err && console.log(this.sid, 'onSessionCommunicatorCb:', err);
        }.bind(this);
        args.push(communicatorCb);
        var communicator = this.communicator;
        if (!communicator || !communicator.signal.apply(this.communicator, ['transaction'].concat(args)))
