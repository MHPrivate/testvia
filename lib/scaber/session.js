...
function Session(evt, communicatorName, arg) { // minimal evt is { headers: { 'Caller-Caller-ID-Number', variable_sip_call_id } }
    ...
    var origin = evt.headers['_e164'] || evt.headers['Caller-Caller-ID-Number'] || evt.headers['from_user'],
        ...
        unique = main.uuidv1();
    Object.assign(this, {
        advisees: [], // [arc(master),luc(protocol)] legs for advise-transactions
        ...
        detached: false,
        diagnostics: Object.assign([], { previous: started }), // context.diagnostics(1=failed protocols,2=all calls)
        ...
        passDtmf: undefined, // per-session allow DTMF between bridged legs
        sid: '$' + origin + '$' + unique,
        started: started,
        tags: [], // for cleanup when finished
        timeout: undefined, // for cleaning up a session with no established protocol
        training: undefined,
        transactingMs: 0,
        unique: unique,
...
Object.assign(Session, { // _this_ of all methods is the StateMachine instance
    ...
    reaperMs: 300000, // a session that has not been consumed within 5 minutes will be killed, if context.enableReaper is true
    ...
    enter: function onSessionEnter(evt) { // TODO: vary action based on firstEvt.type [CHANNEL_CREATE|MESSAGE]
        ...
        if (Session.reaperMs)
            this.timeout = worker.resetTimeout.call(this.sid, this.timeout, this.signal.bind(this, 'reaper'), Session.reaperMs); 
    },
    reaper: function onSessionReaper() {
        if (!this.context.enableReaper) {
            debug(this.sid, 'reaper: enableReaper: ', this.context.enableReaper, ', not proceeding');
            return;
        }
        debug(this.sid, 'reaper: cleaning up the session');
        if (this.communicator)
            return this.communicator.enter(null);
        else
    ...
    leave: function onSessionLeave() {
        ...
        debug.enabled && debug(this.sid, 'leave:', UTIL.stringify({ adverts: Object.keys(this.adverts), tags: this.tags }));
        if (this.timeout)
            this.timeout = worker.resetTimeout.call(this.sid, this.timeout);
    ...
    diagnostic: function onSessionDiagnostic(suffix, date) {
        date || (date = new Date);
        this.diagnostics.push(`${date - this.diagnostics.previous}/${suffix || ''}`);
        this.diagnostics.previous = date;
        return date;
    },
    advertise: function (channel) { // advertise to rpscb
        ...
        debug(this.sid, 'advertise:', channel);
    ...
    advise: function onSessionAdvise(channel, command /*, ..., cb */) { // rpscb-to-communicator transaction
        ...
        if (!this.advisees[0]) // ARC leg not connected
            return cb && cb(null, '-NOARCLEG');
        else if (Date.now() - this.transactingMs < exports.graceMs) // protect any ongoing transaction for upto graceMs (usually 10000)
        ...
        args.push(adviseCb = function onSessionAdviseCb(/* err, outcome, ... */) {
            var args = arguments;
            if (command === 'command.close' && (this.advisees[0] ? this.advisees[0].signal('cleanup') : this.signal('consume', true)))
                args = [null, '+SUCCESS'];
        ...
        if (cmds[0] === 'consumer') {
            ...
            if (!this.advisees[0] || !this.advisees[0].signal.apply(this.advisees[0], ['transaction', cmds[0], cmds[1]].concat(args.slice(2))))
                adviseCb(null, '-UNAVAILABLE');
            this.keepalived = keepalived; // restore keepalived timestamp as consumer transaction is not counted
        } else if (!this.advisees[1] || !this.advisees[1].signal.apply(this.advisees[1], ['transaction', cmds[0], {}, cmds[1]].concat(args.slice(2)))) // {} instead of [] indicates outcome indication required
            adviseCb(null, '-UNAVAILABLE');
    },
    tag: function onSessionTag(tags) {
        debug.enabled && debug(this.sid, 'tag:', UTIL.stringify(tags), callsites()[2].toString());
    ...
    json: function onSessionJson(json) { // { ?nowip, ?scaip } - always after offerOutcome-accepted
        ...
        forwarded || console.log(this.sid, 'onSessionJson:', UTIL.stringify(json));
    ...
    detach: function onSessionDetach() { // consume-done, consume-failed
        debug(this.sid, 'detach:', callsites()[2].toString());
        if (this.detached)
            return this;
    ...
    detached: function onSessionDetached() { // Communicator.leave
        ...
        this.detached = true;
        if (this.consumers.every(function (consumer, idx, arr) { // all orphaned consumers must return falsy
            return !consumer.signal('orphaned');
        })) this.enter(null);
        return this;
    },
    unroute: function onSessionUnroute() { // Communicator-detect:selected + Communicator.nowip:transaction:selectCb
        debug(this.sid, 'unroute: discarding previous route lookup');
        if (this.context.callCode) {
            this.context.callCode = undefined;
            // Reset consumers array to avoid re-use of existing consumer in case of grouped device with second call waiting behind first
            this.context.consumers.length = 0;
    ...
    route: function onSessionRoute() {
        ...
        if (this.context.callCode) // routingLookup already done
            return cb();
    ...
    consume: function onSessionConsume(conclusion) { // undefined=communicator-start; false=consumer-failed; true=consumer-bridged
        debug.enabled && debug(this.sid, 'consume:', UTIL.stringify(conclusion), this.detached ? 'detached' : 'attached', callsites()[2].toString());
        this.timeout = worker.resetTimeout.call(this.sid, this.timeout); // clear the reaper timeout
        ...
        } else if ((conclusion || {}).constructor.name === 'Callback') { // prepare consumer-callback
            consumers.push(new main.config.Consumers.callback(this));
        ...
        if (!consumers.length || this.context.truncate){ // no-consumers OR autoAnswer
            if (this.context.truncate) { // generate cdr - if writeCallData called a second time by session.leave, it will return early because of this write -> no duplicate CDR..
                debug(this.sid, 'consume: generating CDR for auto-answer');
                return process.emit('writeCallData', this, this.signal.bind(this, 'detach') || this);  
            } else 
                return this.signal('detach') || this;
        }
        while (true) {
    ...
    contextRelease: function onSessionContextRelease(context) { // ConsumerDetect:Protocols.timeout
        debug.enabled && debug(this.sid, 'contextRelease:', UTIL.stringify(context));
        if (!this.payload.e164)
            return this.communicator.signal('release');

        var nameSlashed = '/scaber/svc/' + this.payload.service + '/from/+' + this.payload.e164 + '/context';
        var sm = this;
        ...
        // context:
        //  refuse: aka truncate/autoAnswer
        ...
        //  bridgePct: define a percentage of calls to be bridged (default: 100)
        ...
        //  record: boolean for selective activation of scaber based call recording
        //  cpn: replacement value for the CPN in the CDR (used for forwardingDDI discovery)
        //  ddi: replacement value for the CPN in the CDR (used for forwardingDDI discovery)
        //  diagnostics: protocol timing audit to CDR (1=failed protocol,2=all calls)
        //  inband: {interMs,volumeDb} activate sending inband DTMF
        //  ignoreChksm: boolean - allows ignoring TT/BS identity checksum failures
        //  noCdr: boolean - prevent calls from generating a CDR (used for hair-pins to ISDNs)
        //  abugs: rtp-bugs to set on A-leg
        //  enableReaper: boolean - allows a session that has not bean consumed within reaperMs to be killed
        //  ---
        ...
        //  tt92NoStmf: undefined=stmf, false=0=stmf, true=1=noStmf, 2=stmfDump
    ...
    consumer: function onSessionConsumer(signal, type, match, /* , ..., ?cb */) { // communicator-to-consumer transaction
        ...
        this.keepalived = new Date(Date.now() + exports.graceMs); // give ourself breathing space for this transaction
        args.push(consumerCb = function onSessionConsumerCb(err) {
            this.keepalived = (Array.isArray(match) ? arguments[1] !== '' : (arguments[1] || '').startsWith('+')) // successful transaction
                ? new Date // update timestamp for keepalive timing
                : keepalived; // restore saved timestamp on failure for keepalive timing
            this.transactingMs = 0;
        ...
        var consumer = this.consumers[this.consumers.index];
        if (!consumer || !consumer.signal.apply(consumer, ['transaction'].concat(args)))
            consumerCb(null, Array.isArray(match) ? '' : '-UNAVAILABLE');
    },
    CHANNEL_CREATE: function onSessionChannelCreate(evt, first) { // worker.onWorkerEslChannel
        ...
        if (evt.headers['variable_sip_h_x-inin-cnv'])
            this.signal('advertise', 'bridge:genid:' + evt.headers['variable_sip_h_x-inin-cnv']);
        ...
        if (evt.headers['Call-Direction'] === 'outbound') { // Consumer
            ...
            forwarded || console.log(this.sid, 'onSessionChannelCreate: Consumer', UTIL.stringify(evt)) || this.enter(null);
        ...
        chain(function cleanup(err) {
            ...
            debug.enabled && debug(sm.sid, 'context:', UTIL.stringify(sm.context));
            forwarded = sm.communicator && sm.communicator.signal(evt.type, evt, first);
            forwarded || console.log(sm.sid, 'onSessionChannelCreate: Communicator', UTIL.stringify(evt));
            ...
            var contexts = [
                ...
                '/scaber/svc/' + sm.payload.service + '/from/' + user + '/context',
            ...
            if ((evt.headers['variable_sip_user_agent'] || '').match(worker.uagentScaber))
                contexts.push('/scaber/uagent/scaber/context');
    ...
    CHANNEL_: function onSessionChannel(evt, first) { // worker.onWorkerEslChannel - catchall for CHANNEL_PROGRESS, CHANNEL_PROGRESS_MEDIA, CHANNEL_ANSWER
        ...
        if (evt.headers['variable_sip_rh_x-inin-cnv'])
            this.signal('advertise', 'bridge:genid:' + evt.headers['variable_sip_rh_x-inin-cnv']);
        ...
        if (evt.headers['Call-Direction'] === 'outbound') { // Consumer
            ...
            forwarded || console.log(this.sid, 'onSessionChannel:', 'Consumer', UTIL.stringify(evt)) || this.enter(null);
            return this; // indicate as handled
        }
        ...
        forwarded || console.log(this.sid, 'onSessionChannel:', 'Communicator', UTIL.stringify(evt)) || this.enter(null);
        return this; // indicate as handled
    },
    CHANNEL_DESTROY: function onSessionChannelDestory(evt, first) { // worker.onWorkerEslChannel
        ...
        if (evt.headers['Call-Direction'] === 'outbound') { // Consumer
            ...
            forwarded = this.consumers[this.consumers.index] && this.consumers[this.consumers.index].signal(evt.type, evt, first);
            forwarded || console.log(this.sid, 'onSessionChannelDestroy:', 'Consumer', UTIL.stringify(evt)) || this.enter(null);
            return this; // indicate as handled
        }
        ...
        forwarded || console.log(this.sid, 'onSessionChannelDestroy:', 'Communicator', UTIL.stringify(evt)) || this.enter(null);
        return this; // indicate as handled
    },
    CUSTOM: function onSessionCustom(evt, first) { // worker.onWorkerEslCustom
        ...
        if (evt.headers['Call-Direction'] === 'outbound') { // Consumer
            ...
            forwarded = this.consumers[this.consumers.index] && this.consumers[this.consumers.index].signal(evt.type, evt, first);
            forwarded || console.log(this.sid, 'onSessionCustom:', 'Consumer', UTIL.stringify(evt)) || this.enter(null);
            return this; // indicate as handled
        }
        ...
        forwarded = this.communicator && this.communicator.signal(evt.type, evt, first);
        forwarded || console.log(this.sid, 'onSessionCustom:', 'Communicator', UTIL.stringify(evt)) || this.enter(null);
        return this; // indicate as handled
    },
    DETECTED_TONE: function onSessionTone(evt, first) { // worker.onWorkerEslTone
        ...
        if (evt.headers['Call-Direction'] === 'outbound') { // Consumer
            ...
            forwarded = this.consumers[this.consumers.index] && this.consumers[this.consumers.index].signal(evt.type, evt, first);
            forwarded || console.log(this.sid, 'onSessionTone:', 'Consumer', UTIL.stringify(evt)) || this.enter(null);
            return this; // indicate as handled
        }
        ...
        forwarded = this.communicator && this.communicator.signal(evt.type, evt, first);
        forwarded || console.log(this.sid, 'onSessionTone:', 'Communicator', UTIL.stringify(evt)) || this.enter(null);
        return this; // indicate as handled
    },
    DTMF: function onSessionDtmf(evt, first) { // worker.onWorkerEslDtmf
        ...
        if (evt.headers['Call-Direction'] === 'outbound') { // Consumer
            ...
            forwarded = this.consumers[this.consumers.index] && this.consumers[this.consumers.index].signal(evt.type, evt, first);
            forwarded || console.log(this.sid, 'onSessionDtmf:', 'Consumer', UTIL.stringify(evt)) || this.enter(null);
            return this; // indicate as handled
        }
        ...
        forwarded = this.communicator && this.communicator.signal(evt.type, evt, first);
        forwarded || console.log(this.sid, 'onSessionDtmf:', 'Communicator', UTIL.stringify(evt)) || this.enter(null);
        return this; // indicate as handled
    },
    MESSAGE: function onSessionMessage(evt, first) { // worker.onWorkerEslMessage - { headers, hPtr, type, subclass, body }, boolean
        var forwarded,
            sid = this.sid,
            direction = evt.headers['Call-Direction'];

        // direction determines whether to send the message to the a-leg aka Communicator (inbound) or b-leg aka Consumer (outbound)
        if (!direction) {
            // for callback services a digitial scheme does not necessarily reply with to_user === payload.originUser
            if (evt.headers['to_user'] === this.payload.originUser || (this.payload.service && this.payload.service.startsWith(evt.headers['from_user']))) {
                direction = 'outbound';
            } else {
                direction = 'inbound';
            }
        ...
        if (direction === 'outbound') {
            ...
            forwarded = this.consumers[this.consumers.index] && this.consumers[this.consumers.index].signal(evt.type, evt, first);
            forwarded || console.log(this.sid, 'onSessionMessage: Consumer', UTIL.stringify(evt)) || this.enter(null);
            return this; // indicate as handled
        }
        ...
        chain(function cleanup(err) {
            ...
            debug.enabled && debug(sm.sid, 'context:', UTIL.stringify(sm.context));
            forwarded = sm.communicator && sm.communicator.signal(evt.type, evt, first);
            forwarded || console.log(sm.sid, 'onSessionMessage: Communicator', UTIL.stringify(evt)) || sm.enter(null);
            sm.sid === sid || debug(sm.sid, 'renamed: from', sid);
            ...
            if (sm.payload.service && sm.payload.service.startsWith('scaip-'))
                contexts.push('/scaber/svc/scaip/context')
    ...
    RECV_INFO: function onSessionInfo(evt, first) {
        ...
        if (evt.headers['Call-Direction'] === 'outbound') { // Consumer
            ...
            forwarded = this.consumers[this.consumers.index] && this.consumers[this.consumers.index].signal(evt.type, evt, first);
            !forwarded && evt.body && console.log(this.sid, 'onSessionInfo:', 'Consumer', UTIL.stringify(evt));
            return this; // indicate as handled
        }
        ...
        forwarded = this.communicator && this.communicator.signal(evt.type, evt, first);
        !forwarded && evt.body && console.log(this.sid, 'onSessionDtmf:', 'Communicator', UTIL.stringify(evt));
        return this; // indicate as handled
    },
});

function oystaReport(session, ocm) {
    if (!session.payload.mrq || !session.context.eventCodes)
        return;

    var message = session.context.eventCodes[0]
        ? session.context.eventCodes[0].sbr_name
        : `${(session.context.eventCodes[1] || {}).sbr_name || '?'} - ${(session.context.eventCodes[2] || {}).sbr_name || '?'}`
    var oysta = {
        method: 'PUT',
    ...
    debug.enabled && debug(session.sid, 'oystaReport:', UTIL.stringify(oysta));
    request(oysta, function (err, resp, json) {
        ...
        debug.enabled && debug(session.sid, 'oystaReport:', UTIL.stringify({ statusCode: resp.statusCode, statusMessage: resp.statusMessage, body: resp.body }));
    });
}
