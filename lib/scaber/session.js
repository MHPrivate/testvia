...
function Session(evt, communicatorName) { // minimal evt is { headers: { 'Caller-Caller-ID-Number', variable_sip_call_id } }
    ...
    Object.assign(this, {
        ...
        leaving: 0,
        payload: { service: service }, // { ?ATM:{version,type,data,time,mac} }, ?mrq:{ref,cid,dty,...} } - NOWIP/SCAIP payload
...
var sessionState = {// _this_ of all methods is the StateMachine instance
    ...
    CHANNEL_CREATE: function onSessionChannelCreate(evt, first) { // called by _enter_
        ...
        chain(function cleanup(err) {
            ...
            var contexts = [
                '/scaber/default/context', '/scaber/svc/' + sm.payload.service + '/context',
                '/scaber/srv/' + os.hostname() + '/context',
            ];
            ...
            evt.headers['variable_sip_req_user'] && contexts.push('/scaber/user/' + evt.headers['variable_sip_req_user'] + '/context');
            ...
            //  lhdigits: 'digits to override the lef-hand digits of the controllerunit'
            //  ---
    ...
    MESSAGE: function onSessionMessage(evt, first) { // { headers, hPtr, type, subclass, body }, boolean
        ...
        chain(function cleanup(err) {
            ...
            evt.headers['from_user'] && contexts.push('/scaber/from/' + evt.headers['from'] + '/context');
            evt.headers['to_user'] && contexts.push('/scaber/user/' + evt.headers['to_user'] + '/context');
