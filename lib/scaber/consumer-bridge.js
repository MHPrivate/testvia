...
require('util').inherits(module.exports = exports = ConsumerBridge, require('../state-machine'));
...
Object.assign(ConsumerBridge, { // _this_ of all methods is the StateMachine instance
    advisee: 0,             // arc
    ...
    activate: function onConsumerBridgeActivate() { // multiple activations by session each time round the list of consumers
        ...
        chain(this.signal.bind(this, 'bridged'), function () {
            esl.executeAsyncX('multiset', ['bypass_media=true hangup_after_bridge=true'], sm.session.communicator.uuid, this);
    ...
    bridged: function onConsumerBridgeBridged(err, evt) { // bridge has completed
        ...
        if (!evt.body.startsWith('+OK')) // ultimately unsuccessful
            this.session.signal('cleanup');
        return this;
    },
    orphaned: function onConsumerBridgeOrphaned() { // communicator has detached
