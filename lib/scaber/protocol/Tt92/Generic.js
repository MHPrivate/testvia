...
Object.assign(exports, { // class setup
    ...
    action: function () {
        ...
        if (this.args[1].regex) // expecting a response
            this.timeout = worker.resetTimeout.call(this.leg.session.sid, this.timeout, this.signal.bind(this, 'action'), sendMs + (this.args[1].retryMs || exports.retryMs));
        else // no response expected
            this.timeout = worker.resetTimeout.call(this.leg.session.sid, this.timeout, this.enter.bind(this, null), sendMs + (this.args[1].delayMs || 70));
        ...
        chain(function cleanup(err) {
            ...
            var n = +sm.leg.stmf;
            debug(sm.leg.session.sid, 'action.2:', module.parent.exports.tonesCmd[n], send, sendMs + 'ms', new Date);
            if (n) {
