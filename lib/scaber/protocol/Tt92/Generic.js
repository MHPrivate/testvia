...
Object.assign(exports, { // class setup
    ...
    action: function () {
        ...
        var send = this.args[1].send,
            sendMs = this.leg.stmf ? esl.tgmlMs(send) : esl.dtmfMs(send),
            ms;
        if (this.args[1].regex) // expecting a response
            this.timeout = worker.resetTimeout.call(this.leg.session.sid, this.timeout, this.signal.bind(this, 'action'), ms = sendMs + (this.args[1].retryMs || exports.retryMs));
        else // no response expected
            this.timeout = worker.resetTimeout.call(this.leg.session.sid, this.timeout, this.enter.bind(this, null), ms = sendMs + (this.args[1].delayMs || 70));
        ...
        chain(function cleanup(err) {
            ...
            var n = +sm.leg.stmf;
            debug(sm.leg.session.sid, 'action.2:', module.parent.exports.tonesCmd[n], send, ms + 'ms', new Date);
            if (n) {
                sm.leg.session.signal('diagnostic', '?');
                esl.executeAsyncX(module.parent.exports.tonesCmd[n], send, sm.leg.uuid, this);
            } else {
                worker.legDtmf(sm.leg, send, null, '?', this); // allow+send+block
            }
