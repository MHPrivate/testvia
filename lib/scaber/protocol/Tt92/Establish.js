...
Object.assign(exports, { // class setup
    ...
    enter: function () {
        ...
        debug(this.leg.session.sid, 'enter:', this.leg.session.context.tt92NoStmf ? 'DTMF only' : 'spandsp_start_tone_detect:stmf', new Date);

        if (!this.leg.session.context.tt92NoStmf)
            esl.executeAsyncX('spandsp_start_tone_detect', ['telecare-stmf'], this.leg.uuid);
    },
    leave: function (conclusion) {
        ...
        chain(function cleanup(err) {
            ...
            if (sm.leg.stmf && sm.verified) // retain spandsp-tone-detection if we have established using STMF
                return this();
            else if (sm.leg.session.context.tt92NoStmf) // not enabled - so don't need to disable 
                return this();
    ...
    action: function () {
        ...
        chain(function cleanup(err) {
            ...
            var n = sm.leg.session.context.tt92NoStmf ? 0 : 1;
            debug(sm.leg.session.sid, 'action:', module.parent.exports.tonesCmd[n], 'ENQ', exports.enqTones[n], new Date);
            if (n) {
                sm.leg.session.signal('diagnostic', 'en9');
                esl.executeAsyncX(module.parent.exports.tonesCmd[n], exports.enqTones[n], sm.leg.uuid, this);
            } else {
                worker.legDtmf(sm.leg, exports.enqTones[n], undefined, 'en9', this); // send only
            }
    ...
    timeout: function () {
        ...
        while (match = exports.DH22.exec(data)) { // until verified, for-each matching segment
            ...
            tt = module.parent.exports.parse(match[1], module.parent.exports.parse.alarm22) || {};
        ...
        chain(function cleanup(err) {
            ...
            var n = +sm.leg.stmf;
            debug(sm.leg.session.sid, 'timeout:', module.parent.exports.tonesCmd[n], 'ACK', exports.ackTones[n], new Date);
            if (n) {
                sm.leg.session.signal('diagnostic', 'ack');
                esl.executeAsyncX(module.parent.exports.tonesCmd[n], exports.ackTones[n], sm.leg.uuid, this);
            } else {
                worker.legDtmf(sm.leg, exports.ackTones[n], undefined, 'ack', this); // send only
            }
