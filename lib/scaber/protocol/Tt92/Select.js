...
Object.assign(exports, { // class setup
    ...
    leave: function (abort) {
        ...
        if (typeof this.args[2] === 'number') { // alarm-driven select - not a directed select
            fetched = this.communicator.grouped.fetch(true);
        ...
        this.communicator.selected = this.args[2];
        debug.enabled && debug(this.communicator.session.sid, 'leave:', JSON.stringify({ selected: this.communicator.selected, speech: this.communicator.speech || 'duplex', fetched: fetched, parsed: this.parsed, payload: payload }));
