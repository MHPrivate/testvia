...
Object.assign(exports, { // class setup
    ...
    enter: function () {
        debug(this.leg.session.sid, new Date, 'enter:', this.attempts ? 'spandsp_start_tone_detect telecare-junk' : 'SKIP');
        ...
        this.timeout = worker.resetTimeout.call(this.leg.session.sid, this.timeout, this.signal.bind(this, 'action'), exports.junkMs); // usually 3700ms
    ...
    leave: function (conclusion) {
        ...
        chain(function cleanup(err) {
            ...
            debug(sm.leg.session.sid, new Date, 'leave.1:', 'spandsp_stop_tone_detect');
            ...
            debug(sm.leg.session.sid, new Date, 'leave.2:', 'block_dtmf');
            worker.legDtmf(sm.leg, '', true, '?', this); // just block

        });
    },
    action: function () {
        debug(this.leg.session.sid, new Date, 'action:', this.attempts);
        if (!this.attempts--)
        ...
        chain(function cleanup(err) {
            ...
            debug(sm.leg.session.sid, new Date, 'action.1:', 'spandsp_stop_tone_detect');
            esl.executeAsyncX('spandsp_stop_tone_detect', [], sm.leg.uuid, this);
            ...
            debug(sm.leg.session.sid, new Date, 'action.2:', 'spandsp_start_tone_detect telecare-burst');
            esl.executeAsyncX('spandsp_start_tone_detect', [sm.dsp = 'telecare-burst'], sm.leg.uuid, this);

        }, function () {
            var ms = worker.legDtmf(sm.leg, exports.enqTones, undefined, 'en0', this) + exports.enqMs; // send only TGML
            debug(sm.leg.session.sid, new Date, 'action.3:', 'ENQ', exports.enqTones, ms + 'ms', new Date);
            sm.timeout = worker.resetTimeout.call(sm.leg.session.sid, sm.timeout, sm.signal.bind(sm, 'action'), ms); // usually 2000ms + 7000ms
    ...
    DETECTED_TONE: function (evt) {
        if (evt.headers['Detected-Tone'].startsWith('telecare-junk')) {
            debug(this.leg.session.sid, new Date, 'TONE:', evt.headers['Detected-Tone'], exports.actionMs + 'ms');
            return this.timeout = worker.resetTimeout.call(this.leg.session.sid, this.timeout, this.signal.bind(this, 'action'), exports.actionMs); // usually 500ms
        }

        this.timeout = worker.resetTimeout.call(this.leg.session.sid, this.timeout, this.signal.bind(this, 'timeout'), exports.silenceMs); // usually 1000ms
        this.data[0] += evt.headers['Detected-Tone']['telecare-burst:'.length];
        debug.enabled && debug(this.leg.session.sid, new Date, 'TONE:', evt.headers['Detected-Tone'], UTIL.stringify(this.data), exports.silenceMs);
        return this;
    },
    timeout: function () {
        if (this.data[0]) // received telecare-burst - so note the protocol for CDR generation
            this.leg.session.payload.protocol = 'TT Old';

        if (/^\d{4,5}$/.test(this.data[0]) === false || this.data[0] !== this.data[1]) {
            debug.enabled && debug(this.leg.session.sid, new Date, 'timeout:', UTIL.stringify({ data: this.data }));
        ...
        debug.enabled && debug(this.leg.session.sid, new Date, 'timeout:', UTIL.stringify({ data: this.data, tt: tt }));
        ...
        var ms = worker.legDtmf(this.leg, exports.ackTones, undefined, 'ack') + exports.ackMs; // send only TGML
        debug(this.leg.session.sid, new Date, 'timeout.1:', 'ACK', exports.ackTones, ms + 'ms');
