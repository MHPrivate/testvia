...
Object.assign(exports, { // class setup
    ...
    leave: function (abort) {
        ...
        debug.enabled && debug(this.leg.session.sid, new Date, 'leave:', JSON.stringify({ selected: this.leg.selected, speech: this.leg.speech || 'duplex', fetched: fetched, parsed: this.parsed, payload: payload }));
    ...
    action: function () {
        ...
        //}, function (evt) {
            ...
            debug(sm.leg.session.sid, new Date, 'action.2:', 'send_dtmf', selectTones, esl.dtmfMs(selectTones) + 'ms');
            worker.legDtmf(sm.leg, selectTones, null, this); // allow+send+block
    ...
    timeout: function () {
        ...
        if (this.parsed = module.parent.exports.parse(this.data, module.parent.exports.parse.alarm8)) // alarm-digits
            ...
            this.enter(null);
        debug(this.leg.session.sid, new Date, 'timeout:', this.data, this.parsed ? 'parsed' : 'invalid', this.leg.grouped && JSON.stringify(this.leg.grouped.alarms));
