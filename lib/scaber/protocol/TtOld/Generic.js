...
function Generic(leg, conclude, args) {
    ...
    debug.enabled && debug(leg.session.sid, new Date, exports.name + ':', util.inspect(args, { breakLength: Infinity }));
...
Object.assign(exports, { // class setup
    ...
    action: function () {
        debug(this.leg.session.sid, new Date, 'action: attempts', this.attempts, this.args[1].send);
        ...
        if (this.args[1].regex) // expecting a response
            this.timeout = worker.resetTimeout.call(this.leg.session.sid, this.timeout, this.signal.bind(this, 'action'), dtmfMs += (this.args[1].retryMs || exports.retryMs));
        else // no response expected
            this.timeout = worker.resetTimeout.call(this.leg.session.sid, this.timeout, this.enter.bind(this, null), dtmfMs += (this.args[1].delayMs || 70));
        ...
        //    debug(sm.leg.session.sid, new Date, 'action.1:', 'allow_dtmf');
        //    esl.executeAsyncX('eval', ['${uuid_drop_dtmf ${uuid} off mask_digits -}'], sm.leg.uuid, this);
        //
        //}, function (evt) {
            debug(sm.leg.session.sid, new Date, 'action.2:', 'send_dtmf', sm.args[1].send, dtmfMs + 'ms');
            worker.legDtmf(sm.leg, sm.args[1].send, null, this); // allow+send+block
            //esl.executeAsyncX('send_dtmf', [sm.args[1].send], sm.leg.uuid, this);
        //
        //}, function () {
        //    debug(sm.leg.session.sid, new Date, 'action.3:', 'block_dtmf');
    ...
    DTMF: function (evt) {
        ...
        this.data += digit;
        debug(this.leg.session.sid, new Date, 'DTMF:', digit + '@' + durationMs, this.data, exports.idleMs + 'ms');
        if (this.args[1].regex) // only update timeout if we're waiting for a response
            this.timeout = worker.resetTimeout.call(this.leg.session.sid, this.timeout, this.signal.bind(this, 'timeout'), exports.idleMs); // usually 230ms
        return this;
    },
    timeout: function () {
        if (this.args[1].regex)
            this.parsed = this.data.match(this.args[1].regex);
        debug(this.leg.session.sid, new Date, 'timeout:', this.data, this.parsed ? 'parsed' : 'invalid');
        return this.enter(null);
    },
