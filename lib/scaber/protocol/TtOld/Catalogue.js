...
Object.assign(exports, { // class setup
    alarmMs: 4000,          // 1st alarm-digits timeout
    fdcDtmf: 'ddda@80',    // acceptor is full duplex capable tones
    ...
    leave: function (abort) {
        ...
        debug.enabled && debug(this.leg.session.sid, new Date, 'leave:', UTIL.stringify(this.leg.grouped));
    ...
    action: function () {
        ...
        chain(function cleanup(err) {
            ...
            debug(sm.leg.session.sid, new Date, 'action.2:', exports.fdcDtmf, 'FDC', new Date);
            worker.legDtmf(sm.leg, exports.fdcDtmf, null, 'fdc', this); // allow+send+block
    ...
    timeout: function () {
        ...
        debug.enabled && debug(this.leg.session.sid, new Date, 'timeout:', this.data || '-', parsed ? 'parsed' : 'invalid', UTIL.stringify(this.leg.grouped.alarms));
        if (!this.callbacks.length)
            null;
        else if (this.callbacks.outcomes)
