...
Object.assign(Stabilise, { // class setup
    ...
    ackDtmf: 'db@80',              // DTMF to acknowledge DataMessage
    ackMs: 230,                     // delay to check ACK has been heard
    dumpMs: 10000,                  // silence delay to forced hangup
    enqDtmf: '*@2000',             // DTMF to provoke the DataMessage
    fdcMs: 230,                     // delay after sending Full Duplex Capable
    fdcDtmf: 'ddda@80',            // DTMF to indicate Full Duplex Capable ARC
    ...
    actionEnq: function () {
        debug(this.leg.session.sid, new Date, 'actionEnq:', 'ENQ', Stabilise.enqDtmf, new Date);
        worker.legDtmf(this.leg, Stabilise.enqDtmf, undefined, 'enO'); // send only
    },
    actionAck: function () {
        var ms = worker.legDtmf(this.leg, Stabilise.ackDtmf, undefined, 'ack') + Stabilise.ackMs; // send only
        debug(this.leg.session.sid, new Date, 'actionAck:', 'ACK', Stabilise.ackDtmf, ms + 'ms', new Date);
        this.timeout = worker.resetTimeout.call(this.leg.session.sid, this.timeout, this.signal.bind(this, 'actionFdc'), ms);
    },
    actionFdc: function () {
        if (!this.tt.hvs)
            return this.enter(null);

        var ms = worker.legDtmf(this.leg, Stabilise.fdcDtmf, undefined, 'fdc') + Stabilise.fdcMs; // send only
        debug(this.leg.session.sid, new Date, 'actionFdc:', 'FDC', Stabilise.fdcDtmf, ms + 'ms', new Date);
    ...
    DTMF: function (evt) {
        ...
        debug(this.leg.session.sid, new Date, 'DTMF:', evt.headers['DTMF-Digit'] + '@' + durationMs, this.data, Stabilise.pauseMs + 'ms');
        return this;
    },
    timeout: function () {
        ...
        debug.enabled && debug(this.leg.session.sid, new Date, 'timeout:', this.data || '-', UTIL.stringify(tt));
        if (!this.verified)
            return this.signal('release');

        Object.assign(this.leg.session.payload, tt && {
            protocol: 'TT Old',
            tt: tt,
