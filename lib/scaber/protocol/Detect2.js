...
Detect2.patterns = Detect2.re({ // _this_ is leg    *** DON'T USE BACKSLASH IN REGEXs ***
    ...
    '-9#$': function tt_21_challenge(match) { // received a TT21 protocol challenge
        debug(this.session.sid, 'tt_21_challenge', UTIL.stringify(match), this.d2.digits.slice(this.d2.lastIndex));
        return false && 'tt_21_challenge'; // passive-matched;
    },
    '-\d#$': function tt_92_challenge(match) { // received a TT92 protocol challenge
        debug(this.session.sid, 'tt_92_challenge', UTIL.stringify(match), this.d2.digits.slice(this.d2.lastIndex));
        return false && 'tt_92_challenge'; // passive-matched;
    },
    '-DB$': function tt_new_challenge(match) { // received a TTNEW protocol challenge
        debug(this.session.sid, 'tt_new_challenge', UTIL.stringify(match), this.d2.digits.slice(this.d2.lastIndex));
        return false && 'tt_new_challenge'; // passive-matched;
    },
