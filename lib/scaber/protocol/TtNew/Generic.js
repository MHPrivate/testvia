...
Object.assign(exports, { // class setup
    ...
    action: function () {
        ...
        if (this.args[1].regex) // expecting a response
            this.timeout = worker.resetTimeout.call(this.leg.session.sid, this.timeout, this.signal.bind(this, 'action'), ms = dtmfMs + (this.args[1].retryMs || exports.retryMs));
        else // no response expected
            this.timeout = worker.resetTimeout.call(this.leg.session.sid, this.timeout, this.enter.bind(this, null), ms = dtmfMs + (this.args[1].delayMs || 70));
        ...
        chain(function cleanup(err) {
            ...
            debug(sm.leg.session.sid, 'action.2:', 'send_dtmf', sm.args[1].send, ms + 'ms', new Date);
