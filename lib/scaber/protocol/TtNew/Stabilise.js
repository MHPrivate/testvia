...
function Stabilise(leg, conclude) {
    ...
    if (this instanceof Stabilise === false)
        throw new Error('Constructor Protocol:TtNew:' + Stabilise.name + ' requires \'new\'');
...
Object.assign(Stabilise, { // class setup
    ...
    enqDtmf: 'da@80',               // DTMF to provoke the DataMessage
    ...
    actionEnq: function () {
        debug(this.leg.session.sid, 'actionEnq:', 'ENQ', Stabilise.enqDtmf, new Date);
        worker.legDtmf(this.leg, Stabilise.enqDtmf, undefined, 'enN'); // send only
    },
    actionAck: function () {
        var ms = worker.legDtmf(this.leg, Stabilise.ackDtmf, undefined, 'ack') + Stabilise.ackMs; // send only
        debug(this.leg.session.sid, 'actionAck:', 'ACK', Stabilise.ackDtmf, ms + 'ms', new Date);
    ...
    actionFdc: function () {
        ...
        debug(this.leg.session.sid, 'actionFdc:', 'FDC', Stabilise.fdcDtmf, ms + 'ms', new Date);
    ...
    timeout: function () {
        if (this.data) // received DTMF - so note the protocol for CDR generation
            this.leg.session.payload.protocol = 'TT New';

        var match,
            tt = this.verified = undefined,
            data = this.data;
