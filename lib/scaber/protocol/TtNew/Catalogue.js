...
Object.assign(exports, { // class setup
    ...
    leave: function (abort) {
        ...
        debug.enabled && debug(this.leg.session.sid, 'leave:', JSON.stringify(this.leg.grouped));
    ...
    action: function () {
        ...
        //
        //}, function (evt) {
            debug(sm.leg.session.sid, 'action.2:', 'send_dtmf', exports.fdcTones, 'FDC', esl.dtmfMs(exports.fdcTones) + 'ms', new Date);
            worker.legDtmf(sm.leg, exports.fdcTones, null, this); // allow+send+block
            //esl.executeAsyncX('send_dtmf', [exports.fdcTones], sm.leg.uuid, this);
        //
        //}, function () {
    ...
    timeout: function () {
        ...
        debug(this.leg.session.sid, 'timeout:', this.data || '-', parsed ? 'parsed' : 'invalid', JSON.stringify(this.leg.grouped.alarms), new Date);
        if (!this.callbacks.length)
            null;
        else if (this.callbacks.outcomes)
