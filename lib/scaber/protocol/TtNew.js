...
var debug = require('debug')('protocol:ttnew');
...
module.exports = exports = new (function TtNew() { // Catalogue/Clear/Select are co-dependent
    Object.assign(this, {
        ...
        Stabilise: require('./TtNew/Stabilise'),    // state-machine to outgoing establish a grouped call
        chksm: chksm,               // checksum calculator function for TTNew data
        debug: debug,               // for reference by substates
        dtmfMaxMs: 2500,            // max valid DTMF duration
        listenDtmf: '*@500',       // also used for keepalive
        parse: parse,               // validating parser function for TTNew data
        speakDtmf: 'c@500',        // also used for keepalive
        transaction: transaction,   // helper function to prepare the necessary dispatch
        unlockDtmf: '3@500',       // tones to achieve door-release
        ...
        case 'paramSet': // ('paramSet', match, param, value) - param is a string, value is a digit-string
            switch (arg2) {
                case 'arc1': // Telephone number 1 (ARC)
                    wksp = { param: '000', value: typeof arg3 === 'string' ? arg3 : '' };
                    break;
                case 'arc2': // Telephone number 2 (ARC)
                    wksp = { param: '001', value: typeof arg3 === 'string' ? arg3 : '' };
                    break;
                case 'unitId1':
                    wksp = { param: '002', value: typeof arg3 === 'string' ? arg3 : '' };
                    break;
                case 'ttcode': // User option tree code
                    wksp = { param: '011', value: typeof arg3 === 'string' ? arg3 : '' };
                    break;
                case 'generic':
                    wksp = { param: ('000' + +arg3).slice(-3), value: typeof arg4 === 'string' ? arg4 : '' };
                    break;
            }
            if (wksp) {
            ...
            switch (arg2) {
                ...
                case 'duplex': // ack: B
                    ...
                    if (!this.grouped) { // always deliver to dispersed Legs
                        match.send = '701@80';
                        match.retryMs = 1100; // over&above the digit send time
                        match.regex = /701/;
                        Substate = exports.Generic;
                        this.speech = 'duplex';
...
function chksm(raw) {
    return Array.from(('00' + (100 - Array.from(raw.toUpperCase()).reduce(function (checksum, digit, idx, arr) {
        return checksum + chksm.checksum.indexOf(digit);
    }, 0) % 100)).slice(-2)).map(function (n, idx, arr) { return (+n + 1) % 10 }).join('');
}
Object.assign(chksm, {
    checksum: Array.from('B1234567890*CAD#'),
});
