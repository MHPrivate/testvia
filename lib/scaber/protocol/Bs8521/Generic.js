...
Object.assign(exports, { // class setup
    ...
    leave: function (abort) {
        ...
        debug.enabled && debug(this.communicator.session.sid, 'leave:', JSON.stringify({ type: this.args[0], subtype: this.args[2], data: data }));
        ...
        if (['catalogue', 'select'].includes(this.args[0]) && (bs8521 = nowip.parse(data.slice(1, -1), 'unit,event,location,priority,status'))) {
            if (this.args[0] === 'select')
                this.communicator.selected = bs8521.unit;
            debug.enabled && debug(this.communicator.session.sid, 'leave: updating payload', JSON.stringify({ selected: this.communicator.selected, bs8521: bs8521 }));
            ...
            this.communicator.selected = null;
            debug.enabled && debug(this.communicator.session.sid, 'leave: pending', +bs8521[1], JSON.stringify({ selected: this.communicator.selected, bs8521: bs8521 }));
