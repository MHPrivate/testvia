...
Object.assign(exports, { // class setup
    ackDtmf: 'b@80',// delay after sending ACK
    ackMs: 100,     // delay after received ACK
    hashMs: NaN,    // delay after a received '#'
    moreMs: 230,    // delay waiting for more data
    regexMs: 110,   // delay after regex match
    retryMs: 500,   // extra delay after sending CMD
    ...
    leave: function (abort) {
        ...
        if (['catalogue', 'select'].includes(this.args[0]) && (bs8521 = nowip.parse(data.slice(1, -1), 'unit,event,location,priority,status'))) {
            ...
            debug.enabled && debug(this.leg.session.sid, 'leave: updating payload', UTIL.stringify({ selected: this.leg.selected, bs8521: bs8521 }));
            ...
            debug.enabled && debug(this.leg.session.sid, 'leave: pending', +bs8521[1], UTIL.stringify({ selected: this.leg.selected, bs8521: bs8521 }));
    ...
    action: function () {
        ...
        var ms = worker.legDtmf(this.leg, this.args[1].send, null, '?') + (this.args[1].retryMs || exports.retryMs); // allow+send+block
    ...
    timeout: function () {
        ...
        chain(function cleanup(err) {
            ...
            debug(sm.leg.session.sid, 'timeout.2:', exports.ackDtmf, sm.data, new Date);
            worker.legDtmf(sm.leg, exports.ackDtmf, null, 'ack', this); // allow+send+block
