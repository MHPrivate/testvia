...
Object.assign(Stabilise, { // class setup
    ...
    actionId: function () {
        debug(this.leg.session.sid, 'actionId:', 'ID', Stabilise.idTones, new Date);
        worker.legDtmf(this.leg, Stabilise.idTones, undefined, 'idB'); // send only
    },
    actionEnq: function () {
        debug(this.leg.session.sid, 'actionEnq:', 'ENQ', Stabilise.enqTones, new Date);
        worker.legDtmf(this.leg, Stabilise.enqTones, undefined, 'enB'); // send only
    },
    actionAck: function () {
        var ms = worker.legDtmf(this.leg, Stabilise.ackTones, undefined, 'ack') + Stabilise.ackMs; // send only
        debug(this.leg.session.sid, 'actionAck:', 'ACK', Stabilise.ackTones, ms + 'ms', new Date);
    ...
    timeout: function () {
        ...
        Object.assign(this.leg.session.payload, bs8521 && {
