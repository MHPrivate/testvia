...
Object.assign(Establish, { // class setup
    ...
    leave: function (conclusion) {
        ...
        debug.enabled && debug(this.leg.session.sid, 'Establish.leave:', JSON.stringify({ conclusion: conclusion }));
    ...
    action: function () {
        ...
        rpscb.publish('offerCli:' + evt.headers['Caller-Caller-ID-Number'], function (err, outs, res) {
            err && console.log(sm.leg.session.sid, Variant + ':Establish.action', outs, err);
    ...
    conclude: function (bridge) {
        ...
        debug.enabled && debug(this.leg.session.sid, 'Establish.conclude:', JSON.stringify({ conclusion: conclusion }), new Date);

        var sm = this;
        chain(function cleanup(err) {
            err && console.log(sm.leg.session.sid, Variant + ':Establish.conclude', err);
            ...
            else if (main.hack.record) // proceed with next protocol
