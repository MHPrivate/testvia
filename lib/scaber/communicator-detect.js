...
Object.assign(CommunicatorDetect, { // _this_ of all methods is the StateMachine instance
    ...
    close: function onCommunicatorClose() {
        ...
        this.interval = worker.resetInterval.call(this.session.sid, this.interval);
        if (!this.protocols[0].transaction)
            return this.signal('substate', this.protocols[0].Close, 'release', undefined); // abandon substate processing

        var args = Object.assign(args = ['quick', [, 'd'], 'close', Object], { Transaction: this.protocols[0].transaction.apply(this, args), signal: 'release' });
        args.Transaction && this.signal.call(this, 'substate', args.Transaction, args.signal, Array.from(args));
        return this;
    },
    clear: function onCommunicatorDetectClear() {
    ...
    cleared: function onCommunicatorDetectCleared(data) {
        debug(this.session.sid, 'cleared: pending', this.grouped.pending, data);
        if (!this.grouped.pending)
            return this.signal('close');

        var args = Object.assign(args = ['catalogue', [, '0'], 0, Object], { Transaction: this.protocols[0].transaction.apply(this, args), signal: 'catalogued' });
        args.Transaction && this.signal.call(this, 'substate', args.Transaction, args.signal, Array.from(args));
    },
    ...
    establish: function onCommunicatorDetectEstablish(conclusion) {
        ...
        switch (conclusion) { // undefined/true/false OR undefined/verified/refused/answer/release
            ...
            case 'verified': // establish-success - from a protocol-Establish
                this.signal('keepalive'); // start keepalive if supported by protocol
    ...
    keepalive: function onCommunicatorDetectKeepalive() { // setup OR reset keepalive interval
        debug(this.session.sid, 'keepalive:', this.interval ? 'reset' : 'setup');
        if (this.protocols[0].keepaliveMs && this.protocols[0].Keepalive)
            this.interval = worker.resetInterval.call(this.session.sid, this.interval, this.signal.bind(this, 'substate', this.protocols[0].Keepalive, undefined, true), this.protocols[0].keepaliveMs);
        return this;
    },
    established: function onCommunicatorDetectEstablished() { // communicator-established
    ...
    catalogued: function onCommunicatorDetectCatalogued(data) {
        debug(this.session.sid, 'catalogued: unit', this.grouped.unit, data);
        if (!this.grouped.unit)
            return this.signal('close');

        var args = Object.assign(args = ['select', [, this.grouped.unit.toString()], this.grouped.unit, Object], { Transaction: this.protocols[0].transaction.apply(this, args), signal: 'selected' });
        args.Transaction && this.signal.call(this, 'substate', args.Transaction, args.signal, Array.from(args));
    },
    selected: function onCommunicatorDetectSelected(data) {
        debug(this.session.sid, 'selected: unit', this.grouped.unit, data);
    ...
    speeched: function onCommunicatorDetectSpeeched() { // communicator-speeched
    ...
    hack: function onCommunicatorDetectHack(arg) {
        if (isNaN(this.nxt)) {
            ...
            exports.max += 60;
            debug(this.session.sid, 'hack: init', JSON.stringify({ nxt: this.nxt, max: exports.max, arg: arg }));
        } else if (arg === 'B') { // received an ACK
            if (this.nxt < 0) // succeeded on 2nd attempt
                this.nxt = -this.nxt;
            exports.max = this.nxt - 1; // this is what worked
            debug(this.session.sid, 'hack: ackn', JSON.stringify({ nxt: this.nxt, max: exports.max, arg: arg }), '**** SUCCESS ****', exports.max);
            return this.session.signal('consume', undefined);
        } else if (arg) { // received a NAK
            if (this.nxt < 0) // failed on 2nd attempt
                this.nxt = -this.nxt;
            debug(this.session.sid, 'hack: nack', JSON.stringify({ nxt: this.nxt, max: exports.max, arg: arg }));
        } else { // received nothing
            if (this.nxt < 0) { // was 2nd attempt
                this.nxt = -this.nxt;
                exports.max = this.nxt - 1;
                debug(this.session.sid, 'hack: abrt', JSON.stringify({ nxt: this.nxt, max: exports.max, arg: arg }), '**** ABORTED ****');
                return this.session.signal('consume', undefined);
            }
            // otherwise retry
            this.nxt = -this.nxt;
            debug(this.session.sid, 'hack: rtry', JSON.stringify({ nxt: this.nxt, max: exports.max, arg: arg }));
        }
        if (this.nxt >= Math.min(exports.max, 10000)) {
            debug(this.session.sid, 'hack: done', JSON.stringify({ nxt: this.nxt, max: exports.max, arg: arg }), '**** FINISHED ****');
            return this.session.signal('consume', undefined);
        }
