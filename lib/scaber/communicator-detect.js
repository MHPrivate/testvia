...
function CommunicatorDetect(session, evt) {
    ...
    CommunicatorDetect.super_.call(this, CommunicatorDetect, {
        ...
        stmf: false,                // whether we're using STMF, set in Tt92:Establish - used by various sub-protocols
...
Object.assign(CommunicatorDetect, { // _this_ of all methods is the StateMachine instance
    Protocols: [ // default protocol delivery order - adjustable via session.context configuration
        require('./protocol/Bridge'),
        require('./protocol/Guard'),
        require('./protocol/Bs8521'),
        require('./protocol/Tt92'),
        require('./protocol/TtNew'),
        require('./protocol/Bsia'),
        require('./protocol/TtOld'),
        require('./protocol/Unknown'),
    ...
    closed: function onCommunicatorDetectClosed(data) { // from close
    ...
    clear: function onCommunicatorDetectClear() { // from session.detach
        ...
        if (!this.grouped || !this.protocols[0].transaction || !this.selected)
    ...
    establish: function onCommunicatorDetectEstablish(conclusion) { // from establish/CHANNEL_CREATE
        ...
        switch (conclusion) { // undefined/true/false OR undefined/verified/refused/answer/release
            ...
            case 'start': // should be from CHANNEL_CREATE
                while (this.protocols.length && !this.protocols[0].Establish) // check for an Establish substate
                    this.protocols.shift();
    ...
    established: function onCommunicatorDetectEstablished() { // from establish
        debug.enabled && debug(this.session.sid, 'established:', UTIL.stringify({ grouped: this.grouped, hvs: this.hvs}));
        ...
        args.Transaction ? this.signal.call(this, 'substate', args.Transaction, args.signal, Array.from(args)) : this.session.signal('route');
        return this;
    },
    ...
    selected: function onCommunicatorDetectSelected() { // from established/catalogued
        debug(this.session.sid, 'selected:', this.selected);
        this.session.signal('unroute'); // discard previous routeLookup so this 'select' runs a fresh routeLookup
        this.timeout = worker.resetTimeout.call(this.session.sid, this.timeout, this.session.signal.bind(this.session, 'route'), exports.selectedMs);
    },
    hack: function onCommunicatorDetectHack(arg) {
        if (isNaN(this.nxt)) {
            ...
            exports.max += 60;
            debug.enabled && debug(this.session.sid, 'hack: init', UTIL.stringify({ nxt: this.nxt, max: exports.max, arg: arg }));
        } else if (arg === 'B') { // received an ACK
            if (this.nxt < 0) // succeeded on 2nd attempt
                this.nxt = -this.nxt;
            exports.max = this.nxt - 1; // this is what worked
            debug.enabled && debug(this.session.sid, 'hack: ackn', UTIL.stringify({ nxt: this.nxt, max: exports.max, arg: arg }), '**** SUCCESS ****', exports.max);
            return this.session.signal('route');
        } else if (arg) { // received a NAK
            if (this.nxt < 0) // failed on 2nd attempt
                this.nxt = -this.nxt;
            debug.enabled && debug(this.session.sid, 'hack: nack', UTIL.stringify({ nxt: this.nxt, max: exports.max, arg: arg }));
        } else { // received nothing
            if (this.nxt < 0) { // was 2nd attempt
                this.nxt = -this.nxt;
                exports.max = this.nxt - 1;
                debug.enabled && debug(this.session.sid, 'hack: abrt', UTIL.stringify({ nxt: this.nxt, max: exports.max, arg: arg }), '**** ABORTED ****');
                return this.session.signal('route');
            }
            // otherwise retry
            this.nxt = -this.nxt;
            debug.enabled && debug(this.session.sid, 'hack: rtry', UTIL.stringify({ nxt: this.nxt, max: exports.max, arg: arg }));
        }
        if (this.nxt >= Math.min(exports.max, 10000)) {
            debug.enabled && debug(this.session.sid, 'hack: done', UTIL.stringify({ nxt: this.nxt, max: exports.max, arg: arg }), '**** FINISHED ****');
            return this.session.signal('route');
        }
    ...
    CHANNEL_CREATE: function onCommunicatorDetectChannelCreate(evt, first) {
        debug(this.session.sid, 'CHANNEL_CREATE:', evt.headers['Unique-ID'], this.session.context.detect, new Date);
        ...
        if (typeof this.session.context.detect === 'string') { // cherry-pick/order the protocols
            this.protocols.length = 0; // flush the array
            this.session.context.detect.split(/,\s*/).forEach(function (name, idx, arr) {
                typeof (exports.Protocols[name] && exports.Protocols[name].Establish) === 'function' // a valid Protocol
                    && !this.protocols.includes(exports.Protocols[name]) // AND not yet added
                    && this.protocols.push(exports.Protocols[name]); // THEN add it
            }, this);
        }
    ...
    CHANNEL_: function onCommunicatorDetectChannel(evt, first) { // miscellaneous CHANNEL_*** events
        ...
        this.session.signal('diagnostic', 'ans', this.answered = new Date); 
        return this;
    },
    CHANNEL_DESTROY: function onCommunicatorDetectChannelDestroy(evt, first) { // a-leg has ended
        ...
        this.session.signal('diagnostic', 'end');
    ...
    DETECTED_TONE: function onCommunicatorDetectTone(evt, first) { // received tone
        var date = this.session.signal('diagnostic', 'T') || new Date,
            signal = this.d2.digit(evt.headers['Detected-Tone']); // offer to Detect2
        // next STMF should be around 180ms
        if (this.substates._ && this.substates._.signal(signal || evt.type, evt, first))
            return this;

        debug(this.session.sid, `TONE:${signal || ''}`, evt.headers['Detected-Tone'], date);
        return this;
    },
    DTMF: function onCommunicatorDetectDtmf(evt, first) { // received dtmf
        var dtmf = evt.headers['DTMF-Digit'] + '@' + (evt.headers['DTMF-Duration'] / 8);
        var date = this.session.signal('diagnostic', dtmf) || new Date,
            signal = this.d2.digit(evt.headers['DTMF-Digit']); // offer to Detect2
        if (this.substates._ && this.substates._.signal(signal || evt.type, evt, first))
            return this;

        debug(this.session.sid, `DTMF:${signal || ''}`, dtmf, date);
        return this;
    },
