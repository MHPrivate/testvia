...
function scaipCid2Tnu(session, tag, cb) { // cb(err, tnu)
    ...
    if (match[2]) { // the SCAIP request supplied a reliable device CLI
        ...
        return cb(null, {tag: match[1] + '+' + exports.e164, cli: match[2]});
    }
    ...
    chain(cb, function () {
        ...
        if ((configs[0] || {}).valueString) {
            ...
            return cb(null, {tag: tag + '+' + exports.e164, cli: configs[0].valueString}); // have device call the common TNU
        }
        ...
        this(null, {tag: tag + '+' + reserve.e164}); // controller CLI is unknown - training DDI
