...
function onWorkerEslDtmf(evt, hdrs, body) {
    ...
    if (session && session.context && session.context.dtmfErrorRate && Math.random() < Math.abs(session.context.dtmfErrorRate)) { // Simulate a DTMF transmission error
        if (session.context.dtmfErrorRate < 0) // Drop the digit
            debug.enabled && debug('%s %s dtmfErrorRate trigger, dropping DTMF %s@%s', process.pid, evt.headers['Unique-ID'], evt.headers['DTMF-Digit'], evt.headers['DTMF-Duration']);
        else { // Duplicate the digit
            debug.enabled && debug('%s %s dtmfErrorRate trigger, duplicating DTMF %s@%s', process.pid, evt.headers['Unique-ID'], evt.headers['DTMF-Digit'], evt.headers['DTMF-Duration']);
