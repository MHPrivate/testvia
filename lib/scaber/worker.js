...
process.on('sessionAnnc', function onSessionAnnc(session) {
    debug.enabled && debug(process.pid, session.sid, 'sessionAnnc:', JSON.stringify(session.tags));
...
process.on('sessionDone', function onSessionDone(session) {
    debug.enabled && debug(process.pid, session.sid, 'sessionDone:', JSON.stringify(session.tags));
...
function legDtmf(leg, dtmf, lock, cb) {
    // lock:
    //  false = allow+send
    //  true = send+block
    //  null = allow+send+block
    //  undefined = send only

    debug(leg.session.sid, 'legDtmf:', JSON.stringify({ dtmf: dtmf, passDtmf: leg.session.context.passDtmf || false }));
    ...
    chain(cb, function () {
        ...
        debug && debug(leg.session.sid, 'allow_dtmf', new Date);
        ...
        debug && debug(leg.session.sid, 'send_dtmf', dtmf, new Date);
        ...
        debug && debug(leg.session.sid, 'block_dtmf', new Date);
...
function scaipCid2Tnu(session, tag, cb) { // cb(err, tnu)
    ...
    chain(cb, function () {
        ...
        this(null, tag + '+' + reserve.e164); // controller CLI is unknown - training DDI
...
function onWorkerEslChannel(evt, hdrs, body) { // {headers,hptr,type,body}, {'Content-Length','Content-Type'}, string
    ...
    if (evt.type !== 'CHANNEL_CREATE' || evt.headers['Call-Direction'] !== 'inbound') {
        null;
    } else { // inbound CHANNEL_CREATE
        if (evt.headers['Caller-Destination-Number'].match(/^\+?\d+\D\d{4}$/))
    ...
    } else if (!(match = (req_params || evt.headers['variable_sip_req_params'] || '').match(exports.appelloScaber))) { // PSTN call with appello=scaber parameter
        debug(' ' + process.pid, 'req_params:', req_params || evt.headers['variable_sip_req_params']) || null;
    } else switch (match[1]) {
        case 'pnc':
...
function onWorkerEslDtmf(evt, hdrs, body) {
    ...
    session ? session.signal(evt.type, evt, false) : debug.enabled && debug(process.pid, 'tags:', JSON.stringify(Object.keys(exports.tags)));
    debug('-' + process.pid, evt.headers['Unique-ID'], evt.type, process.hrtime(hr).reduce(function (w, n, i, a) { return w = w * 1000000000 + n }, 0) / 1000000 + 'ms');
}
...
function onWorkerEslMessage(evt, hdrs, body) {
    ...
    if (exports.retired)
        ...
        hint = 'from!cid ' + JSON.stringify({ user: evt.headers['from_user'] || '', cid: evt.parsed.mrq.cid[0] || '', crd: evt.parsed.mrq.crd || [] });
    ...
    chain(function cleanup(err) {
        ...
        process.running.ready.then(function onWorkerReady() { // only announce takeover when completely ready for service
            ...
            rpscb.on(`bsia:${exports.uuid}`, function onBsiaMessage(bsia, cb) {
                ...
                debug('+' + process.pid, evt.type, new Date, JSON.stringify(bsia));
            ...
            rpscb.on('bsia:bridges', function onBsiaBridges(receiver, cb) {
                debug(process.pid, 'onBsiaBridges:', JSON.stringify(Object.assign({ buid: exports.uuid, host: os.hostname() }, receiver)));
