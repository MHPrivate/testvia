...
function untrain(session, from_user) {
    ...
    chain(function cleanup(err) {
        ...
        // handle AWS 
        if (main.modules.aws.getIsAwsEnvironment()) {
            await main.modules.aws.releaseTrainingDID( training );
            this.noerror();
        } else {
...
function scaipCid2Tnu(session, tag, cb) { // cb(err, tnu)
    ...
    chain(cb, function () {
    ...
    }, async function (configs, meta) { // [{id,nameSlashed,schemeId,touched,valueBoolean,valueNumber,valueString}]
        ...
        // handle AWS 
        if (main.modules.aws.getIsAwsEnvironment()) {
            let reservation = { e164: await main.modules.aws.reserveTrainingDID()};
            // Pass number to the next step
            this.noerror(reservation);
        } else {
...
function onWorkerEslReady() { // _this_ is the Connection
    ...
    chain(function cleanup(err) {
        ...
        this.index = 'startup';
        if (main.modules.aws.getIsAwsEnvironment()) {
            // For AWS use the same set of DID numbers for each instance - REDIS will be used to distribute DIDs when required.
            // Send numbers to master.js here - so that OVH wil work
            mysql('select * from config where schemeId=0 and nameSlashed=?', ['/scaber/default/fqdn/e164s'], this); // plaintext
        } else {
        ...
        //For AWS, store the e164s, so that the REDIS-based mechanism can be initialised when necesary (on 1st number request)
        if (main.modules.aws.getIsAwsEnvironment()) 
            main.modules.aws.storeE164s( e164s );
        ...
        process.running.ready.then(function onWorkerReady() { // only announce takeover when completely ready for service
            ...
            var host = os.hostname(), weight = ((main.secrets.bsia || {})[host] || {}).weight;
            // Only default to 2 where no matching host data was available in main.secrets.bsia - will allow zero weight
            weight = (weight === undefined || isNaN(weight)) ? 2 : weight;
