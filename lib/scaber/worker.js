...
worker.on('workerAuction', function onWorkerAuction(auction, handle) { // _this_ is worker { event: 'auction', origin, unique }, undefined
    ...
    new main.modules.Session({ headers: { 'Caller-Caller-ID-Number': auction.origin, 'variable_sip_call_id': auction.unique } });
});
...
function onWorkerEslChannel(evt, hdrs, body) { // {headers,hptr,type,body}, {'Content-Length','Content-Type'}, string
    ...
    var aleg = (!evt.headers['variable_appello_consumer']) && (evt.headers['Caller-Destination-Number'] in main.config.Communicators); // aleg ? communicator : consumer
    ...
    } else if (evt.headers['Unique-ID'] in exports.routes) { // managed a-leg
    ...
    } else if (aleg && evt.type === 'CHANNEL_CREATE' && evt.headers['Caller-Direction'] === 'inbound') { // new a-leg to manage
