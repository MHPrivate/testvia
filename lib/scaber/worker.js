...
process.on('sessionAnnc', function onSessionAnnc(session) {
    debug.enabled && debug(process.pid, 'sessionAnnc:', session.sid, JSON.stringify(session.routes));
...
process.on('sessionDone', function onSessionDone(session) {
    debug.enabled && debug(process.pid, 'sessionDone:', session.sid, JSON.stringify(session.routes));
    exports.retired && !Object.keys(exports.sessions).length && process.emit('delayedTerminate', true);
    session.training && untrain(session, undefined); // cleanup any learning for this session
...
function untrain(session, from_user) {
    ...
    chain(function cleanup(err) {
        ...
        exports.sendCb({ event: 'release', e164: session.training }, exports.handle, this.noerror);
        session.training = undefined; // only release once
        ...
        if (!from_user && !(session.routes.length && !session.communicator.uuid)) // nothing to update
            return this();
...
function onWorkerEslChannel(evt, hdrs, body) { // {headers,hptr,type,body}, {'Content-Length','Content-Type'}, string
    ...
    debug.enabled && debug('+' + process.pid, evt.type, evt.headers['Unique-ID'], new Date, JSON.stringify({ matched: matched }) || 'unmatched');
...
function onWorkerEslTone(evt, hdrs, body) {
    ...
    if (!evt.headers['Unique-ID'])
        ...
        matched = { 'variable_appello_unique': evt.headers['variable_appello_unique'] };
    debug.enabled && debug('+' + process.pid, evt.type, evt.headers['Unique-ID'], new Date, evt.headers['Detected-Tone'], JSON.stringify({ matched: matched }) || 'unmatched');
    session && session.signal(evt.type, evt, false);
    debug('-' + process.pid, evt.type, evt.headers['Unique-ID'], process.hrtime(hr).reduce(function (w, n, i, a) { return w = w * 1000000000 + n }, 0) / 1000000 + 'ms');
}
...
function onWorkerEslDtmf(evt, hdrs, body) {
    ...
    if (session = exports.routes[evt.headers['Unique-ID']])
        ...
        matched = { 'variable_appello_unique': evt.headers['variable_appello_unique'] };
    debug.enabled && debug('+' + process.pid, evt.type, evt.headers['Unique-ID'], new Date, evt.headers['DTMF-Digit'] + '@' + evt.headers['DTMF-Duration'], JSON.stringify({ matched: matched }) || 'unmatched');
...
function onWorkerEslMessage(evt, hdrs, body) {
    ...
    debug.enabled && debug('+' + process.pid, evt.type, evt.headers['Unique-ID'] || evt.headers['Event-Sequence'], new Date, JSON.stringify({ matched: matched }) || 'unmatched');
    if (session || exports.retired)
        hint = '*A';
    ...
    if (!session)
        return debug('-' + process.pid, evt.type, evt.headers['Unique-ID'] || evt.headers['Event-Sequence'], process.hrtime(hr).reduce(function (w, n, i, a) { return w = w * 1000000000 + n }, 0) / 1000000 + 'ms', hint);

    session.signal(evt.type, evt, !!first);
    debug('-' + process.pid, evt.type, evt.headers['Unique-ID'] || evt.headers['Event-Sequence'], process.hrtime(hr).reduce(function (w, n, i, a) { return w = w * 1000000000 + n }, 0) / 1000000 + 'ms');
}
