...
function onWorkerEslTone(evt, hdrs, body) {
    ...
    if (!evt.headers['Unique-ID'])
        ...
        matched = { 'variable_appello_unique': evt.headers['variable_appello_unique'] };
    debug.enabled && debug('+' + process.pid, evt.type, evt.headers['Unique-ID'], new Date, evt.headers['Detected-Tone'], JSON.stringify(matched) || 'unmatched');
...
function onWorkerEslDtmf(evt, hdrs, body) {
    ...
    if (session = exports.routes[evt.headers['Unique-ID']])
        ...
        matched = { 'variable_appello_unique': evt.headers['variable_appello_unique'] };
    debug.enabled && debug('+' + process.pid, evt.type, evt.headers['Unique-ID'], new Date, evt.headers['DTMF-Digit'] + '@' + evt.headers['DTMF-Duration'], JSON.stringify(matched) || 'unmatched');
...
function onWorkerEslMessage(evt, hdrs, body) {
    ...
    if (evt.scaber) // possibly set by 'process.emit('sipMessagePreProcess', evt)' above (e.g. scaip)
        matched = (session = exports.sessions[evt.scaber.sid]) && { 'sid': evt.scaber.sid };
    ...
    debug.enabled && debug('+' + process.pid, evt.type, evt.headers['Unique-ID'] || evt.headers['Event-Sequence'], new Date, JSON.stringify(matched) || 'unmatched');
