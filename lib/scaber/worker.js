...
function untrain(session, from_user) {
    ...
    chain(function cleanup(err) {
        ...
        if (from_user) // training success - record learnt CLI for next time
            mysql(mysql.mksql('config', { nameSlashed: '/scaip/cid/' + session.payload.mrq.cid[0] + '/e164', schemeId: 0, valueString: from_user }), this);
        else if (session.established && !session.communicator.uuid) // training failure - flush so as to relearn next time
            mysql('delete from config where schemeId=0 and nameSlashed=?', ['/scaip/cid/' + session.payload.mrq.cid[0] + '/e164'], this);
        else
            this();
...
function scaipCid2Tnu(session, tag, cb) { // cb(err, tnu)
    ...
    chain(cb, function () {
        this.index = session.sid;
        mysql('select * from config where schemeId=0 and nameSlashed=?', ['/scaip/cid/' + session.payload.mrq.cid[0] + '/e164'], this);
...
function resetInterval(handle, func, msec, arg0) { // utility function to clear+set a Timeout
    ...
    switch (arguments.length) {
        case 0: return;
        case 1: return;
        case 2: return setInterval(func);
        case 3: return setInterval(func, msec);
        case 4: return setInterval(func, msec, arg0);
        default: return setInterval.apply(null, Array.from(arguments).slice(1));
    }
}
...
function resetTimeout(handle, func, msec, arg0) { // utility function to clear+set a Timeout
    ...
    switch (arguments.length) {
        case 0: return;
        case 1: return;
        case 2: return setTimeout(func);
        case 3: return setTimeout(func, msec);
        case 4: return setTimeout(func, msec, arg0);
        default: return setTimeout.apply(null, Array.from(arguments).slice(1));
    }
}
...
function onWorkerEslChannel(evt, hdrs, body) { // {headers,hptr,type,body}, {'Content-Length','Content-Type'}, string
    ...
    var match, session, matched, training, first;
    ...
    if (session || exports.retired || evt.type !== 'CHANNEL_CREATE' || evt.headers['Call-Direction'] !== 'inbound')
        null; // ignore
    else if (exports.isCommunicatorUser(evt.headers['Caller-Destination-Number'])) // SIP call to <protocol>@host
        first = session = new main.modules.Session(evt, evt.headers['Caller-Destination-Number']); // (evt, 'communicatorName')
    ...
    if (!session)
        return debug('-' + process.pid, evt.type, evt.headers['Unique-ID']);
    ...
    debug('-' + process.pid, evt.type, evt.headers['Unique-ID'] + (training ? 'trained' : ''));
}
...
function onWorkerEslDtmf(evt, hdrs, body) {
    ...
    if (session = exports.routes[evt.headers['Unique-ID']])
        ...
        matched = { 'variable_appello_unique': evt.headers['variable_appello_unique'] };
    debug.enabled && debug('+' + process.pid, evt.type, evt.headers['Event-Sequence'], new Date, evt.headers['DTMF-Digit'] + '@' + evt.headers['DTMF-Duration'], JSON.stringify(matched) || 'unmatched');
    session && session.signal(evt.type, evt, false);
    debug('-' + process.pid, evt.type, evt.headers['Event-Sequence']);
}
...
function onWorkerEslMessage(evt, hdrs, body) {
    ...
    if (!session)
        return debug('-' + process.pid, evt.type, evt.headers['Event-Sequence']);
...
function onWorkerEslCustom(evt, hdrs, body) { // _this_ is the Connection
    switch (evt.subclass) {
        ...
        case 'tu::alarm':
            esl.parseEvt.call(this, evt, hdrs, body);
            var session, matched, first;

            if (session = exports.routes[evt.headers['Unique-ID']]) // Communicator - existing session
                matched = { 'Unique-ID': evt.headers['Unique-ID'] };

            debug.enabled && debug('+' + process.pid, evt.type, evt.headers['Unique-ID'], evt.subclass, new Date, JSON.stringify(matched) || 'unmatched');
            if (!session)
                return debug('-' + process.pid, evt.type, evt.headers['Unique-ID']);

            session.signal(evt.type, evt, false);
            debug('-' + process.pid, evt.type, evt.headers['Unique-ID'], evt.subclass);
            break;
    }
}
