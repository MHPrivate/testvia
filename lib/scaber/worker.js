...
function untrain(session, from_user) {
    debug.enabled && debug(session.sid, 'untrain:', JSON.stringify({ from_user: from_user || 'undefined' }));
...
function scaipCid2Tnu(session, cb) { // cb(err, tnu)
    ...
    chain(cb, function () {
        this.index = session.sid;
        mysql('select * from config where schemeId=0 and nameSlashed=?', ['/scaip/cid/' + session.origin + '/e164'], this);
        ...
        if (!configs.length) // controller CLI is known - regular route
            return exports.sendCb({ event: 'reserve' }, exports.handle, this.noerror);

        session.signal('route', [configs[0].valueString]); // apply CLI based route
        ...
        var gsm = reserve.e164 || exports.e164;
...
function onWorkerEslChannel(evt, hdrs, body) { // {headers,hptr,type,body}, {'Content-Length','Content-Type'}, string
    ...
    var training = !session && (session = exports.trainings[evt.headers['Caller-Destination-Number']]); // cid in CLI training
    ...
    training && untrain(session, evt.headers['Caller-Caller-ID-Number']);
