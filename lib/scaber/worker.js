    ...
    chain(function cleanup(err) {
        ...
        for (var e164 in exports.trainings) {
            ...
            if (exports.trainings[e164] === session && delete exports.trainings[e164]) {
                ...
                return exports.sendCb({ event: 'release', e164: e164 }, exports.handle, loop.bind(this));
            }
        }

        //debug(session.sid, 'untrain:', JSON.stringify({ untrained: this.untrained }));
        if (!session.payload.mrq || process.env.NOTRAINING) // not SCAIP or TRAINING disabled, so training to cleanup
            return this();
        ...
        this.nameSlashed = '/scaip/cid/' + session.payload.mrq.cid[0] + '/e164';
...
function scaipCid2Tnu(session, tag, cb) { // cb(err, tnu)
    ...
    chain(cb, function () {
        this.index = session.sid;
        mysql('select * from config where nameSlashed=? and schemeId=0', ['/scaip/cid/' + session.payload.mrq.cid[0] + '/e164'], this); // plaintext
        ...
        if ((configs[0] || {}).valueString) {
            ...
            return cb(null, 'gsm:+' + exports.e164); // have device call the common TNU
        }
        if (process.env.NOTRAINING)
            return cb();
        ...
        exports.trainings[reserve.e164] = session; // go with reserved training route
