...
function onWorkerEslMessage(evt, hdrs, body) {
    ...
    if (exports.retired)
        ...
        hint = 'from!cid (' + (evt.headers['from_user'] || '') + '/' + (evt.parsed.mrq.cid[0] || '') + ')';
...
function onWorkerEslInfo(evt, hdrs, body) {
    ...
    if (session = exports.routes[evt.headers['Unique-ID']])
        ...
        matched = { 'variable_appello_unique': evt.headers['variable_appello_unique'] };
    debug.enabled && debug('+' + process.pid, evt.type, evt.headers['Unique-ID'], new Date, evt.body, JSON.stringify({ matched: matched }) || 'unmatched');
    session ? session.signal(evt.type, evt, false) : debug(process.pid, 'routes:', JSON.stringify(Object.keys(exports.routes)));
    debug('-' + process.pid, evt.type, evt.headers['Unique-ID'], process.hrtime(hr).reduce(function (w, n, i, a) { return w = w * 1000000000 + n }, 0) / 1000000 + 'ms');
}
