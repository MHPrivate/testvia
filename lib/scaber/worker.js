...
function scaipCid2Tnu(session, tag, cb) { // cb(err, tnu)
    ...
    if (match[2]) { // the SCAIP request supplied a reliable device CLI
        session.signal('route', [match[2].slice(1)]);
...
function onWorkerEslMessage(evt, hdrs, body) {
    ...
    if (!session)
        return debug('-' + process.pid, evt.type, evt.headers['Event-Sequence'], process.hrtime(hr).reduce(function (w, n, i, a) { return w = w * 1000000000 + n }, 0) / 1000000 + 'ms', hint);
