...
Object.assign(CommunicatorBs8521, {
// _this_ of all methods is the StateMachine instance
    ...
    send: function onCommunicatorBs8521Send(cond, op, /* ..., */ cb) { // ?cond(), op, ?args..., ?cb(err)
        ...
        debug(this.session.sid, 'onCommunicatorBs8521Send:', op, sends[op]);
        this.timeout = worker.resetTimeout(this.timeout);
        if (!op || !sends[op] || (cond && !cond.call(this)))
            return cb && cb(), this;

        if (op !== 'acknowledge') // continue accumlating any further received digits into current phase
            this.dtmfs[this.phase = op] = '';
        else if (!this.dtmfs['speechDuplex']) // 'acknowledge' AND not yet received ack for 'speechDuplex'
            this.timeout = worker.resetTimeout(this.timeout, this.signal.bind(this, 'send', 'speechDuplex'), 1000);

        //if (op === 'acknowledge' && this.testing-- > 0) return cb && cb(); // ************** THIS IS FOR TESTING - SHOULD NOT BE HERE FOR LIVE **************
    ...
    answer: function onCommunicatorScaipAnswer() { // request to answer a-leg if not already answered
        ...
        this.keepalive = worker.resetInterval(this.keepalive, this.signal.bind(this, 'send', 'pathNull'), 60000);
        return this;
    },
    CHANNEL_CREATE: function onCommunicatorBs8521ChannelCreate(evt, first) {
        ...
        chain(function cleanup(err) {
            ...
            esl.executeAsyncX('start_dtmf', [], sm.uuid, this);
    ...
    CHANNEL_: function onCommunicatorBs8521Channel(evt, first) { // miscellaneous CHANNEL_*** events
        ...
        var retries = 3;
        var cb = function (err, evt) {
            err && console.log(sm.session.sid, 'onCommunicatorBs8521Channel', err);
            ((evt || { body: '' }).body[0] === '-') && console.log(sm.session.sid, 'onCommunicatorBs8521Channel', evt.body.slice(0, -1));

            debug.enabled && debug(this.session.sid, 'onCommunicatorBs8521ChannelCb:', JSON.stringify({ retries: retries, phase: this.phase, dtmf: this.dtmfs[this.phase] }));
            if (this.dtmfs['dataRequest']) // nothing received so far - schedule another 'dataRequest'
                null;
            else if (!retries--)
                this.timeout = worker.resetTimeout(this.timeout, this.signal.bind(this, 'release'), 5000);
            else
                this.timeout = worker.resetTimeout(this.timeout, this.signal.bind(this, 'send', cond, 'dataRequest', cb), 5000);
        }.bind(this);
        this.timeout = worker.resetTimeout(this.timeout, this.signal.bind(this, 'send', cond, 'dataRequest', cb), 1000);
        return this;
    },
    ...
    DTMF: function onCommunicatorBs8521Dtmf(evt, first) { // received BS8521 dtmf
        ...
        !this.phase || (this.timeout = worker.resetTimeout(this.timeout, timeout, 200));
        if (evt.headers['DTMF-Duration'] > 20000) // digit longer than 2500ms
            null; // ignore digit
        else if (evt.headers['DTMF-Digit'] === 'A' && this.dtmfs[this.phase].slice(-1) === 'A') // out-of-band AND in-band DTMF
            esl.executeAsyncX('stop_dtmf', [], this.uuid); // disable in-band detection
        else
            this.dtmfs[this.phase] += evt.headers['DTMF-Digit'];
        debug(this.session.sid, 'DTMF:', evt.headers['DTMF-Digit'] + '@' + (evt.headers['DTMF-Duration'] / 8), this.phase, this.dtmfs[this.phase]);
        return this;
    },
    dataRequestTimeout: function onCommunicatorBs8521DataRequestTimeout() { // A CC TT GGGGGGGG RRRR EEE LL P SS XX #
        ...
        var match = A26H.exec(this.dtmfs['dataRequest']);
        if (!match)
            return this.dtmfs['dataRequest'] = '';

        Object.assign(this.session.payload, { ATM: { data: [match[1].slice(0, 24)] } });
