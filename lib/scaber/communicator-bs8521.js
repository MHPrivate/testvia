...
module.exports = Object.assign(CommunicatorBs8521, {
    ...
    answerMs: 1000,         // post-answer delay to 1st data-request
    dataReqMs: 5000,        // repeat dataReq delay
    dumpMs: 10000,          // delay to forced hangup
    duplexMs: 1000,         // post dataAck delay to speechDuplex
    hashMs: 80,             // special silence delay following a '#' digit
...
Object.assign(CommunicatorBs8521, { // _this_ of all methods is the StateMachine instance
    ...
    send: function onCommunicatorBs8521Send(cond, op, /* ..., */ cb) { // ?cond(), op, ?args..., ?cb(err)
        ...
        if (op !== 'acknowledge') // continue accumlating any further received digits into current phase
            ...
            this.timeout = worker.resetTimeout(this.timeout, this.signal.bind(this, 'send', 'speechDuplex'), CommunicatorBs8521.duplexMs);
    ...
    DTMF: function onCommunicatorBs8521Dtmf(evt, first) { // received BS8521 dtmf
        ...
        if (this.phase && this.phase !== 'speechDuplex') // only set an inter-digit timeout if expecting DTMF other that speechDuplex-ack
            this.timeout = worker.resetTimeout(this.timeout, timeout, this.dtmfs[this.phase].endsWith('#') ? CommunicatorBs8521.hashMs : CommunicatorBs8521.silenceMs);
    ...
    dataRequestTimeout: function onCommunicatorBs8521DataRequestTimeout() { // A CC TT GGGGGGGG RRRR EEE LL P SS XX #
        ...
        var parsed = {}, match;
        CommunicatorBs8521.A26H.lastIndex = 0;
        ...
        parsed.$.speech = 1; // VOX
        parsed.$.controller = this.dataset + parsed.$.controller.slice(this.dataset.length);
