...
module.exports = Object.assign(CommunicatorBs8521, {
    dtmfMs: 2500,   // max valid DTMF duration
    silenceMs: 180,   // inter-digit timeout (180:360)
});
...
Object.assign(CommunicatorBs8521, { // _this_ of all methods is the StateMachine instance
    ...
    send: function onCommunicatorBs8521Send(cond, op, /* ..., */ cb) { // ?cond(), op, ?args..., ?cb(err)
        ...
        else if (!this.dtmfs['speechDuplex']) // op=='acknowledge' AND not yet received ack for 'speechDuplex'
            this.timeout = worker.resetTimeout(this.timeout, this.signal.bind(this, 'send', 'speechDuplex'), 1000);

        if (op === 'speechDuplex') { // for 'speechDuplex', pretend we've already seen the ack
            this.dtmfs[this.phase] = 'b';
    ...
    CHANNEL_: function onCommunicatorBs8521Channel(evt, first) { // miscellaneous CHANNEL_*** events
        ...
        var cb = function onCommunicatorBs8521ChannelCb(err, evt) {
            ...
            if (retries === inbandRetry) {
                debug(this.session.sid, 'onCommunicatorBs8521ChannelCb: activating in-band DTMF detection');
            ...
            if (this.dtmfs['dataRequest']) // received some data - send no further 'dataRequest's
                ...
                this.timeout = worker.resetTimeout(this.timeout, this.signal.bind(this, 'send', cond, 'dataRequest', cb), 5000); // send Nth 'dataRequest' 5sec thereafter
        }.bind(this);
        this.timeout = worker.resetTimeout(this.timeout, this.signal.bind(this, 'send', cond, 'dataRequest', cb), 1000); // send 1st 'dataRequest' 1sec after answer
        return this;
    },
    ...
    DTMF: function onCommunicatorBs8521Dtmf(evt, first) { // received BS8521 dtmf
        ...
        var timeout = function onCommunicatorBs8521DtmfTimeout() {
        ...
        if (this.phase && this.phase !== 'speechDuplex') // only set an inter-digit timeout if expecting DTMF other that speechDuplex-ack
            this.timeout = worker.resetTimeout(this.timeout, timeout, CommunicatorBs8521.silenceMs);
        if (evt.headers['DTMF-Duration'] / 8 < CommunicatorBs8521.dtmfMs)
    ...
    dataRequestTimeout: function onCommunicatorBs8521DataRequestTimeout() { // A CC TT GGGGGGGG RRRR EEE LL P SS XX #
        ...
        var parsed = {}, match;
        while (match = this.A26H.exec(this.dtmfs[this.phase])) // until verified, for-each matching segment
            if ((parsed = nowip.parse(match[1]) || {}).verified) // parse & validate the NOWIP digits
                break;
        if (!parsed.verified)
            return this.timeout = worker.resetTimeout(this.timeout, this.signal.bind(this, 'release'), 10000);
