...
Object.assign(CommunicatorBs8521Pnc, { // _this_ of all methods is the StateMachine instance
    ...
    scheme: function onCommunicatorBs8521PncScheme(conclusion) {
        debug.enabled && debug(this.session.sid, 'scheme:', UTIL.stringify(conclusion));
    ...
    identity: function onCommunicatorBs8521PncIdentity(conclusion) {
        debug.enabled && debug(this.session.sid, 'identity:', UTIL.stringify(conclusion));
        if (typeof conclusion !== 'object')
            return this.signal('release');
    ...
    commands: function onCommunicatorBs8521PncCommands(conclusion) {
        debug.enabled && debug(this.session.sid, 'commands:', UTIL.stringify(conclusion));
        var match;
        if (conclusion === 'release') {
            return this.signal('release');
        } else {
    ...
    CHANNEL_CREATE: function onCommunicatorBs8521PncChannelCreate(evt, first) { // from esl
        ...
        this.session.payload.outbound = true; // for azure:sbr_direction
...
Object.assign(Scheme, { // class setup
    ...
    lookup: function (match) {
        debug.enabled && debug(this.communicator.session.sid, 'Scheme.lookup:', UTIL.stringify(match));
...
Object.assign(Identity, { // class setup
    ...
    ackMs: 790,                 // delay to check ACK has been heard
    ...
    action: function () {
        ...
        var ms = worker.legDtmf(this.communicator, Identity.helloTones, undefined, 'enB') + Identity.helloMs; // send only
        debug(this.communicator.session.sid, 'Identity.action:', 'ENQ', Identity.helloTones, ms + 'ms', new Date);
        this.timeout = worker.resetTimeout.call(this.communicator.session.sid, this.timeout, this.signal.bind(this, 'action'), ms);
    },
    CUSTOM: function onCommunicatorBs8521PncCustom(evt, first) {
    ...
    DTMF: function (evt) {
        ...
        if (durationMs < CommunicatorBs8521Pnc.dtmfMaxMs || +evt.headers['DTMF-Duration'] === 65535)
    ...
    timeout: function () {
        ...
        var ms = worker.legDtmf(this.communicator, Identity.ackTones, undefined, 'ack') + Identity.ackMs; // send only
        debug(this.communicator.session.sid, 'Identity.timeout:', 'ACK', Identity.ackTones, ms + 'ms', new Date);
        this.timeout = worker.resetTimeout.call(this.communicator.session.sid, this.timeout, this.enter.bind(this, null, { ARC: { data: [data] }, bs8521: parsed }), ms);
    },
});
...
Object.assign(Establish, {
    dataMs: 700, // (160 * 28) + 700,       // delay between repeat data28 sequence
    ...
    sendGuard: function () {
        ...
        var ms = worker.legDtmf(this.communicator, Establish.guardTone, undefined, 'grd') + Establish.guardMs; // send only TGML
        debug(this.communicator.session.sid, 'Establish.guard:', this.guardTries--, Establish.guardTone, ms + 'ms', new Date);
        this.timeout = worker.resetTimeout.call(this.communicator.session.sid, this.timeout, this.signal.bind(this, 'sendGuard'), ms);
    },
    sendData: function () {
        if (!this.dataTries)
            return this.enter(null, 'release');

        var ms = worker.legDtmf(this.communicator, this.dataDtmf, undefined, 'idB') + Establish.dataMs; // send only
        debug(this.communicator.session.sid, 'Establish.sendData:', this.dataTries--, this.dataDtmf, ms + 'ms', new Date);
        this.timeout = worker.resetTimeout.call(this.communicator.session.sid, this.timeout, this.signal.bind(this, 'sendData'), ms);
        this.sent = true;
    },
    CUSTOM: function onCommunicatorBs8521PncCustom(evt, first) {
...
function Commands(communicator, conclude, dialPrefix) {
    ...
    Commands.super_.call(this, Commands, { // instance setup
        ...
        leaving: 0,                 // used to prevent recursive calls to state:leave method
        conclude: conclude,         // callback to signal State complete
        queue: [],                  // queue of A<blah># messages to trot out on each successive ACK (attrib selected records unit selected)
...
Object.assign(Commands, { // class setup
    ...
    dispatch: function (retries) {
        ...
        debug.enabled && debug(this.communicator.session.sid, 'Commands.dispatch:', this.retries, UTIL.stringify(this.queue), new Date);
        if (!this.retries--)
            return;

        var ms = worker.legDtmf(this.communicator, this.queue[0], undefined, '?') + Commands.retryMs; // send only
        this.retry = worker.resetTimeout.call(this.communicator.session.sid, this.retry, this.signal.bind(this, 'dispatch', undefined), ms);
    },
    '/A0(\\d{4})#$': function (match) { // select unit
        ...
        chain(function cleanup(err) {
            ...
            debug.enabled && debug(sm.communicator.session.sid, 'Commands.select:', UTIL.stringify(match), 'uuid_park');
            esl.executeAsyncX('eval', ['${uuid_park ${uuid}}'], sm.communicator.uuid, this); // instead of bgapiX

        }, function () {
            debug.enabled && debug(sm.communicator.session.sid, 'Commands.select:', UTIL.stringify(match), 'queue', ackSelect, sls4n8);
    ...
    '/A1(\\d{4})#$': function (match) { // request catalogue
        ...
        debug.enabled && debug(this.communicator.session.sid, 'Commands.pendings:', UTIL.stringify(match), 'queue', ackPend);
    ...
    '/A2(\\d{2})(\\d{2})#$': function (match) { // equipment control (testing: [2]21234#)
        ...
        debug.enabled && debug(this.communicator.session.sid, 'Commands.control:', UTIL.stringify(match), 'queue', ackCtrl, txt);
    ...
    '/A3(\\d)#$': function (match) { // speech-control (testing: [2]3n#)
        ...
        debug.enabled && debug(this.communicator.session.sid, 'Commands.' + speech[match[1]] + ':', UTIL.stringify(match), Commands.ackDtmf, new Date);
    ...
    '/A4(\\d{2})(\\d{3})(\\d{20})#$': function (match) { // parameter define  (testing: [2]41234501234568901234567890#)
        ...
        debug.enabled && debug(this.communicator.session.sid, 'Commands.define:', UTIL.stringify(match), ackParm, new Date);
    ...
    '/A5(\\d{2})(\\d{3})#$': function (match) { // parameter enquiry (testing: [2]512345#)
        var ackParm = Commands.ackParm.replace('TT', match[1]);
        debug.enabled && debug(this.communicator.session.sid, 'Commands.enquiry:', UTIL.stringify(match), ackParm, new Date);
        this.queue.push(ackParam) && this.signal('dispatch', Commands.retries);
    },
    '/AC(\\d{4})#$': function (match) { // programming mode (testing: [28]1234#)
        debug.enabled && debug(this.communicator.session.sid, 'Commands.program:', UTIL.stringify(match), Commands.ackProg, new Date);
        this.queue.push(Commands.ackProg) && this.signal('dispatch', Commands.retries);
    },
    '/B$': function (match) { // ack (testing: [5]#)
        this.queue.shift();
        debug.enabled && debug(this.communicator.session.sid, 'Commands.ack:', UTIL.stringify(match));
        this.signal('dispatch', undefined);
    },
    '/a7$': function (match) { // speak (testing: [*]7)
        debug.enabled && debug(this.communicator.session.sid, 'Commands.speak:', UTIL.stringify(match), Commands.ackDtmf, new Date);
        worker.legDtmf(this.communicator, Commands.ackDtmf, undefined, 'spk'); // send only
    },
    '/a8$': function (match) { // listen (testing: [*]8)
        debug.enabled && debug(this.communicator.session.sid, 'Commands.listen:', UTIL.stringify(match), Commands.ackDtmf, new Date);
        worker.legDtmf(this.communicator, Commands.ackDtmf, undefined, 'lsn'); // send only
    },
    '/a9$': function (match) { // clear (testing: [*]9)
        var ackClear = 'A000000#@80';
        debug.enabled && debug(this.communicator.session.sid, 'Commands.clear:', UTIL.stringify(match), ackClear, new Date);
        this.queue.push(ackClear) && this.signal('dispatch', Commands.retries);
        if (!this.communicator.bridged)
            return this;

        // must use uuid_park as dptool-park is blocked by the CF_CONTROLLED internal FS flag
        debug.enabled && debug(this.communicator.session.sid, 'Commands.select:', UTIL.stringify(match), 'uuid_park');
        esl.executeAsyncX('eval', ['${uuid_park ${uuid}}'], this.communicator.uuid); // instead of bgapiX

    },
    '/aD$': function (match) { // close (testing: [*0])
        var ms = worker.legDtmf(this.communicator, Commands.ackDtmf, undefined, 'cls') + CommunicatorBs8521Pnc.closeMs; // send only
        debug.enabled && debug(this.communicator.session.sid, 'Commands.close:', UTIL.stringify(match), Commands.ackDtmf, ms + 'ms', new Date);
        this.timeout = worker.resetTimeout.call(this.communicator.session.sid, this.timeout, this.enter.bind(this, null), ms);
    },
    '/a#$': function (match) { // null (testing: [*]#)
        debug.enabled && debug(this.communicator.session.sid, 'Commands.null:', UTIL.stringify(match), new Date);
        worker.legDtmf(this.communicator, Commands.ackDtmf, undefined, 'nul'); // send only
    },
});
