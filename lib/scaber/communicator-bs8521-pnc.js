...
Object.assign(Identity, { // class setup
    ...
    action: function () {
        ...
        worker.legDtmf(this.communicator, Identity.helloTones, undefined, 'enB'); // send only
    },
    CUSTOM: function onCommunicatorBs8521PncCustom(evt, first) {
    ...
    timeout: function () {
        ...
        worker.legDtmf(this.communicator, Identity.ackTones, undefined, 'ack'); // send only
    },
});
...
Object.assign(Establish, {
    ...
    sendGuard: function () {
        ...
        this.communicator.session.signal('diagnostic', 'grd');
    ...
    sendData: function () {
        ...
        this.sent = true;
        worker.legDtmf(this.communicator, this.dataDtmf, undefined, 'idB'); // send only
    },
    CUSTOM: function onCommunicatorBs8521PncCustom(evt, first) {
...
Object.assign(Commands, { // class setup
    ...
    '/A3(\\d)#$': function (match) { // speech-control (testing: [2]3n#)
        ...
        debug.enabled && debug(this.communicator.session.sid, 'Commands.' + speech[match[1]] + ':', UTIL.stringify(match), 'send_dtmf', Commands.ackDtmf, new Date);
    ...
    '/A4(\\d{2})(\\d{3})(\\d{20})#$': function (match) { // parameter define  (testing: [2]41234501234568901234567890#)
        ...
        debug.enabled && debug(this.communicator.session.sid, 'Commands.define:', UTIL.stringify(match), 'send_dtmf', ackParm, new Date);
    ...
    '/A5(\\d{2})(\\d{3})#$': function (match) { // parameter enquiry (testing: [2]512345#)
        var ackParm = Commands.ackParm.replace('TT', match[1]);
        debug.enabled && debug(this.communicator.session.sid, 'Commands.enquiry:', UTIL.stringify(match), 'send_dtmf', ackParm, new Date);
        this.queue.push(ackParam) && this.signal('dispatch', Commands.retries);
    },
    '/AC(\\d{4})#$': function (match) { // programming mode (testing: [28]1234#)
        debug.enabled && debug(this.communicator.session.sid, 'Commands.program:', UTIL.stringify(match), 'send_dtmf', Commands.ackProg, new Date);
    ...
    '/a7$': function (match) { // speak (testing: [*]7)
        debug.enabled && debug(this.communicator.session.sid, 'Commands.speak:', UTIL.stringify(match), 'send_dtmf', Commands.ackDtmf, new Date);
        worker.legDtmf(this.communicator, Commands.ackDtmf, undefined, 'spk'); // send only
    },
    '/a8$': function (match) { // listen (testing: [*]8)
        debug.enabled && debug(this.communicator.session.sid, 'Commands.listen:', UTIL.stringify(match), 'send_dtmf', Commands.ackDtmf, new Date);
    ...
    '/a9$': function (match) { // clear (testing: [*]9)
        ...
        debug.enabled && debug(this.communicator.session.sid, 'Commands.clear:', UTIL.stringify(match), 'send_dtmf', ackClear, new Date);
    ...
    '/aD$': function (match) { // close (testing: [*0])
        debug.enabled && debug(this.communicator.session.sid, 'Commands.close:', UTIL.stringify(match), 'send_dtmf', Commands.ackDtmf, new Date);
        worker.legDtmf(this.communicator, Commands.ackDtmf, undefined, 'cls'); // send only
