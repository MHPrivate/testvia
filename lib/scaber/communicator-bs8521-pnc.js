...
module.exports = CommunicatorBs8521Pnc;
...
require('util').inherits(CommunicatorBs8521Pnc, StateMachine);
...
Object.assign(CommunicatorBs8521Pnc, { // _this_ of all methods is the StateMachine instance
    ...
    commands: function onCommunicatorBs8521PncCommands(conclusion) {
        debug(this.session.sid, 'commands:', JSON.stringify(conclusion));
        var match;
        if (conclusion === 'release') {
            return this.signal('release');
        } else {
...
function Scheme(communicator, conclude, dialPrefix) {
    if (this instanceof Scheme === false)
        throw new Error('Constructor', Variant + ':Scheme requires \'new\'');
...
Object.assign(Scheme, { // class setup
    ...
    enter: function (dialPrefix) {
        debug.enabled && debug(this.communicator.session.sid, 'Scheme.enter:', JSON.stringify({ enterMs: Date.now() - this.communicator.answered }));
        if (dialPrefix)
            return this.signal('lookup', [, dialPrefix]);

        this.timeout = worker.resetTimeout(this.timeout, this.signal.bind(this, 'timeout'), Scheme.timeoutMs);
    },
    leave: function (conclusion) {
        if (this.leaving++) // already leaving - prevent recursion
            return;

        debug.enabled && debug(this.communicator.session.sid, 'Scheme.leave:', JSON.stringify({ leaveMs: Date.now() - this.communicator.answered }));
        this.timeout = worker.resetTimeout(this.timeout);
        this.conclude(conclusion); // Scheme
    },
    timeout: function () {
        debug.enabled && debug(this.communicator.session.sid, 'Scheme.timeout:', JSON.stringify({ actionMs: Date.now() - this.communicator.answered }));
    ...
    lookup: function (match) {
        debug(this.communicator.session.sid, 'Scheme.lookup:', JSON.stringify(match));
...
function Identity(communicator, conclude) {
    if (this instanceof Identity === false)
        throw new Error('Constructor', Variant + ':Identity requires \'new\'');
...
Object.assign(Identity, { // class setup
    ...
    enter: function () {
        debug.enabled && debug(this.communicator.session.sid, 'Identity.enter:', JSON.stringify({ enterMs: Date.now() - this.communicator.answered }));
        this.timeout = worker.resetTimeout(this.timeout, this.signal.bind(this, 'action'), Identity.initialMs);
    },
    leave: function (conclusion) {
        if (this.leaving++) // already leaving - prevent recursion
            return;

        debug.enabled && debug(this.communicator.session.sid, 'Identity.leave:', JSON.stringify({ leaveMs: Date.now() - this.communicator.answered }));
        this.timeout = worker.resetTimeout(this.timeout);
        this.conclude(conclusion || (this.verified ? 'verified' : 'release')); // Identity
    },
    action: function () {
        debug.enabled && debug(this.communicator.session.sid, 'Identity.action:', JSON.stringify({ actionMs: Date.now() - this.communicator.answered }));
        if (!this.attempts--)
            return this.enter(null);

        this.timeout = worker.resetTimeout(this.timeout, this.signal.bind(this, 'action'), Identity.helloMs);
        debug(this.communicator.session.sid, 'Identity.action:', 'send_dtmf', 'ENQ', Identity.helloTones);
        esl.executeAsyncX('send_dtmf', [Identity.helloTones], this.communicator.uuid);
       // esl.executeAsyncX('gentones', ['%(1000,0,770,1633)'], this.communicator.uuid);
    ...
    timeout: function () {
        ...
        if (!parsed.verified)
            return this.timeout = worker.resetTimeout(this.timeout, this.signal.bind(this, 'release'), Identity.dumpMs);

        var data = nowip.stringify(parsed);
        this.timeout = worker.resetTimeout(this.timeout, this.enter.bind(this, null, { ARC: { data: [data] }, bs8521: parsed }), Identity.ackMs);
        debug(this.communicator.session.sid, 'Identity.timeout:', 'send_dtmf', 'ACK', Identity.ackTones);
...
Object.assign(Establish, {
    ...
    sendGuard: function () {
        ...
        debug.enabled && debug(this.communicator.session.sid, 'Establish.guard:', this.guardTries--, 'gentones', Establish.guardTone, JSON.stringify({ guardMs: Date.now() - this.communicator.answered }));
    ...
    sendData: function () {
        ...
        debug.enabled && debug(this.communicator.session.sid, 'Establish.sendData:', this.dataTries--, 'send_dtmf', this.dataDtmf, JSON.stringify({ dataMs: Date.now() - this.communicator.answered }));
...
function Commands(communicator, conclude, dialPrefix) {
    ...
    Commands.super_.call(this, Commands, { // instance setup
        ...
        timeout: undefined,         // timeout handle
        watchdog: undefined,        // watchdog handle
    }); // this, ?initial, ?assign, ?enterArgs...
}
Object.assign(Commands, { // class setup
    ...
    enter: function (cb) {
        debug.enabled && debug(this.communicator.session.sid, 'Commands.enter:', JSON.stringify({ enterMs: Date.now() - this.communicator.answered }));
        this.timeout = worker.resetTimeout(this.timeout, this.enter.bind(this, null), CommunicatorBs8521Pnc.nullMs || 130000);
        return this;
    },
    leave: function (conclusion) {
        if (this.leaving++) // already leaving - prevent recursion
        ...
        debug.enabled && debug(this.communicator.session.sid, 'Commands.leave:', JSON.stringify({ enterMs: Date.now() - this.communicator.answered }));
        this.retry = worker.resetTimeout(this.retry);
        this.timeout = worker.resetTimeout(this.timeout);
    ...
    dispatch: function (retries) {
        ...
        debug(this.communicator.session.sid, 'Commands.dispatch:', this.retries, JSON.stringify(this.queue));
        if (!this.retries--)
            return;

        this.retry = worker.resetTimeout(this.retry, this.signal.bind(this, 'dispatch', undefined), this.queue[0].indexOf('@') * 160 + Commands.retryMs);
    ...
    '/A3(\\d)#$': function (match) { // speech-control (testing: [2]3n#)
        ...
        debug.enabled && debug(this.communicator.session.sid, 'Commands.' + speech[match[1]] + ':', JSON.stringify(match), 'send_dtmf', Commands.ackDtmf);
    ...
    '/A4(\\d{2})(\\d{3})(\\d{20})#$': function (match) { // parameter define  (testing: [2]41234501234568901234567890#)
        ...
        debug.enabled && debug(this.communicator.session.sid, 'Commands.define:', JSON.stringify(match), 'send_dtmf', ackParm);
    ...
    '/A5(\\d{2})(\\d{3})#$': function (match) { // parameter enquiry (testing: [2]512345#)
        var ackParm = Commands.ackParm.replace('TT', match[1]);
        debug.enabled && debug(this.communicator.session.sid, 'Commands.enquiry:', JSON.stringify(match), 'send_dtmf', ackParm);
        this.queue.push(ackParam) && this.signal('dispatch', Commands.retries);
    },
    '/AC(\\d{4})#$': function (match) { // programming mode (testing: [28]1234#)
        debug.enabled && debug(this.communicator.session.sid, 'Commands.program:', JSON.stringify(match), 'send_dtmf', Commands.ackProg);
    ...
    '/a7$': function (match) { // speak (testing: [*]7)
        debug.enabled && debug(this.communicator.session.sid, 'Commands.speak:', JSON.stringify(match), 'send_dtmf', Commands.ackDtmf);
        esl.executeAsyncX('send_dtmf', [Commands.ackDtmf], this.communicator.uuid);
    },
    '/a8$': function (match) { // listen (testing: [*]8)
        debug.enabled && debug(this.communicator.session.sid, 'Commands.listen:', JSON.stringify(match), 'send_dtmf', Commands.ackDtmf);
        esl.executeAsyncX('send_dtmf', [Commands.ackDtmf], this.communicator.uuid);
    },
    '/a9$': function (match) { // clear (testing: [*]9)
        var ackClear = 'A000000#@80';
        debug.enabled && debug(this.communicator.session.sid, 'Commands.clear:', JSON.stringify(match), 'send_dtmf', ackClear);
    ...
    '/aD$': function (match) { // close (testing: [*0])
        debug.enabled && debug(this.communicator.session.sid, 'Commands.close:', JSON.stringify(match), 'send_dtmf', Commands.ackDtmf);
        esl.executeAsyncX('send_dtmf', [Commands.ackDtmf], this.communicator.uuid);
        this.timeout = worker.resetTimeout(this.timeout, this.enter.bind(this, null), CommunicatorBs8521Pnc.closeMs);
    },
    '/a#$': function (match) { // null (testing: [*]#)
        debug.enabled && debug(this.communicator.session.sid, 'Commands.null:', JSON.stringify(match));
        esl.executeAsyncX('send_dtmf', [Commands.ackDtmf], this.communicator.uuid);
    },
});
