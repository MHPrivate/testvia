...
Object.assign(Establish, {
    ...
    sendData: function () {
        ...
        debug.enabled && debug(this.communicator.session.sid, 'Establish.sendData:', this.dataTries--, 'send_dtmf', this.dataDtmf, JSON.stringify({ dataMs: Date.now() - this.communicator.answered }), new Date);
...
Object.assign(Commands, { // class setup
    ...
    dispatch: function (retries) {
        ...
        debug.enabled && debug(this.communicator.session.sid, 'Commands.dispatch:', this.retries, JSON.stringify(this.queue));
        if (!this.retries--)
            return;
