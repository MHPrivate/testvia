...
require('util').inherits(module.exports = exports = ConsumerSlsUser, require('../state-machine'));
...
Object.assign(ConsumerSlsUser, { // _this_ of all methods is the StateMachine instance
    ...
    activate: function onConsumerSlsUserActivate(slsuser) { // { from_name, from_number, privacy, to_number }
        debug.enabled && debug(this.session.sid, 'activate:', UTIL.stringify(slsuser), callsites()[2].toString());
        if (typeof slsuser === 'string')
            return this.signal(slsuser);

        var sm = this;
        chain(function (err) {
            err && console.log(sm.session.sid, 'onConsumerSlsUserActivate:', err);
            ...
            try {
                this.sipUser = sipUsers && sipUsers.length ? JSON.parse(sipUsers[0].value) : { expires: 0 };
            } catch (ex) {
            ...
            if (this.sipUser.hostname !== os.hostname()) // need to expedite via another host
                ...
                this.originate = esl.nvp({
                    ...
                    appello_consumer: !undefined, // true as we are a consumer leg
                    appello_unique: sm.session.sid,
                    fs_send_unsupported_info: true,
                    fs_send_unsupported_message: true,
                    origination_caller_id_name: slsuser.from_name,
                    origination_caller_id_number: slsuser.from_number,
                    origination_privacy: slsuser.privacy,
    ...
    atmCommandControlRelease1: function () {
        debug(this.session.sid, 'atmCommandControlRelease1:', 'send_dtmf', ConsumerSlsUser.doorRelease, new Date);
        worker.legDtmf(this, ConsumerSlsUser.doorRelease, undefined, 'rel'); // send only
    },
    atmCommandControlRelease2: function () {
        debug(this.session.sid, 'atmCommandControlRelease2:', 'send_dtmf', ConsumerSlsUser.doorRelease, new Date);
        worker.legDtmf(this, ConsumerSlsUser.doorRelease, undefined, 'rel'); // send only
    },
    atmCommandControlReleaseKeysafe: function () {
        debug(this.session.sid, 'atmCommandControlReleaseKeysafe:', 'send_dtmf', ConsumerSlsUser.doorRelease, new Date);
        worker.legDtmf(this, ConsumerSlsUser.doorRelease, undefined, 'rel'); // send only
    },
    atmCommandControlReleaseAll: function () {
        debug(this.session.sid, 'atmCommandControlReleaseAll:', 'send_dtmf', ConsumerSlsUser.doorRelease, new Date);
        worker.legDtmf(this, ConsumerSlsUser.doorRelease, undefined, 'rel'); // send only
    },
    orphaned: function oonConsumerSlsUserOrphaned() { // communicator has detached
