...
Object.assign(CommunicatorCallback, {// _this_ of all methods is the StateMachine instance
    ...
    cleanup: function onCommunicatorCallbackCleanup(err, evt) {
        debug.enabled && debug(this.session.sid, 'cleanup:', evt ? UTIL.stringify(evt) : '', callsites()[2].toString());
        if (!this.uuid)
            return this.enter(null) || this;
    ...
    clear: function onCommunicatorCallbackClear(err, evt) {
        debug.enabled && debug(this.session.sid, 'clear:', evt ? UTIL.stringify(evt) : '', callsites()[2].toString());
        if (!this.uuid)
            return this.enter(null) || this;
    ...
    CHANNEL_CREATE: function onCommunicatorCallbackChannelCreate(evt, first) {
        ...
        Object.assign(this.session.payload, {
            //ATM: { data: [nowip.stringify({ controllerunit: originUser })] },
            e164: evt.headers['_e164'] || evt.headers['Caller-Caller-ID-Number'],
            originUser: originUser,
            outbound: true, // affects sbr_call.sbr_direction
        });
    ...
    DTMF: function onCommunicatorCallbackDtmf(evt, first) {
        ...
        this.session.signal('consumer', 'dtmf', {} , evt.headers['DTMF-Digit'], function arcDtmfCb(err, payload) {
            debug(this.session.sid, 'arcDtmfCb:', payload, new Date);
        }.bind(this));
        return this;
    },
    transaction: function onCommunicatorCallbackTransaction(type, subtype /* , ..., cb */) {
    ...
    timeout: function onCommunicatorCallbackTimeout() {
        var dueMs = +this.session.keepalived - Date.now() + 60000;
        ...
        this.session.signal('consumer', 'quick', {}, 'null', function timeoutCb(err, outcome) {
            var dueMs = +this.session.keepalived - Date.now() + 60000;
            debug(this.session.sid, 'timeoutCb:', argsMap(arguments), JSON.stringify({ dueMs: dueMs, keepalived: this.session.keepalived }));
