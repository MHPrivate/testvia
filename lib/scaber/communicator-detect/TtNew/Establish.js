...
Object.assign(exports, { // class setup
    ...
    timeout: function () {
        ...
        debug.enabled && debug(this.communicator.session.sid, 'timeout:', JSON.stringify(tt));
        if (!(tt || {}).verified)
        ...
        if (tt.grouped && 'ttnewGrp' in this.communicator.session.context) // redirect dial-string for any TTNew Grouped Communicator
            return this.communicator.session.signal('contextRelease', { bridge: this.communicator.session.context.ttnewGrp, _ttnewGrp: data });

        if (!tt.hvs && 'ttnewTvs' in this.communicator.session.context) // non-HVS Communicator and have non-HVS redirect dial-string
        ...
        chain(function cleanup(err) {
            ...
            ackMs -= Date.now();
            debug(sm.communicator.session.sid, 'timeout.2:', 'wait', ackMs + 'ms', new Date);
