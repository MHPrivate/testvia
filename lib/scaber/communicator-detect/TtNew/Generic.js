...
Object.assign(exports, { // class setup
    ...
    leave: function (abort) {
        ...
        if (this.args[1].send === true) // fake success (false indicates failure)
            this.parsed = true;
        this.callbacks.length && (this.parsed || !this.args[1].regex ? this.callbacks.shift()() : this.callbacks.shift()(null, ''));
    ...
    action: function () {
        ...
        var digitsMs = esl.digitsMs(this.args[1].send);
        if (this.args[1].regex) // expecting a response
            this.timeout = worker.resetTimeout.call(this.communicator.session.sid, this.timeout, this.signal.bind(this, 'action'), digitsMs + (this.args[1].retryMs || exports.retryMs));
        else // no response expected
            this.timeout = worker.resetTimeout.call(this.communicator.session.sid, this.timeout, this.enter.bind(this, null), digitsMs + (this.args[1].delayMs || 70));
        ...
        chain(function cleanup(err) {
            ...
            debug(sm.communicator.session.sid, 'action.2:', 'send_dtmf', sm.args[1].send, digitsMs + 'ms', new Date);
