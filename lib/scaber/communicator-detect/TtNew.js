...
Object.assign(Establish, { // class setup
    ...
    action: function () {
        ...
        chain(function cleanup(err) {
            ...
            debug(sm.communicator.session.sid, 'Establish.action:', 'gentones', 'ENQ', Establish.enqTones);
    ...
    timeout: function () {
        ...
        if (this.communicator.session.context.ttnewAny) // redirect dial-string for any TTNew Communicator
            return this.communicator.session.signal('contextRelease', { bridge: this.communicator.session.context.ttnewAny, _ttnewAny: data });

        if (!parsed.hvs && this.communicator.session.context.ttnewTvs) // non-HVS Communicator and have non-HVS redirect dial-string
            return this.communicator.session.signal('contextRelease', { bridge: this.communicator.session.context.ttnew, _ttnewTvs: data });
