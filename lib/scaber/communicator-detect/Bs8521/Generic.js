...
Object.assign(exports, { // class setup
    ...
    leave: function (abort) {
        ...
        if (['catalogue', 'select'].includes(this.args[0]) && (parsed = nowip.parse(data.slice(1, -1), 'unit,event,location,priority,status'))) {
            debug.enabled && debug(this.communicator.session.sid, 'leave: updating payload', JSON.stringify(parsed));
            ...
            debug.enabled && debug(this.communicator.session.sid, 'leave: pending', +parsed[1], JSON.stringify(parsed));
            Object.assign(this.communicator.grouped, { pending: +parsed[1], unit: 0 });
        } else {
