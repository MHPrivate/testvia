...
Object.assign(exports, { // class setup
    ...
    enter: function () {
        debug.enabled && debug(this.communicator.session.sid, 'enter:', JSON.stringify({ enterMs: Date.now() - this.communicator.answered }));
        if (!this.attempts)
    ...
    leave: function (conclusion) {
        ...
        debug.enabled && debug(this.communicator.session.sid, 'leave:', JSON.stringify({ leaveMs: Date.now() - this.communicator.answered }));
        this.timeout = worker.resetTimeout.call(this.communicator.session.sid, this.timeout);
        var sm = this;
        chain(function cleanup(err) {
            err && console.log(sm.communicator.session.sid, 'leave:', err);
            ...
            debug(sm.communicator.session.sid, 'leave.1:', 'drop_dtmf');
    ...
    action: function () {
        debug.enabled && debug(this.communicator.session.sid, 'action:', this.attempts, JSON.stringify({ actionMs: Date.now() - this.communicator.answered }));
        if (!this.attempts--)
        ...
        debug(this.communicator.session.sid, 'action:', 'send_dtmf', 'ENQ', exports.enqTones);
    ...
    DTMF: function (evt) {
        ...
        debug(this.communicator.session.sid, 'DTMF:', evt.headers['DTMF-Digit'] + '@' + durationMs, this.data, timeoutMs + 'ms');
        return this;
    },
    timeout: function () {
        ...
        Object.assign(this.communicator.session.payload, { ATM: { data: [data] }, bs8521: parsed, originUser: data.slice(4, 16).replace(/^0+/, '') || '0' });
        ...
        debug(this.communicator.session.sid, 'timeout:', 'send_dtmf', 'ACK', exports.ackTones);
