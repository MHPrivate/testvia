...
Object.assign(exports, { // class setup
    ...
    timeout: function () {
        ...
        debug.enabled && debug(this.communicator.session.sid, 'timeout:', JSON.stringify({ data: this.data, parsed: parsed }));

        if (parsed.grouped && 'ttoldGrp' in this.communicator.session.context) // redirect dial-string for any TTOld Grouped Communicator
            return this.communicator.session.signal('contextRelease', { bridge: this.communicator.session.context.ttoldGrp, _ttoldGrp: data });

        if (!parsed.hvs && 'ttoldTvs' in this.communicator.session.context) // non-HVS Communicator and have non-HVS redirect dial-string
        ...
        Object.assign(this.communicator.session.payload, { tt: parsed, originUser: parsed.identity.replace(/^0+/, '') || '0' });
