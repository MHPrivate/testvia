...
module.exports = new (function Tt92() {
    Object.assign(this, {
...
Object.assign(Establish, { // class setup
    DH22: /D#([0-9*A#]{22})/g,  // regexp to scan TT92 digits
    ...
    enqMs: 2000,                // repeat enquire delay
    ...
    enter: function () {
        ...
        debug(this.communicator.session.sid, 'Establish.enter:', this.communicator.session.context.tt92NoStmf ? 'DTMF only' : 'spandsp_start_tone_detect');
    ...
    leave: function () {
        ...
        chain(function cleanup(err) {
            ...
            sm.communicator.signal('establish', !!sm.verified);
            ...
            debug(sm.communicator.session.sid, 'Establish.leave:', 'spandsp_stop_tone_detect');
            ...
            debug(sm.communicator.session.sid, 'Establish.leave:', 'drop_dtmf');
            esl.executeAsyncX('eval', ['${uuid_drop_dtmf ${uuid} on mask_digits -}'], sm.communicator.uuid, this);

        });
    },
    action: function () {
        debug(this.communicator.session.sid, 'Establish.action:', this.attempts);
        if (!this.attempts--)
        ...
        chain(function cleanup(err) {
            ...
            var n = (sm.communicator.session.context.tt92NoStmf || sm.communicator.session.context.tt92Stmf) ? 0 : 1;
    ...
    release: function () {
        ...
        this.communicator.signal('release');
    },
    DETECTED_TONE: function (evt) {
        this.communicator.stmf = true;
        var gapMs, lastDetect = this.lastDetect;
        ...
        var index = data.indexOf('D#'); // find latest instance of 'D#' in received data
        var segment = data.slice(index); // extract candidate D#22 segment
        var pendingMs = (25 - segment.length) * 160;
        this.timeout = worker.resetTimeout(this.timeout, this.signal.bind(this, 'timeout'), pendingMs);
        debug(this.communicator.session.sid, 'Establish.TONE:', evt.headers['Detected-Tone'], this.data, gapMs || '-', pendingMs);
        return this;
    },
    DTMF: function (evt) {
        ...
        this.timeout = worker.resetTimeout(this.timeout, this.signal.bind(this, 'timeout'), Establish.silenceMs);
        debug(this.communicator.session.sid, 'Establish.DTMF:', evt.headers['DTMF-Digit'] + '@' + durationMs, this.data, Establish.silenceMs + 'ms');
        return this;
    },
    timeout: function () {
        var match, parsed, data = this.data + backfill(this.data, this.lastDetect && 1);
        ...
        if (this.communicator.stmf && this.communicator.session.context.tt92Stmf) // STMF Communicator and have STMF redirect dial-string
            return this.communicator.session.signal('contextRelease', { bridge: this.communicator.session.context.tt92Stmf, _tt92Stmf: data });

        if (this.communicator.session.context.tt92Any) // redirect dial-string for any TT92 Communicator
            return this.communicator.session.signal('contextRelease', { bridge: this.communicator.session.context.tt92Any, _tt92Any: data });

        if (!parsed.hvs && this.communicator.session.context.tt92Tvs) // non-HVS Communicator and have non-HVS redirect dial-string
        ...
        chain(function cleanup(err) {
            ...
            sm.timeout = worker.resetTimeout(sm.timeout, this, Establish.ackMs);
...
Object.assign(Speech, {
    ...
    enter: function () {
        ...
        this.timeout = worker.resetTimeout(this.timeout, this.signal.bind(this, 'action'), Speech.actionMs);
        return this;
    },
    leave: function () {
        if (this.leaving++) // already leaving - prevent recursion
        ...
        this.timeout = worker.resetTimeout(this.timeout);
        this.communicator.signal('phase', 'speech');
    },
    action: function () {
        debug(this.communicator.session.sid, 'Speech.action:', this.communicator.stmf ? 'STMF' : 'DTMF');
        this.timeout = worker.resetTimeout(this.timeout, this.enter.bind(this, null), Speech.silenceMs);
        var sm = this;
        chain(function cleanup(err) {
    ...
    DETECTED_TONE: function (evt) {
        ...
        this.timeout = worker.resetTimeout(this.timeout, this.enter.bind(this, null), isNaN(this.communicator.simplex) ? Speech.silenceMs : 0);
...
Object.assign(Listen, {
    ...
    leave: function () {
        ...
        this.timeout = worker.resetTimeout(this.timeout);
        this.communicator.signal('phase', 'listen');
    },
    action: function () {
        debug(this.communicator.session.sid, 'Listen.action:', this.communicator.stmf ? 'STMF' : 'DTMF');
        this.timeout = worker.resetTimeout(this.timeout, this.enter.bind(this, null), Listen.silenceMs);
        var sm = this;
        chain(function cleanup(err) {
...
function parse(data, fields, self) {
    ...
    if (data.length - match[0].length === 2) { // have 2 checksum digits
        var a, b = parse.checksum1[Array.from(self.raw).reduce(function (wksp, digit, idx, arr) {
