...
Object.assign(Establish, { // class setup
    ...
    leave: function (conclusion) {
        ...
        chain(function cleanup(err) {
            ...
            debug(sm.communicator.session.sid, 'Establish.leave.2:', 'block_dtmf');
    ...
    action: function () {
        ...
        chain(function cleanup(err) {
            ...
            debug(sm.communicator.session.sid, 'Establish.action:', tonesCmd[n], 'ENQ', Establish.enqTones[n], new Date);
    ...
    DETECTED_TONE: function (evt) {
        ...
        debug(this.communicator.session.sid, 'Establish.TONE:', evt.headers['Detected-Tone'], this.data.slice(1), gapMs || '-', pendingMs, new Date);
        return this;
    },
    DTMF: function (evt) {
        ...
        debug(this.communicator.session.sid, 'Establish.DTMF:', evt.headers['DTMF-Digit'] + '@' + durationMs, this.data.slice(1), Establish.silenceMs + 'ms', new Date);
        return this;
    },
    timeout: function () {
        var match,
        ...
        debug.enabled && debug(this.communicator.session.sid, 'Establish.timeout:', this.data, JSON.stringify(parsed), new Date);
        if (!parsed && !this.data.includes('#')) // only numeric DTMF
            return this.enter(null);
        ...
        Object.assign(this.communicator, { grouped: parsed.grouped ? {} : undefined, hvs: parsed.hvs });
        var sm = this;
        chain(function cleanup(err) {
            ...
            debug(sm.communicator.session.sid, 'Establish.timeout:', tonesCmd[+sm.communicator.stmf], 'ACK', Establish.ackTones[+sm.communicator.stmf], new Date);
...
Object.assign(Close, { // class setup
    ...
    action: function () {
        ...
        chain(function cleanup(err) {
            ...
            debug(sm.communicator.session.sid, 'Close.action:', tonesCmd[+sm.communicator.stmf], Close.tones[+sm.communicator.stmf], new Date);
...
Object.assign(Speech, { // class setup
    ...
    action: function () {
        ...
        chain(function cleanup(err) {
            ...
            debug(sm.communicator.session.sid, 'Speech.action:', tonesCmd[+sm.communicator.stmf], sm.tones[+sm.communicator.stmf], new Date);
    ...
    DETECTED_TONE: function (evt) {
        ...
        debug(this.communicator.session.sid, 'Speech.TONE:', evt.headers['Detected-Tone'], this.data, new Date);
        return this;
    },
    DTMF: function (evt) {
        var durationMs = evt.headers['DTMF-Duration'] / 8;
        ...
        if (this.data.includes('702'))
            ...
            this.communicator.simplex = false;
        this.timeout = worker.resetTimeout.call(this.communicator.session.sid, this.timeout, this.enter.bind(this, null), isNaN(this.communicator.simplex) ? Speech.silenceMs : 0);
        debug(this.communicator.session.sid, 'Speech.DTMF:', evt.headers['DTMF-Digit'] + '@' + durationMs, this.data, new Date);
        return this;
    },
});
...
Object.assign(Listen, { // class setup
    ...
    action: function () {
        ...
        chain(function cleanup(err) {
            ...
            debug(sm.communicator.session.sid, 'Listen.action:', tonesCmd[+sm.communicator.stmf], Listen.tones[+sm.communicator.stmf], new Date);
