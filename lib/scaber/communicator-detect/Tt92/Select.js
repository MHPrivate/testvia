...
Object.assign(exports, { // class setup
    ...
    leave: function (abort) {
        ...
        payload.originUser = payload.tt.identity.replace(/\**$/, '').replace(/^0+/, '') + '0' + this.parsed.unit.replace(/D/g, '0');
        Object.assign(payload.tt, {
            callcode: fetched.callcode,
            speech: this.parsed.speech, // D=simplex, 1=duplex
            battery: fetched.battery, // D=ok, 1=low
            location: fetched.location,
        });
        debug.enabled && debug(this.communicator.session.sid, 'leave:', JSON.stringify({ fetched: fetched, parsed: this.parsed, payload: payload }));
    ...
    action: function () {
        ...
        chain(function cleanup(err) {
            ...
            var n = +sm.communicator.stmf,
                selectTones = [('000' + sm.args[2]).slice(-3).replace(/0/g, 'd') + 'b@80'],
                ms = n ? selectTones.push(esl.dtmf2stmf(selectTones[0])) && esl.tgmlMs(selectTones[1]) : esl.dtmfMs(selectTones[0]);
            debug(sm.communicator.session.sid, 'action.2:', module.parent.exports.tonesCmd[n], selectTones[n], ms + 'ms', new Date);
            esl.executeAsyncX(module.parent.exports.tonesCmd[n], [selectTones[n]], sm.communicator.uuid, this);
