...
Object.assign(Commands, { // class setup
    ...
    '/A0(\\d{4})#$': function (match) { // unit select
        ...
        debug.enabled && debug(this.consumer.session.sid, 'Commands.select:', JSON.stringify(match), 'send_dtmf', ackSelect, new Date);
    ...
    '/A1(\\d{4})#$': function (match) { // catalogue
        ...
        debug.enabled && debug(this.consumer.session.sid, 'Commands.pendings:', JSON.stringify(match), 'send_dtmf', ackNext, new Date);
    ...
    '/A2(\\d{2})(\\d{2})#$': function (match) { // equipment control (testing: [2]21234#)
        ...
        debug.enabled && debug(this.consumer.session.sid, 'Commands.control:', JSON.stringify(match), 'send_dtmf', ackCtrl, txt, new Date);
    ...
    '/A3(\\d)#$': function (match) { // speech-control  (testing: [2]3n#)
        ...
        debug.enabled && debug(this.consumer.session.sid, 'Commands.' + speech[match[1]] + ':', JSON.stringify(match), 'send_dtmf', Commands.ackDtmf, new Date);
    ...
    '/A4(\\d{2})(\\d{3})(\\d{20})#$': function (match) { // parameter-define  (testing: [2]41234501234568901234567890#)
        ...
        debug.enabled && debug(this.consumer.session.sid, 'Commands.define:', JSON.stringify(match), 'send_dtmf', ackParm, new Date);
        worker.legDtmf(this.consumer, ackParm, undefined, 'ack'); // send only
    },
    '/A5(\\d{2})(\\d{3})#$': function (match) { // parameter-enquiry (testing: [2]512345#)
        var ackParm = Commands.ackParm.replace('TT', match[1]);
        debug.enabled && debug(this.consumer.session.sid, 'Commands.enquiry:', JSON.stringify(match), 'send_dtmf', ackParm, new Date);
        worker.legDtmf(this.consumer, ackParm, undefined, 'ack'); // send only
    },
    '/AC(\\d{4})#$': function (match) { // programming-mode (testing: [28]1234#)
        debug.enabled && debug(this.consumer.session.sid, 'Commands.program:', JSON.stringify(match), 'send_dtmf', Commands.ackProg, new Date);
    ...
    '/a7$': function (match) { // quick-speak (testing: [*]7)
        debug.enabled && debug(this.consumer.session.sid, 'Commands.speak:', JSON.stringify(match), 'send_dtmf', Commands.ackDtmf, new Date);
        worker.legDtmf(this.consumer, Commands.ackDtmf, undefined, 'ack'); // send only
    },
    '/a8$': function (match) { // quick-listen (testing: [*]8)
        debug.enabled && debug(this.consumer.session.sid, 'Commands.listen:', JSON.stringify(match), 'send_dtmf', Commands.ackDtmf, new Date);
        worker.legDtmf(this.consumer, Commands.ackDtmf, undefined, 'ack'); // send only
    },
    '/a9$': function (match) { // quick-clear (testing: [*]9)
        var ackClear = 'A000000#@80';
        debug.enabled && debug(this.consumer.session.sid, 'Commands.clear:', JSON.stringify(match), 'send_dtmf', ackClear, new Date);
    ...
    '/aD$': function (match) { // quick-close (testing: [*0])
        debug.enabled && debug(this.consumer.session.sid, 'Commands.close:', JSON.stringify(match), 'send_dtmf', Commands.ackDtmf, new Date);
        worker.legDtmf(this.consumer, Commands.ackDtmf, undefined, 'ack'); // send only
        this.success = true;
        this.timeout = worker.resetTimeout.call(this.consumer.session.sid, this.timeout, this.enter.bind(this, null), ConsumerBs8521Pnc.closeMs);
    },
