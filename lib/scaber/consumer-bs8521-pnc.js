...
Object.assign(Establish, {
    ...
    sendGuard: function () {
        ...
        this.timeout = worker.resetTimeout.call(this.consumer.session.sid, this.timeout, this.signal.bind(this, 'sendGuard'), Establish.guardMs || 2000);
        this.consumer.session.signal('diagnostic', 'grd');
    ...
    sendData: function () {
        ...
        debug(this.consumer.session.sid, 'Establish.sendData:', this.dataTries--, 'send_dtmf', this.dataDtmf, new Date);
        this.timeout = worker.resetTimeout.call(this.consumer.session.sid, this.timeout, this.signal.bind(this, 'sendData'), Establish.dataMs || 5180);
        this.sent = true;
        worker.legDtmf(this.consumer, this.dataDtmf, undefined, 'idB'); // send only
    },
...
Object.assign(Commands, { // class setup
    ...
    enter: function (cb) {
        ...
        this.timeout = worker.resetTimeout.call(this.consumer.session.sid, this.timeout, this.enter.bind(this, null), ConsumerBs8521Pnc.nullMs || 130000);
        return this;
    },
    leave: function () {
        if (this.leaving++) // already leaving - prevent recursion
    ...
    '/A0(\\d{4})#$': function (match) { // unit select
        ...
        debug.enabled && debug(this.consumer.session.sid, 'Commands.select:', UTIL.stringify(match), 'send_dtmf', ackSelect, new Date);
    ...
    '/A1(\\d{4})#$': function (match) { // catalogue
        ...
        debug.enabled && debug(this.consumer.session.sid, 'Commands.pendings:', UTIL.stringify(match), 'send_dtmf', ackNext, new Date);
    ...
    '/A2(\\d{2})(\\d{2})#$': function (match) { // equipment control (testing: [2]21234#)
        ...
        debug.enabled && debug(this.consumer.session.sid, 'Commands.control:', UTIL.stringify(match), 'send_dtmf', ackCtrl, txt, new Date);
    ...
    '/A3(\\d)#$': function (match) { // speech-control  (testing: [2]3n#)
        ...
        debug.enabled && debug(this.consumer.session.sid, 'Commands.' + speech[match[1]] + ':', UTIL.stringify(match), 'send_dtmf', Commands.ackDtmf, new Date);
    ...
    '/A4(\\d{2})(\\d{3})(\\d{20})#$': function (match) { // parameter-define  (testing: [2]41234501234568901234567890#)
        ...
        debug.enabled && debug(this.consumer.session.sid, 'Commands.define:', UTIL.stringify(match), 'send_dtmf', ackParm, new Date);
        worker.legDtmf(this.consumer, ackParm, undefined, 'ack'); // send only
    },
    '/A5(\\d{2})(\\d{3})#$': function (match) { // parameter-enquiry (testing: [2]512345#)
        var ackParm = Commands.ackParm.replace('TT', match[1]);
        debug.enabled && debug(this.consumer.session.sid, 'Commands.enquiry:', UTIL.stringify(match), 'send_dtmf', ackParm, new Date);
        worker.legDtmf(this.consumer, ackParm, undefined, 'ack'); // send only
    },
    '/AC(\\d{4})#$': function (match) { // programming-mode (testing: [28]1234#)
        debug.enabled && debug(this.consumer.session.sid, 'Commands.program:', UTIL.stringify(match), 'send_dtmf', Commands.ackProg, new Date);
    ...
    '/a7$': function (match) { // quick-speak (testing: [*]7)
        debug.enabled && debug(this.consumer.session.sid, 'Commands.speak:', UTIL.stringify(match), 'send_dtmf', Commands.ackDtmf, new Date);
        worker.legDtmf(this.consumer, Commands.ackDtmf, undefined, 'ack'); // send only
    },
    '/a8$': function (match) { // quick-listen (testing: [*]8)
        debug.enabled && debug(this.consumer.session.sid, 'Commands.listen:', UTIL.stringify(match), 'send_dtmf', Commands.ackDtmf, new Date);
        worker.legDtmf(this.consumer, Commands.ackDtmf, undefined, 'ack'); // send only
    },
    '/a9$': function (match) { // quick-clear (testing: [*]9)
        var ackClear = 'A000000#@80';
        debug.enabled && debug(this.consumer.session.sid, 'Commands.clear:', UTIL.stringify(match), 'send_dtmf', ackClear, new Date);
        worker.legDtmf(this.consumer, ackClear, undefined, 'ack'); // send only
        //this.success = true;
        //this.timeout = worker.resetTimeout.call(this.consumer.session.sid, this.timeout, this.enter.bind(this, null), ConsumerBs8521Pnc.closeMs);
    },
    '/aD$': function (match) { // quick-close (testing: [*0])
        debug.enabled && debug(this.consumer.session.sid, 'Commands.close:', UTIL.stringify(match), 'send_dtmf', Commands.ackDtmf, new Date);
        worker.legDtmf(this.consumer, Commands.ackDtmf, undefined, 'ack'); // send only
        this.success = true;
        this.timeout = worker.resetTimeout.call(this.consumer.session.sid, this.timeout, this.enter.bind(this, null), ConsumerBs8521Pnc.closeMs);
    },
