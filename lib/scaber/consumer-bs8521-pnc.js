...
require('util').inherits(module.exports = exports = ConsumerBs8521Pnc, StateMachine);
...
Object.assign(ConsumerBs8521Pnc, {// _this_ of all methods is the StateMachine instance
    ...
    closeMs: 450,                   // hangup delay after close command
    ...
    activate: function onConsumerBs8521PncActivate() { // multiple activations by session each time round the list of consumers
        debug.enabled && debug(this.session.sid, 'activate:', UTIL.stringify(this.uris), callsites()[2].toString());
    ...
    originate: function () {
        ...
        this.session.payload.paid = this.session.context.paid && ('sip:' + this.session.context.paid + '@' + os.hostname());
        esl.bgapiX('originate', [esl.nvp({
            ...
            appello_unique: this.session.sid,
            //drop_dtmf: true,
            ...
            'sip_h_P-Asserted-Identity': this.session.payload.paid,
    ...
    cleanup: function onConsumerBs8521PncCleanup(success) { // originate-fail, establish-fail, bridge-fail, CHANNEL_DESTROY
        ...
        if (legs) // await CHANNEL_DESTROY from remaining legs
            ...
            this.timeout = worker.resetTimeout.call(this.session.sid, this.timeout, this.signal.bind(this, 'originate'), ConsumerBs8521Pnc.attemptMs);
        return this;
    },
    ...
    orphaned: function onConsumerBs8521PncOrphaned() { // communicator has detached
    ...
    CHANNEL_CREATE: function onConsumerBs8521PncChannelCreate(evt, first) {
        ...
        this.session.signal('tag', [this.uuid]); // to receive all events for this Consumer channel
        return this;
    },
    ...
    CHANNEL_DESTROY: function onConsumerBs8521PncChannelDestroy(evt, first) { // b-leg has ended
        ...
        if (!legs) // all legs cleaned-up
            this.signal('cleanup', this.success); // try next consumer OR task;
        return this;
    },
...
Object.assign(Establish, {
    dataMs: 700,                    // delay between repeat data28 sequence
    dataTries: 4,                   // attempts to send the A26#
    dataDtmf: 'A26#@80',            // base DTMF - '26' will be replaced
    guardMs: 2750,                  // delay between repeat guard tones
    ...
    enter: function () {
        ...
        this.timeout = worker.resetTimeout.call(this.consumer.session.sid, this.timeout, this.signal.bind(this, 'sendGuard'), Establish.initialMs);
        return this;
    },
    leave: function () {
        if (this.leaving++) // already leaving - prevent recursion
        ...
        this.timeout = worker.resetTimeout.call(this.consumer.session.sid, this.timeout);
    ...
    sendGuard: function () {
        ...
        var ms = worker.legDtmf(this.consumer, Establish.guardTone, undefined, 'grd') + Establish.guardMs; // send only TGML
        debug(this.consumer.session.sid, 'Establish.guard:', this.guardTries--, Establish.guardTone, ms + 'ms', new Date);
        this.timeout = worker.resetTimeout.call(this.consumer.session.sid, this.timeout, this.signal.bind(this, 'sendGuard'), ms);
    },
    sendData: function () {
        if (!this.dataTries)
            return this.enter(null);

        var ms = worker.legDtmf(this.consumer, this.dataDtmf, undefined, 'idB') + Establish.dataMs; // send only
        debug(this.consumer.session.sid, 'Establish.sendData:', this.dataTries--, this.dataDtmf, ms + 'ms', new Date);
        this.timeout = worker.resetTimeout.call(this.consumer.session.sid, this.timeout, this.signal.bind(this, 'sendData'), ms);
        this.sent = true;
    },
...
Object.assign(Commands, { // class setup
    ...
    enter: function (cb) {
        ...
        this.timeout = worker.resetTimeout.call(this.consumer.session.sid, this.timeout, this.enter.bind(this, null), ConsumerBs8521Pnc.nullMs);
        return this;
    },
    leave: function () {
        if (this.leaving++) // already leaving - prevent recursion
    ...
    '/A0(\\d{4})#$': function (match) { // unit select
        var ackSelect = 'A' + match[1] + '00000000#@80';
        if (!this.queue.selected)
            ackSelect = 'A' + this.consumer.bs8521.raw.slice(12, -2) + '#@80';
        debug.enabled && debug(this.consumer.session.sid, 'Commands.select:', UTIL.stringify(match), ackSelect, new Date);
        this.queue.selected = true;
        worker.legDtmf(this.consumer, ackSelect, undefined, 'ack'); // send only
    },
    '/A1(\\d{4})#$': function (match) { // catalogue
        if (!this.queue.selected)
            this.queue.push('A' + this.consumer.bs8521.raw.slice(12, -2) + '#@80');
        this.queue.push('A000000000000#@80');
        var ackNext = this.queue.shift();
        debug.enabled && debug(this.consumer.session.sid, 'Commands.pendings:', UTIL.stringify(match), ackNext, new Date);
        worker.legDtmf(this.consumer, ackNext, undefined, 'ack'); // send only
    },
    '/A2(\\d{2})(\\d{2})#$': function (match) { // equipment control (testing: [2]21234#)
        ...
        if (match[1] !== '00') { // TT
            ...
            case '01':// Activate door release 1
                ...
                if (this.consumer.session.communicator.signal('atmCommandControlRelease1'))
                    ackCtrl = ackCtrl.replace('NN', match[2]);
                break;
            case '02':// Activate door release 1
                txt = 'Activate door release 2';
                if (this.consumer.session.communicator.signal('atmCommandControlRelease2'))
                    ackCtrl = ackCtrl.replace('NN', match[2]);
                break;
        ...
        debug.enabled && debug(this.consumer.session.sid, 'Commands.control:', UTIL.stringify(match), ackCtrl, txt, new Date);
        worker.legDtmf(this.consumer, ackCtrl, undefined, 'ack'); // send only
    },
    '/A3(\\d)#$': function (match) { // speech-control  (testing: [2]3n#)
        ...
        debug.enabled && debug(this.consumer.session.sid, 'Commands.' + speech[match[1]] + ':', UTIL.stringify(match), Commands.ackDtmf, new Date);
        worker.legDtmf(this.consumer, Commands.ackDtmf, undefined, 'ack'); // send only
    },
    '/A4(\\d{2})(\\d{3})(\\d{20})#$': function (match) { // parameter-define  (testing: [2]41234501234568901234567890#)
        ...
        debug.enabled && debug(this.consumer.session.sid, 'Commands.define:', UTIL.stringify(match), ackParm, new Date);
        worker.legDtmf(this.consumer, ackParm, undefined, 'ack'); // send only
    },
    '/A5(\\d{2})(\\d{3})#$': function (match) { // parameter-enquiry (testing: [2]512345#)
        var ackParm = Commands.ackParm.replace('TT', match[1]);
        debug.enabled && debug(this.consumer.session.sid, 'Commands.enquiry:', UTIL.stringify(match), ackParm, new Date);
        worker.legDtmf(this.consumer, ackParm, undefined, 'ack'); // send only
    },
    '/AC(\\d{4})#$': function (match) { // programming-mode (testing: [28]1234#)
        debug.enabled && debug(this.consumer.session.sid, 'Commands.program:', UTIL.stringify(match), Commands.ackProg, new Date);
        worker.legDtmf(this.consumer, Commands.ackProg, undefined, 'ack'); // send only
    },
    '/B$': function (match) { // acknowledge (testing: [5]#)
        var ackNext = this.queue.shift();
        debug.enabled && debug(this.consumer.session.sid, 'Commands.ack:', UTIL.stringify(match), ackNext || '-');
        if (!ackNext)
            return;

        worker.legDtmf(this.consumer, Commands.ackNext, undefined, 'ack'); // send only
    },
    '/a7$': function (match) { // quick-speak (testing: [*]7)
        debug.enabled && debug(this.consumer.session.sid, 'Commands.speak:', UTIL.stringify(match), Commands.ackDtmf, new Date);
        worker.legDtmf(this.consumer, Commands.ackDtmf, undefined, 'ack'); // send only
    },
    '/a8$': function (match) { // quick-listen (testing: [*]8)
        debug.enabled && debug(this.consumer.session.sid, 'Commands.listen:', UTIL.stringify(match), Commands.ackDtmf, new Date);
        worker.legDtmf(this.consumer, Commands.ackDtmf, undefined, 'ack'); // send only
    },
    '/a9$': function (match) { // quick-clear (testing: [*]9)
        var ackClear = 'A000000#@80';
        debug.enabled && debug(this.consumer.session.sid, 'Commands.clear:', UTIL.stringify(match), ackClear, new Date);
        var ms = worker.legDtmf(this.consumer, ackClear, undefined, 'ack') + ConsumerBs8521Pnc.closeMs; // send only
        //this.timeout = worker.resetTimeout.call(this.consumer.session.sid, this.timeout, this.enter.bind(this, null), ms);
        //this.success = true;
    },
    '/aD$': function (match) { // quick-close (testing: [*0])
        var ms = worker.legDtmf(this.consumer, Commands.ackDtmf, undefined, 'ack') + ConsumerBs8521Pnc.closeMs; // send only
        debug.enabled && debug(this.consumer.session.sid, 'Commands.close:', UTIL.stringify(match), Commands.ackDtmf, ms + 'ms', new Date);
        this.timeout = worker.resetTimeout.call(this.consumer.session.sid, this.timeout, this.enter.bind(this, null), ms);
        this.success = true;
    },
    '/a#$': function (match) { // quick-null (testing: [*]#)
        debug.enabled && debug(this.consumer.session.sid, 'Commands.null:', UTIL.stringify(match));
        worker.legDtmf(this.consumer, Commands.ackDtmf, undefined, 'ack'); // send only
    },
});
