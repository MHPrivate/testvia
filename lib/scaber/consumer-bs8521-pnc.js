...
Object.assign(ConsumerBs8521Pnc, {// _this_ of all methods is the StateMachine instance
    ...
    activate: function onConsumerBs8521PncActivate() { // multiple activations by session each time round the list of consumers
        debug(this.session.sid, 'activate:', JSON.stringify(this.uris), callsites()[2].toString());
    ...
    answered: function onConsumerBs8521PncAnswered() { // CHANNEL_ANSWER
        ...
        this.signal('substate', Establish, 'established', this.bs8521.raw);
    },
    established: function onCunsumerBs8521PncEstablished(acknowledged) { // Substate-Establish
        debug(this.session.sid, 'established:', acknowledged ? 'acknowledged' : 'retry');
        if (!acknowledged)
            return this.signal('cleanup', false) || this;
...
Object.assign(Establish, {
    ...
    DTMF: function (evt, first) {
        ...
        if (ConsumerBs8521Pnc.testing && evt.headers['DTMF-Digit'] === '#')
            evt.headers['DTMF-Digit'] = durationMs > 994 ? 'b' : 'B';
...
function Commands(consumer, next) {
    ...
    Commands.super_.call(this, Commands, {
        consumer: consumer,         // reference to parent state-machine
...
Object.assign(Commands, {
    ...
    '/A0(\\d{4})#$': function (match) {
        var ackSelect = 'A' + this.consumer.bs8521.raw.slice(12, -2) + '#@80';
        debug(this.consumer.session.sid, 'Commands.select:', JSON.stringify(match), 'send_dtmf', ackSelect);
    ...
    '/A1(\\d{4})#$': function (match) {
        ...
        debug(this.consumer.session.sid, 'Commands.pendings:', JSON.stringify(match), 'send_dtmf', ackNext);
    ...
    '/A2(\\d{2})(\\d{2})#$': function (match) { // equipment control (testing: [2]21234#)
        ...
        debug(this.consumer.session.sid, 'Commands.control:', JSON.stringify(match), 'send_dtmf', ackCtrl, txt);
    ...
    '/A3(\\d)#$': function (match) { // default  (testing: [2]3n#)
        ...
        debug(this.consumer.session.sid, 'Commands.'+ speech[match[1]] + ':', JSON.stringify(match), 'send_dtmf', Commands.ackDtmf);
    ...
    '/A4(\\d{2})(\\d{3})(\\d{20})#$': function (match) { // parameter define  (testing: [2]41234501234568901234567890#)
        ...
        debug(this.consumer.session.sid, 'Commands.define:', JSON.stringify(match), 'send_dtmf', ackParm);
        esl.executeAsyncX('send_dtmf', [ackParm], this.consumer.uuid);
    },
    '/A5(\\d{2})(\\d{3})#$': function (match) { // parameter enquiry (testing: [2]512345#)
        var ackParm = Commands.ackParm.replace('TT', match[1]);
        debug(this.consumer.session.sid, 'Commands.enquiry:', JSON.stringify(match), 'send_dtmf', ackParm);
        esl.executeAsyncX('send_dtmf', [ackParm], this.consumer.uuid);
    },
    '/AC(\\d{4})#$': function (match) { // programming mode (testing: [28]1234#)
        debug(this.consumer.session.sid, 'Commands.program:', JSON.stringify(match), 'send_dtmf', Commands.ackProg);
        esl.executeAsyncX('send_dtmf', [Commands.ackProg], this.consumer.uuid);
    },
    '/B$': function (match) { // ack (testing: [5]#)
        var ackNext = this.queue.shift();
        debug(this.consumer.session.sid, 'Commands.ack:', JSON.stringify(match), ackNext || '-');
        if (!ackNext)
            return;
    ...
    '/a7$': function (match) { // speak (testing: [*]7)
        debug(this.consumer.session.sid, 'Commands.speak:', JSON.stringify(match), 'send_dtmf', Commands.ackDtmf);
        esl.executeAsyncX('send_dtmf', [Commands.ackDtmf], this.consumer.uuid);
    },
    '/a8$': function (match) { // listen (testing: [*]8)
        debug(this.consumer.session.sid, 'Commands.listen:', JSON.stringify(match), 'send_dtmf', Commands.ackDtmf);
        esl.executeAsyncX('send_dtmf', [Commands.ackDtmf], this.consumer.uuid);
    },
    '/a9$': function (match) { // clear (testing: [*]9)
        var ackClear = 'A000000#@80';
        debug(this.consumer.session.sid, 'Commands.clear:', JSON.stringify(match), 'send_dtmf', ackClear);
    ...
    '/aD$': function (match) { // close (testing: [*0])
        debug(this.consumer.session.sid, 'Commands.close:', JSON.stringify(match), 'send_dtmf', Commands.ackDtmf);
        esl.executeAsyncX('send_dtmf', [Commands.ackDtmf], this.consumer.uuid);
        this.success = true;
        this.timeout = worker.resetTimeout(this.timeout, this.enter.bind(this, null), ConsumerBs8521Pnc.closeMs);
    },
    '/a#$': function (match) { // null (testing: [*]#)
        debug(this.consumer.session.sid, 'Commands.null:', JSON.stringify(match));
        esl.executeAsyncX('send_dtmf', [Commands.ackDtmf], this.consumer.uuid);
    },
});
