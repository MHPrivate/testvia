...
process.on('sipMessagePreProcess', function onSipMessageProProcessNowip(evt) {
    ...
    if (session = worker.routes[evt.headers['Unique-ID']]) // received via an active call-leg
...
Object.assign(CommunicatorNowip, {// _this_ of all methods is the StateMachine instance
    messageMs: 5000, // max time to wait for NOWIP message after proceeding
    ...
    CHANNEL_CREATE: function onCommunicatorNowipChannelCreate(evt, first) {
        ...
        this.timeout = worker.resetTimeout(this.timeout, this.signal.bind(this, 'clear'), CommunicatorNowip.messageMs);
        this.session.signal('route', [evt.headers['Caller-Caller-ID-Number']]); // expect SIP-MESSAGE from same origin as SIP-INVITE
    ...
    MESSAGE: function onCommunicatorNowipMessage(evt, first) { // received NOWIP message
        ...
        if (!evt.parsed || evt.parsed.ATM.type[0] !== '1')
            null;
        else if (evt.parsed.ATM.data[0].length !== 24)
            this.signal('clear');
        else // only ACK an Alarm message with 24char data
