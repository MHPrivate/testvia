...
Object.assign(ConsumerSimple, { // _this_ of all methods is the StateMachine instance
    ...
    activate: function onConsumerSimpleActivate() { // multiple activations by session each time round the list of consumers
        ...
        var sm = this;
        var duplex, listen, simplex, speak; // set based on context.speechMode value
        switch (this.session.context.speechMode) {
            // Comment out None until Chris H happy to activate for existing devices with this setting.
            //case 858810000: // None
            //    debug(this.session.sid, 'activate: speech mode = None');
            //    duplex = false, listen = false, simplex = false, speak = false;
            //    break;
            case 858810003: // Full Duplex
                debug(this.session.sid, 'activate: speech mode = Full Duplex');
                duplex = true, listen = false, simplex = false, speak = false;
                break;
            case 858810005: // Half and Full Duplex
                debug(this.session.sid, 'activate: speech mode = Half and Full Duplex');
                duplex = true, listen = true, simplex = true, speak = true;
                break;
            default: 
                debug(this.session.sid, 'activate: speech mode', this.session.context.speechMode, 'not found - using default values');
                duplex = true, listen = true, simplex = true, speak = true;
        }
