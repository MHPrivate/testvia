...
Object.assign(ConsumerCallback, { // _this_ of all methods is the StateMachine instance
    ...
    activate: function onConsumerCallbackActivate(callback) { // { to_number, to_unit }
        ...
        chain(function (err) {
            ...
            debug.enabled && debug(sm.session.sid, 'activate:', 'lookup', JSON.stringify(sipUsers));
    ...
    CHANNEL_: function onConsumerCallbackChannel(evt, first) { // miscellaneous CHANNEL_*** events
        switch (evt.type) {
            ...
            case 'CHANNEL_ANSWER':
                ...
                esl.executeAsyncX('eval', ['${uuid_bridge ' + this.session.communicator.uuid + ' ${uuid}}'], this.uuid);
                break;
    ...
    timeout: function onConsumerCallbackTimeout() {
        var match = this.data.match(/(b|0#|DB|T1850|T1850T1400)$/) || [, ''];
        switch (match[1]) {
            case 'b': // Bs8521
                this.protocol = require('./protocol/Bs8521');
                break;

            //case '': // 5.2.1.3.4	Switch to TT92 protocol states dispersed callbacks are assumed to be TtNew
            case '0#': // Tt92
                this.protocol = require('./protocol/Tt92');
                break;

            case '': // no-protocol assume dispersed speaking TtNew
            case 'DB': // TtNew
