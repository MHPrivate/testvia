...
function ConsumerCallback(session) {
    ...
    ConsumerCallback.super_.call(this, ConsumerCallback, {
        ...
        sipUser: undefined,         // registered ATA or digital scheme details
...
Object.assign(ConsumerCallback, { // _this_ of all methods is the StateMachine instance
    ...
    activate: function onConsumerCallbackActivate(callback) { // { to_number, to_unit }
        ...
        chain(function (err) {
            ...
            // Until main.modules.aws.getIsAwsEnvironment() is implemented, we will use the following line
            this.bridgePlatformAws = main.secrets.bridgePlatform && main.secrets.bridgePlatform.toUpperCase().startsWith('AWS');
            if (this.bridgePlatformAws || (callback.to_number.startsWith('+') && !callback.to_number.startsWith('+0'))) // AWS platform or regular e164 - just move along - NB means that all PSTN equipment phone numbers *need* to be in e164 format
                return this();
            ...
            if (!sipUsers) { // skipped the MYSQL lookup - so traditional originate to PSTN
                ...
                console.log(sm.session.sid, 'onConsumerCallbackActivate: unknown ATA or digital scheme');
                return sm.session.signal('consume', true);
            } else try { // non-empty MYSQL result-set - so ATA or digital scheme originate
                this.sipUser = JSON.parse(sipUsers[0].value);
                if (this.sipUser.expires * 1000 < Date.now())
                    return sm.session.signal('consume', true);
                sm.sipUser = this.sipUser;
            } catch (ex) {
            ...
            if (!sipUsers) { // traditional PSTN originate, or call handled by Kamailio in AWS
                debug(sm.session.sid, 'activate:', 'bridge', callback, new Date);
                this.gateway = (main.secrets.freeswitch && main.secrets.freeswitch.outboundGateway) || 'magrathea';
                ...
                }, '{}' + `sofia/gateway/${this.gateway}/` + callback.to_number);
            ...
            if (!this.sipUser) // regular e164 - just move along
                return this();
            
            mysql('select * from sipUsers where name="callbackProtocol" and user=? order by length(user) desc', [callback.to_number], this);

        }, function(sipUsers) {
            if (sipUsers && sipUsers.length)
                sm.data = this.sipUser.callbackProtocol = sipUsers[0].value; // set data to the callback protocol
    ...
    CHANNEL_: function onConsumerCallbackChannel(evt, first) { // miscellaneous CHANNEL_*** events
        switch (evt.type) {
            ...
            case 'CHANNEL_ANSWER':
                ...
                const ms = this.data ? 0 : ConsumerCallback.protocolMs;
                this.timeout = worker.resetTimeout.call(this.session.sid, this.timeout, this.signal.bind(this, 'timeout'), ms);
                var sm = this;
                chain(null, function () {
                    if (sm.spandsp)
                        return this();
    ...
    MESSAGE: function onConsumerCallbackMessage(evt, first) {
        debug(this.session.sid,'MESSAGE:', evt.body);
    
        const response = evt.parsed && evt.parsed.ATM;
        if (!response) {
            debug(this.session.sid, 'MESSAGE: No ATM parsed, ignoring');
            return this;
        }
    
        debug(this.session.sid, 'MESSAGE: Signaling to substate', this.substates._.constructor.name);
        this.substates._.signal('MESSAGE', evt, first);
        return this;
    },
    timeout: function onConsumerCallbackTimeout() {
        var match = this.data.match(/(b|\d#|DB|T1850|T1850T1400|nowipgrp|.)$/); // returns null on failed match
        switch (match && match[1]) {
            case 'nowipgrp':
                this.protocol = require('./protocol/NowipGrp');
                break;
            case 'b': // Bs8521
