...
module.exports = function (options, always) { // both optional
    ...
    if (module.exports.options.pidify)
        module.exports.options.pidify = (module.exports.options.name += '-' + process.pid) && false;
    if (always !== undefined) // _always_ is present
        module.exports.options.always = always;
    if (!module.exports.options.processGlobal)
        process.global && delete process.global;
    else if (!process.global)
        process.global = global;
    if (!signal.replify && module.exports.options.always) // not active AND _always_ is true
...
function signal() {
    ...
    if (!signal.replify || module.exports.options.always) // was inactive OR _always_ is true
...
module.exports.options = { // default repletion options
    always: +process.env.REPLETION,
    contexts: { main: require.main.exports }, // expose mainModule.exports
    name: path.basename(require.main.filename, '.js'), // use app-name
    path: '/run', // path for connector
};

if (module.exports.options.always)
    signal();

process.once('terminate', function _repletion() { // action to deactivate repletion
    if (signal.replify) // currently active
        signal.replify.close(); // so deactivate
    delete signal.replify; // discard deactivation
});

process.stdin.isTTY && process.running.then(function () {
