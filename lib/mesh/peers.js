...
module.exports = extend(exports, {
    ...
    handlers: { // _this_ is ioSocket, client-side has no _server_ atribute AND server-side has no _close_ method
        ...
        established: function onEstablished() { // faked by exports() & 'connect' - peer connection established
            debug.enabled && debug.apply(null, [this.mesh.peer, 'onEstablished:'].concat(argsMap(arguments)));
        },
        ...
        log: function onLog(data) {
            console.log.apply(null, [this.mesh.peer, 'mesh:peers:log'].concat(argsMap(arguments)));
        },
        ...
        mosh2: function onMosh2(larcId, cb) { // larcId, cb(err, { host, port, secret })
            ...
            if (typeof cb !== 'function')
                cb = function () { };
            cb.mesh = this.mesh; // expose mesh to chain methods (3 lines below)
            for (var i in this.mesh.clients)
                if ((this.mesh.clients[i].locals.access || {}).larcId === larcId)
    ...
    larc2: larc2,
    mesh: null, // will be require('.') once running - used by: emit(), peers(), larc2()
    peers: peers,
});
...
function emit(event /* , ... */) { // _this_ is _exports_
...
function peers(event /* , ... */) { // _this_ is _exports_
...
function larc2(larcId, event /* , ..., cb */) { // _this_ is _exports_
    ...
    for (var l in this.mesh.lsnrs) { // check if larcId is a client of us of any of our listeners
        ...
        Object.keys(lsnr.mesh.peers).forEach(function (url, idx, arr) { // forward request to each peer
            ...
            timeout = setTimeout(larcCb, larc2.timeout || 5000, 'timeout');
        }, lsnr.mesh.peers);
    }
...
['drop2', 'find2', 'mosh2', 'proxy2'].forEach(function (name, idx, arr) {
    exports[name] = setFunctionName(name, function (larcId /* , ... , cb */) { // _this_ is _exports_
        var args = Array.from(arguments).slice(1);
        larc2.apply(this, [larcId, name].concat(args));
    });
});

// function doit(repeats) {
//     repeats || (repeats = 0);
//     var id = doit.ids.shift();
//     if (!id) return;
//     main.modules.peers.drop2(id, console.log);
//     repeats-- && setTimeout(doit, doit.timeout || 2000, repeats);
// }
// doit.ids = []
