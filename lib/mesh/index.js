...
function exports(svr) { // module function to activate mesh.io on an httpServer
    ...
    if (!setup)
        setup = ['https://cone-a.appello.care:443']; // main.secrets.peers.slice();
...
function onConnection(soc) { // _this_ is the listener
    ...
    var locals = soc.locals= {
        authority: false, // on the next heartbeat, the remote needs to sent the public part of our renewed HTTPS certificate
    ...
    if (locals.ip.isIPv4MappedAddress())
        locals.ipv4 = locals.ip.toIPv4Address();
    soc.on('accessJwt', onAccessJwt);
    soc.once('heartbeat', this.removeListener.bind(this, 'accessJwt', onAccessJwt));
}

function onAccessJwt(accessJwt, gitdate, cb) { // _this_ is the ioSocket
    // { iat, exp, peer, startUtcms } - peer
    // { iat, exp, larcId, name, ?schemeId, ?dialPrefix } - larc
    debug.apply(null, ['onAccessJwt:', this.handshake.address].concat(Array.from(arguments)))
    var soc = this;
    chain(cb, function () {
        ...
        soc.mesh.peer || (soc.mesh.peer = access.peer); // fixup for legacy client not based on mesh.io
        soc.locals.access = access;
        access.larcId ? larcs.call(soc, gitdate, this) : peers.call(soc, this);
