...
function onAccessJwt(accessJwt, gitdate, cb) { // _this_ is the ioSocket
    ...
    console.log.apply(console, ['onAccessJwt:', this.handshake.address].concat(Array.from(arguments).slice(0, -1)));
    var soc = this;
    chain(function cleanup(err) {
        if (!err)
            return cb && cb.apply(this, arguments);
        console.log('onAccessJwt:', err);
        ...
        mio.fixPeer(soc, access.peer); // fixup for legacy clients not based on mesh.io
