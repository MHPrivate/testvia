...
function onceAccessJwt(accessJwt, gitdate, cb) { // _this_ is the ioSocket
    ...
    console.log.apply(null, ['onceAccessJwt:', this.handshake.address].concat(argsMap(arguments)));
    var soc = this;
    chain(function cleanup(err) {
        ...
        meshio.fixClient(soc, access.peer); // fixup for legacy clients not based on mesh.io - access.peer will be undefined for non-peers
        soc.mesh.peer && (soc.locals.peer = soc.mesh.peer);
        if (access.peer)
            peers.call(soc, this);
        else if (access.larcId)
            larcs.call(soc, gitdate, this);
