...
module.exports = extend(exports, {
    handlers: { // _this_ is the larc socket for each handler
        ...
        cdr: function onCdr(cdr, cb) {
            ...
            var soc = this;
            typeof cb === 'function' || (cb = console.log.bind(null, 'onCdr:'));
            chain(function cleanup(err) {
                err && err.message && Object.definePropery(err, 'message', { enumerable: true });
...
function onAppWakeup(data, cb) {
    debug.enabled
        ...
        : console.log('mesh:appWakeup:', this.locals.access.name, this.locals.access.dialPrefix, JSON.stringify(data));
    var soc = this;
...
function onDeviceEvent(data, cb) { // {uuid,oat,firebase,apns,sandbox}
    debug.enabled
        ...
        : console.log('mesh:deviceEvent:', this.locals.access.name, this.locals.access.dialPrefix, JSON.stringify(data));
    var soc = this;
    chain(cb || null, function () {
        this.index = soc.locals.access.name; // so mysql calls can log the originator
        this.event = {
