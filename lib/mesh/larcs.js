...
module.exports = extend(exports, {
    handlers: { // _this_ is the larc socket for each handler
        ...
        disconnecting: function onDisconnecting(reason) { // update nexus:larcs to record disconnect
            debug.enabled && debug.apply(null, ['onDisconnecting:', this.locals.access.name, this.locals.access.dialPrefix].concat(Array.from(arguments)));
        ...
        log: function onLog(data) {
            debug.enabled
                ? debug.apply(null, ['onLog:', this.locals.access.name, this.locals.access.dialPrefix].concat(Array.from(arguments)))
                : console.log.apply(console, ['mesh:larcs:log:', this.locals.access.name, this.locals.dialPrefix].concat(Array.from(arguments)));
        },
        mosh: function onMosh(data) { // legacy: LARCs upto '2018-06-26T18:00:04Z' inclusive
            debug.enabled && debug.apply(null, ['onMosh:', this.locals.access.name, this.locals.access.dialPrefix].concat(Array.from(arguments)));
        ...
        probe: function onProbe(probe) { // legacy: LARCs upto '2018-06-26T18:00:04Z' inclusive
            debug.enabled
                ? debug.apply(null, ['onProbe:', this.locals.access.name, this.locals.access.dialPrefix].concat(Array.from(arguments)))
        ...
        master: function onMaster(master) {
            debug.enabled
                ? debug.apply(null, ['onMaster:', this.locals.access.name, this.locals.access.dialPrefix].concat(Array.from(arguments)))
                : console.log('mesh:larcs:master:', this.locals.access.name, this.locals.dialPrefix, master);
        },
        offsite: function onOffsite(offsite) {
            debug.enabled
                ? debug.apply(null, ['onOffsite:', this.locals.access.name, this.locals.access.dialPrefix].concat(Array.from(arguments)))
                : console.log('mesh:larcs:offsite:', this.locals.access.name, this.locals.dialPrefix, offsite);
        },
        config: function onConfig(config) {
            debug.enabled
                ? debug.apply(null, ['onConfig:', this.locals.access.name, this.locals.access.dialPrefix].concat(Array.from(arguments)))
        ...
        configItems: function onConfigItems(items) { // {dialPrefix,ipv4,ipv6,mac,mailto,vipv4}
            debug.enabled
                ? debug.apply(null, ['onConfigItems:', this.locals.access.name, this.locals.access.dialPrefix].concat(Array.from(arguments)))
        ...
        iok: function onIok(data, cb) {
            debug.enabled
                ? debug.apply(null, ['onIok:', this.locals.access.name, this.locals.access.dialPrefix].concat(Array.from(arguments)))
        ...
        cdr: function onCdr(cdr, cb) {
            debug.enabled && debug.apply(null, ['onCdr:', this.locals.access.name, this.locals.access.dialPrefix].concat(Array.from(arguments)));
            var soc = this;
            chain(cb, function () {
                this.index = soc.locals.access.name; // so mysql calls can log the originator
...
process.on('certificate', function onCertificateLarcs(certificate) {
    ...
    debug.enabled && debug.apply(null, ['onCertificateLarcs:'].concat(Array.from(arguments)));
