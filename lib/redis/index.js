...
process.running.then(function () {
    try {
        ...
        require('./proxyConfig');   // process.emit('rpscb', 'proxyUpdate', data) // { prefix: fqdn:port, ... }
                                    // process.emit('rpscb', 'proxyCheck', data) // { dialPrefix, name, larcId, proxyActual, proxyConfig, schemeId}
        require('./scheme');        // process.emit('rpscb', 'scheme:N', event, ...)
