...
rpscb.on('proxyUpdate', function onProxyUpdate(prefixFqdnPorts) { // { prefix: fqdn:port, ... }
    ...
    hostnames.forEach(function (hostname, idx, arr) { // regenerate cache of IPs for each hostname
        ...
        dns.lookup(hostname, { all: true }, function (err, addresses) {
            ...
            debug.enabled && debug('onProxyUpdate:', JSON.stringify({ psuedonyms: Array.from(psuedonyms) }));
...
rpscb.on('proxyCheck', async function onProxyCheck(data) { // {dialPrefix,proxyActual,proxyConfig,schemeId}
    ...
    // Add any AWS interface
    // Will do nothing if not in AWS environment
    await main.modules.aws.addAWSElasticIP(locals.ifaces);
