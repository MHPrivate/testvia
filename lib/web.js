...
process.once('running', function () { // setup the https service
    ...
    web.https = https.createServer(main.cache.tls, function (req, res) {
        res.writeHead(404, { 'Content-Length': 0 });
        res.end();
    }).listen(main.setup.https, function () {
        closer({ timeout: 2000 }, this); // closes any keep-alive browser connections
        this.io = mio(this);
    });
    
    process.once('terminate', function _https() {
        web.https.io ? web.https.io.close() : web.https.close();
    });
});
...
process.once('running', function () { // activate port 80 redirection if we are servicing port 443
    ...
    process.once('terminate', function _http() {
        web.http.io ? web.http.io.close() : web.http.close();
    });
});

process.once('mysql', function () { // database is ready - fetch our TLS certificate
    chain(null, function () {
        this.index = 'startup';
