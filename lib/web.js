...
app.use(function (req, res, next) {
    next(Object.assign(new Error(statuses[404]), { status: 404 }));
});
...
process.on('certificate', function onCertificateWeb(dict) { // { fqdn, cert, chain, key }
    ...
    Object.assign(ctx, x509.parseCert(dict.cert));
...
process.running.then(function () { // activate port 80 redirection if we are servicing port 443
    ...
    exports.http = http.createServer(function (req, res) {
        if (!req.headers.host)
            ...
            res.writeHead(301, {
                'Content-Length': 0,
                Location: Object.assign(url.parse('https://' + req.headers.host + req.url), {
...
mysql.running.then(function () { // database is ready - fetch our TLS certificate used by HTTPS:SNICallback
    Object.assign(main.cache, { snis: {}, tls: {} });
